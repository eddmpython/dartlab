...
상세 문서:
- `src/dartlab/providers/dart/docs/DEV.md`
- `src/dartlab/providers/dart/finance/DEV.md`
- `src/dartlab/providers/dart/report/DEV.md`
...
- comparable slot spine(`textComparablePathKey`) 기준 구조 이벤트도 `sections` 안에서 계산한다.
  ...
  - `freqScope='annual'/'quarterly'` 조회는 `mixed` row를 포함하더라도 해당 lane period만 activity에 반영한다.
  - `structureSummary()`는 최신 구조 상태와 최근 구조 이벤트를 한 줄로 요약한다.
  - `structureChanges()`는 최신 구조 변화 row만 압축해서 바로 준다.
    - 기본 `changedOnly=True`는 recent event가 실제로 있는 row만 남긴다.
...
| docs topic | raw `sections` DataFrame (`chapter/topic/blockType/blockOrder/sourceBlockOrder/textNodeType/textStructural/segmentOccurrence/freqScope + period cols`) | `c.show("companyOverview")` |
...
- `show("costByNature")`, `show("rnd")`, `show("segments")`처럼 sections topic에 직접 안 잡히는 항목도 `report -> notes -> _get_primary()` fallback으로 끝까지 찾는다.
- 즉 `show()`는 sections-only lookup이 아니라 **authoritative/direct fallback-aware 조회**다. topic이 sections에 없다고 바로 `None`으로 끝내지 않는다.

### live filings / 원문 읽기

```python
c.filings()                               # 로컬 docs/parquet 기준 filings
c.liveFilings(days=30, keyword="수주")    # OpenDART 기준 최신 공시 목록
c.readFiling("20240312000736")            # rceptNo 기준 원문 본문
```

- DART 본문 회수의 정식 키는 `rceptNo`다.
- `viewerUrl`은 내비게이션/참고용으로 함께 노출하지만, 내부 본문 회수는 `document.xml` 경로를 사용한다.

### metadata / authoritative fast path

- `trace("BS" | "IS" | "CF" | "CIS" | "SCE")`는 exact 검증이 끝난 finance series만으로 provenance를 만든다.
- 위 경로는 `profile.facts`, `retrievalBlocks`, `sections`를 만들면 안 된다.
- `show("BS" | "IS" | "CF" | "CIS" | "SCE" | "ratios")`는 exact 검증이 끝난 단일 finance block 경로라서 `sections`를 우회한다.
...
- `c.docs.sections.topics()`는 전체 `sections` 수평화 DataFrame을 만들지 않고 topic 이름만 반환한다.
- `c.docs.sections.outline()`는 lightweight manifest를 반환한다.
...
위치: `providers/dart/report/types.py` — `_seriesToWide()` 공통 헬퍼.
...
- 위치: `providers/dart/docs/sections/diff.py`

**4b 숫자 변곡점** (finance 위):
- `detectInflections(df)` → list[Inflection]
- 위치: `core/finance/inflection.py`
...
## 데이터 수집 — eddmpython 포팅 완료

공시문서 수집 로직은 eddmpython DartDocs.py에서 dartlab `openapi/collector.py`로 완전 포팅됨.
eddmpython 외부 의존 없이 requests 기반 동기 크롤링으로 동작한다.
상세: `openapi/DEV.md` § 공시문서 수집기.
