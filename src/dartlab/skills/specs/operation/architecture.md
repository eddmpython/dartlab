...
| Layer | 구성 | 역할 |
|---|---|---|
| L0 | `core` | L0 primitive 만: logger·env·types·memory·polarsUtil·formatting·constants·protocols·naming·utils·cache·di·credentials·dualAccess·palette·**accounts** + DIP Protocol (disclosureFetcher·gatherProvider·financeDocAccessor·listingResolver). 상위 import 금지. **`accounts`** = 계정 정규화 단일 SSOT (DART 12단계 + EDGAR tag + 라벨 cascade + snakeId alias) — DART/EDGAR provider(L1) 가 *공유 snakeId 공간* 을 쓰려면 양쪽이 합법 import 가능한 L0 가 유일 위치라 데이터-구동 매핑을 primitive 로 둠 (provider 분기 로직 아님, JSON 룩업). |
...
| 산출물 | 대상 | 결 |
|---|---|---|
| `src/dartlab/skills/catalog.json` | 전체 검색 카탈로그 (landing·wheel 정본) | frontmatter + body preview 1500 자 |
| `src/dartlab/skills/agent.json` | `catalog.json` byte-동일 legacy alias (실 리더 0) | dedup 운영자 결정 대기 |
