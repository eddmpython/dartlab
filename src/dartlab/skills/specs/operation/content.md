...
procedure:
  ...
  - 발행 게이트 통과 + 로컬 시각 검수 + 자동 발행·커밋·푸시
examples:
  - "다음 카드뉴스 주제 잡고 만들어줘"
...
공시 읽기, dartlab 소식, dartlab 이야기, 신용분석 보고서, 기업이야기, 데이터 리포트, 산업 지도, 기술이야기, 투자이야기까지 모든 블로그가 아래 세 원칙을 공유한다. 장르별 규칙은 주어, 근거, 분량, 시각물을 바꿀 수 있지만 이 계약은 덮어쓸 수 없다.
...
3. **천 단위 콤마**: 독자에게 수량으로 보이는 숫자의 정수부가 4자리 이상이면 `1,000`, `12,345.67`처럼 세 자리마다 콤마를 쓴다. 제목, description, 본문, 표, 캡션, 인용문, 검증표에 모두 적용한다. 연도·날짜·종목코드·버전·모델명·URL·코드·해상도처럼 수량이 아닌 식별자는 예외다. 블로그 기획 루프가 먼저 요구하고 공통 `publishGate.py`가 전 카테고리를 하드 실패시킨다.
...
세 서피스는 이미 같은 코어를 **다른 필드명으로 3벌 들고 있다**(brief.json·cards.plan.json·sourceDoc, 그리고 카드 편집 story_brief.json). StoryCore는 그 공유 코어의 **표준 어휘**다. 새 아티팩트 `storyBrief`를 신설하지 않는다(그러면 5번째 이름이라 덕지덕지). 각 서피스의 기존 기획 아티팩트가 StoryCore의 투영이고, 필드명만 아래로 통일한다. 물리 통합은 별도 구현 승인 전까지 하지 않는다.
...
기술이야기 17편부터는 풍부한 시각 설계를 기획 하드 계약으로 둔다. `brief.json.visuals[]`는 10개 이상, `imagePlan[]`은 hero 1장과 inline 4장 이상, `visuals.kind`는 3종 이상이어야 한다. 장면 이미지는 현장과 공간을 먼저 이해시키고, 작동 원리 도식은 단계와 인과를 풀며, 표와 차트는 회사·수치·판단을 비교한다. 한 종류가 다른 역할을 대신하지 않는다. 모든 주요 H2는 `visualAnchor`를 가지며 개념 점프가 두 번 이상인 섹션은 두 번째 시각 앵커를 기획한다. 각 자산은 `placement`, `insertAfter`, `narrativeUse`로 본문 문장과 연결한다. 개수만 채우는 범용 배경과 중복 도식은 실패다. 이 강화 계약은 기존 글을 일괄 파손하지 않도록 기술이야기 `seriesOrder >= 17`부터 `auditBlog.py`와 기획 워크플로가 함께 강제한다.
...
|---|---|---|---|---|
| 블로그 | `blog/{cat}/{post}/index.md` + `brief.json` + 글 루트 `CREDITS.md` + 중앙 `media/catalog.json` | `stockCode` / `topicSlug` | `publishBlogAssets.py` -> `publishGate.py` | `blog_plan_loop.workflow.js` |
...
- 공개 발행(hfMedia)·landing 산출물은 로컬 시각 검수와 서피스별 발행 게이트를 통과하면 별도 승인 대기 없이 발행·커밋·푸시한다. dev 실물 렌더가 가능하면 눈검수하고, 브라우저가 없으면 원본·SVG 래스터·OG 직접 검수와 정적 빌드로 대체한다.
...
- 4 발행: 줄깨짐·약어·맨숫자·범용표지(기계), 로컬 시각 검수 실패·발행 게이트 실패 → 발행 보류.
