...
5. **관련성 있는 배경이미지**: 카드마다 그 내용과 맞는 배경 이미지를 찾아 붙인다. 범용·장식 사진은 실패. 회사를 상징하는 상호·간판이 크게 들어가는 것도 좋다. 맞는 실사를 끝내 못 찾을 때만 깨끗한 다크 텍스트로 둔다.
...
## 2. 기획 루프 (구조): 기획작가 ↔ 평가자, 통과까지 반복
기획은 한 번 쓰고 끝이 아니라 루프다. 기획작가가 1번에서 잡은 전체 기획안(인사이트·스파인·그래프·계약)을 내면, 전문 평가자가 5대원칙으로 독립 비평하고 재작성을 강제한다. 그 피드백을 기획작가가 받아 다시 쓴다. 도장 찍는 루프가 아니라 합격선 미달이면 revise 로 되돌리는 실루프다. 최소 2회전, 모두 통과(pass)할 때까지.
- 실행(구조화된 오케스트레이션): `Workflow({scriptPath: "blog/_scripts/cards_plan_loop.workflow.js", args:{topic, evidence}})`. 기획작가 agent 초안, 평가자 agent 비평(verdict + 구체 findings), 기획작가 agent 개선, pass 까지 루프. 산물은 최종 기획안 + 루프 로그(라운드별 findings·점수·verdict)다.
- 평가자가 깐다(각 항목 reject 권한): 인사이트(통념·반전·메커니즘·렌즈가 다 살아있고 뻔하지 않나), 스캔독해(큰문장만으로 완결되나), 훅(표지가 1초에 멈추나), 시각(그래프가 주장을 증명하고 밀도 있나), 정직성(숫자 전부 검증 데이터 안인가, 과장·투자권유 없나). 5점 중 하나라도 미달이면 revise.
