"""시장 분석 -- quant 엔진을 review 6막 서사에 통합.

주가 OHLCV 기반 기술적 분석 결과를 analysis calc 패턴으로 래핑한다.
재무 분석(스코어카드)과 기술적 판단(강세/중립/약세)의 교차검증이 핵심 가치.

기존 quant 엔진(src/dartlab/quant/)은 독립적으로도 사용 가능하며,
이 모듈은 review에서 소비할 수 있는 calc dict 형태로 변환한다.
"""
...
def _fetchOHLCV(company: Any) -> pl.DataFrame | None:
    ...
    모든 calc 함수가 이 캐시를 공유하므로 네트워크 호출은 1회만 발생.
    """
    ...
    # fallback: quant 엔진의 GatherEntry 경유
    if result is None:
        try:
...
def _fetchBenchmark(company: Any) -> pl.DataFrame | None:
...
def calcMarketBeta(company) -> dict | None:
    ...
    benchmark = _fetchBenchmark(company)
