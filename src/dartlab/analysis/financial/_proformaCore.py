...
def computeCompanyWacc(
    ...
    rf, erp = _resolveRfAndErp(riskFreeRate, marketPremium, damodaran, mkt, countryRiskPremium)

    beta, betaSource = _resolveBeta(
        series,
    ...
    totalDebt = stb + ltb + bonds

    # 자기자본 = 시가총액 or 장부가
    equityBook = getLatest(series, "BS", "total_stockholders_equity") or getLatest(
    ...
    equityValue = marketCap if marketCap else equityBook
    if not equityValue or equityValue <= 0:
        equityValue = None

    kd = _costOfDebt(series, totalDebt, rf)

    # Ke (자기자본비용). CAPM: Rf + beta * (ERP + CRP)
    ke = rf + beta * erp

    taxRate = _effectiveTaxRate(series, mkt)

    # WACC. 자기자본 가치를 세울 수 없으면 자본구조 가중을 만들 수 없다. 억지로 만들면
    # 가중치가 구간을 벗어나고 결과가 자본 크기에 대해 단조롭지도 않게 된다.
    eWeight, dWeight, equityUnknown, equityValue = _capitalWeights(equityValue, totalDebt)

    wacc = eWeight * ke + dWeight * kd * (1 - taxRate)
    # Phase 4 G12.1: 대기업 AA급 현실 (Rf 2.4% + 1.5%p 로 약 4%) 반영. 하한 5 에서 4 로.
    ...
    details = {
        ...
        "equity_weight": eWeight * 100,
        "debt_weight": dWeight * 100,
