...
## 3. 폴더 구조 (2026-03-30 현행)

```
analysis/
├── financial/      # 재무제표 분석 통합 (20축 + 인사이트 + 리서치)
│   ├── insight/    # 10영역 A~F 등급 → c.insights
│   ├── research/   # ResearchResult 조립 → generateResearch()
│   ├── revenue.py, capital.py, ...    # 14축 calc 함수들
│   ├── predictionSignals.py           # 6-2 예측신호 (10 calc)
│   ├── scorecard.py                   # 종합 스코어카드
│   └── _helpers.py                    # 공통 유틸
├── forecast/       # 전망분석 인프라
│   ├── revenueForecast.py             # 4소스 앙상블 매출 예측
│   ├── simulation.py                  # 매크로 시나리오 시뮬레이션
│   ├── prediction.py                  # ContextSignals → 시나리오 확률 조정
│   ├── predictionSpace.py             # 6대 보편 예측축 (경기/금리/환율/원자재/심리/유동성)
│   ├── calibrator.py                  # 시장 데이터 기반 시나리오 보정
│   └── forwardTest.py                 # 예측 저장/백테스트
├── valuation/      # 가치평가 (DCF, 멀티플, DDM, RIM, 종합)
└── qa/             # QA 기록 (기업별 스토리 분석 검증)
```

**삭제된 폴더 (2026-03-29):**
- `accounting/` — 키워드 톤 분석. 가치 없음. 이익의 질/Red Flags는 이미 financial에 있음
- `comparative/` — sector는 core/로 이동, event는 소비자 0이라 삭제
- `macro/` — 빈 폴더
- `risk/` — 빈 폴더. 부실/이상치 분석은 이미 financial에 있음
- `strategy/` → `financial/`로 병합 (2026-03-29). 14축 엔진 + insight + research를 하나로 통합

**이동된 모듈:**
- `sector/` → `core/sector/` — 모든 레이어(L0~L3)가 소비하는 인프라. analysis가 아니라 core

### gather/는 별도 엔진
gather/는 데이터 수집 엔진이지 분석 엔진이 아니다. 현재 위치(L1) 유지.
...
| 폴더 | 상태 | 비고 |
|------|------|------|
| financial/ | **20축 구현 완료** | 수익구조~예측신호. analysis() 단일 진입점 |
| financial/insight/ | **동작** | 10영역 등급 카드 (A~F). AI/대시보드 요약용 |
| financial/research/ | **동작** | ResearchResult 조립. review()가 소비 |
| forecast/ | **구현됨** | 매출예측, 시뮬레이션, PredictionSpace 6대축, 시나리오 확률 조정 |
| valuation/ | 구현됨 | DCF/DDM/멀티플/RIM/종합/역내재 |
| risk/ | **삭제** | 빈 폴더. 리스크 분석은 insight distress/anomaly에 흡수 |

### analysis() vs insight vs research — 역할 분리
...
**analysis() 20축** = 스토리의 원천. 수익구조~예측신호까지 모든 숫자를 구조화.
- 5개년 waterfall, 듀퐁 5요소 시계열, Sloan/Beneish 시계열, 부문별 성장 기여도
- NAVER QA에서 "영업외 손실이 영업이익의 104% 상쇄" 같은 발견이 여기서 나옴
- 소비자: review, AI, 사람 3자 모두

**insight (등급 카드)** = analysis 데이터의 요약 뷰. 10영역 A~F 등급 + 플래그.
- "performance B", "profitability A" 같은 한줄 판정
- 스토리 없이 등급만 — 대시보드, scan 정렬, AI 빠른 요약에 적합
- 분석의 근거는 못 줌. "왜 D인가?"는 analysis 20축이 답함

**research (조립기)** = insight + quant scores + narrative + valuation을 ResearchResult로 묶음.
- review()가 이걸 소비해서 14개 섹션 보고서를 생성
- 단독 호출보다는 review 파이프라인의 중간재

**진입점 원칙:**
- 공개 진입점은 `c.analysis()` / `c.analysis("축")` 하나
- `c.insights`는 등급 카드 property (빠른 요약 전용)
- `dartlab.insights()` 루트 함수는 `c.insights`의 편의 래퍼
- research의 `generateResearch()`는 내부 API — review()가 소비

### analysis() 20축 구현 현황 (2026-03-30 확정)

`analysis()` callable class가 financial/ 20축을 통합 진입점으로 노출.
`src/dartlab/analysis/financial/__init__.py`의 `_AXIS_REGISTRY`가 source of truth.

| partId | 축 | calc 함수 수 | 모듈 | 상태 |
|--------|-----|-------------|------|------|
| 1-1 | 수익구조 | 8 | financial/revenue.py | 구현 완료 |
| 1-2 | 자금조달 | 9 | financial/capital.py | 구현 완료 |
| 1-3 | 자산구조 | 4 | financial/asset.py | 구현 완료 |
| 1-4 | 현금흐름 | 3 | financial/cashflow.py | 구현 완료 |
| 2-1 | 수익성 | 5 | financial/profitability.py | 구현 완료 |
| 2-2 | 성장성 | 4 | financial/growthAnalysis.py | 구현 완료 |
| 2-3 | 안정성 | 6 | financial/stability.py | 구현 완료 |
| 2-4 | 효율성 | 3 | financial/efficiency.py | 구현 완료 |
| 2-5 | 종합평가 | 3 | financial/scorecard.py | 구현 완료 |
| 3-1 | 이익품질 | 4 | financial/earningsQuality.py | 구현 완료 |
| 3-2 | 비용구조 | 4 | financial/costStructure.py | 구현 완료 |
| 3-3 | 자본배분 | 5 | financial/capitalAllocation.py | 구현 완료 |
| 3-4 | 투자효율 | 4 | financial/investmentAnalysis.py | 구현 완료 |
| 3-5 | 재무정합성 | 5 | financial/crossStatement.py + taxAnalysis.py | 구현 완료 |
| 4-1 | 가치평가 | 9 | financial/valuation.py | 구현 완료 |
| 5-1 | 지배구조 | 4 | financial/governance.py | 구현 완료 |
| 5-2 | 공시변화 | 4 | financial/disclosureDelta.py | 구현 완료 |
| 5-3 | 비교분석 | 3 | financial/peerBenchmark.py | 구현 완료 |
| 6-1 | 매출전망 | 7 | financial/forecastCalcs.py | 구현 완료 |
| 6-2 | 예측신호 | 12 | financial/predictionSignals.py | **구현 완료** |

### 6-2 예측신호 축 상세 (2026-03-30)

12개 calc 함수로 구성. 학술 근거 기반 다중 소스 예측 신호.

| calc | 역할 | 학술 근거 | 실제 작동 |
|------|------|----------|----------|
| calcEarningsMomentum | Sloan 분해 + DuPont 추세 → 이익 방향성 | Sloan 1996 | 삼성전자/현대차/SK텔레콤 검증 |
| calcPeerPrediction | 횡단면 피어 회귀 → 괴리 | Cao & You 2024 (G&D) | 모델 캐시 없으면 None |
| calcStructuralBreak | Chow Test → 구조변화 | M Competition | 작동 |
| calcMacroSensitivity | 섹터 탄성치 + PredictionSpace 라이브 축 | ISM PMI 연구 | 작동 (ECOS API 키 필요) |
| calcDisclosureDelta | docs.diff() → 변화량 신호 | MD&A 연구 | docs 데이터 없으면 None |
| calcMacroRegression | 거시-재무 동적 회귀 | PredictionSpace 6축 연동 | 사용자 추가 |
| calcEventImpact | 이벤트 충격 분석 | 공시/뉴스 이벤트 → 매출 영향 | 사용자 추가 |
| calcInventoryDivergence | 재고/매출채권 괴리 → 수요 둔화 | NYU Stern, Oler 2024 | 작동 |
| calcAnnouncementTiming | 동종업계 선발 기업 실적 방향 | Ramnath 2002 | scan 데이터 없으면 None |
| calcSupplyChainSignal | 관계사 모멘텀 전파 | Cohen & Frazzini 2008 | network 데이터 없으면 None |
| calcPredictionSynthesis | 단순 평균 앙상블 | Green & Armstrong 2015 | 작동 |
| calcPredictionFlags | 경고 플래그 | - | 작동 |

### PredictionSpace — 6대 보편 예측축 (forecast/predictionSpace.py)

기업 독립적 인프라. ECOS 실시간 데이터로 전체 경제 상태를 6축으로 계산.

| 축 | 한국어 | 핵심 지표 | 기업 영향 경로 |
|---|---|---|---|
| businessCycle | 경기축 | CLI, IPI, MANUFACTURING | revenueToGdp 탄성치 |
| interestRate | 금리축 | BASE_RATE, TREASURY_3Y | nimToRate (금융), 간접 수요 |
| fxRate | 환율축 | USDKRW | revenueToFx 탄성치 |
| commodity | 원자재축 | PPI | 원가 압박 or 매출 (섹터별) |
| sentiment | 심리축 | CSI, BSI | 소비/투자 심리 |
| liquidity | 유동성축 | M2 | 유동성 환경 |

`getPredictionSpace()` → 세션 싱글톤 캐시 (첫 호출 2-5초, 이후 마이크로초)
`impactOn("반도체")` → 축별 매출 영향 % (예: 경기축 +4.51%, 환율축 +1.33%)

소비자: calcMacroSensitivity, prediction.py, simulation.py, AI 컨텍스트

### 4계층 관계

```
scan()       시장 전체 횡단 (13축)     -- 종목 간 비교/스크리닝
analysis()   단일 종목 심층 (20축)     -- 재무제표 완전 분석 + 예측 신호
...
## 6. 데이터 접근 원칙 — select()가 유일한 도구

### 원칙

analysis()의 모든 calc 함수는 `company.select()`로 데이터에 접근한다.
select()는 IS/BS/CF(finance) 뿐 아니라 docs sections, report까지 **동일한 패턴**으로 접근하는 단일 경로다.

```python
# finance — 재무제표 숫자
c.select("IS", ["매출액", "영업이익"])
c.select("BS", ["자산총계", "자본총계"])
c.select("CF", ["depreciation", "change_in_working_capital"])

# docs — 사업보고서 테이블 (sections 수평화)
c.select("productService", ["DX_매출액", "DS_매출액"])
c.select("salesOrder", ["TV, 모니터 등"])
```

### 내부 동작

```
select(topic, indList, colList)
    ↓
show(topic)  →  finance: _showFinanceTopic()   → 숫자 DataFrame
             →  docs:    sections 테이블 수평화  → 항목 × 기간 DataFrame
             →  report:  _showReportTopic()     → 정형 데이터
    ↓
selectFromShow(df, indList, colList)  →  행/열 필터  →  SelectResult
```

### docs topic 테이블 접근

sections mapper가 사업보고서를 topic으로 분류하고, 각 topic 내부를 `blockLabel`(subtopic)로 세분화한다.

```
topic (대분류)           예: productService, salesOrder, rawMaterial
  └─ blockLabel (내부)   예: "가. 주요 제품 매출", "(1) 주요 제품별 매출실적"
       └─ 기간별 셀      예: 2025Q4 → 마크다운 테이블 텍스트
```

retrievalBlocks의 subtopic 우선순위: `detailTopic` > `semanticTopic` > `blockLabel`

테이블 블록은 `_horizontalizeTableBlock()`으로 수평화되어 `항목 × 기간` DataFrame이 된다.
block 번호는 기간마다 바뀔 수 있으므로 **고정값으로 쓰지 않는다** — 의미 기반 탐색이 원칙.

### 데이터 소스별 가용 계정 예시

| 소스 | 계정 예시 | 용도 |
|------|----------|------|
| IS | 매출액, 매출원가, 판관비, 영업이익, 당기순이익 | 손익 분석 |
| BS | 자산총계, 자본총계, 유형자산, 무형자산, 리스부채(기업별) | 재무 구조 |
| CF | depreciation, change_in_working_capital, interest_paid, dividends_paid | 현금흐름 분해 |
| CF (조정항목) | depreciation, retirement_benefits, stock_compensation_expenses | IS에 비용 세부 없는 기업의 대안 |
| docs | productService(사업부별 매출), salesOrder(지역별/유형별), rawMaterial(원재료) | 사업 구조 |

### 부족하면 select를 강화한다

analysis에서 필요한 데이터가 select()로 안 나오면, 새 경로를 만들지 않고 **select() 자체를 강화**한다.
segments 계열은 폐기됨 — `select("segments:*")` 사용 금지.
...
## 7. Company 부착 (2026-03-29 현행)

| 진입점 | Company 노출 | 역할 |
|--------|-------------|------|
| **analysis** | `c.analysis()`, `c.analysis("축")` | 14축 스토리 데이터 (핵심) |
| **review** | `c.review()`, `c.review("섹션")` | 구조화 보고서 |
| **reviewer** | `c.reviewer()` | review + AI 해석 |
| insight | `c.insights` | 등급 카드 (A~F 요약) |
| sector | `c.sector`, `c.sectorParams` | WICS 분류 (core/sector) |

모든 부착은 lazy import -- Company 초기화 시 로드되지 않음.

**삭제된 진입점 (2026-03-29):**
- `c.eventStudy()` — event/ 삭제로 제거
...
### 2026-03-29 — 구조 정리 + insight/research 역할 확정

**삭제:**
- `accounting/` — 키워드 톤 분석(disclosureSignal). 분석 가치 없음
- `comparative/event/` — 소비자 0, QA 14축에서 한번도 사용 안 됨
- `comparative/` — sector를 core/로 이동한 후 빈 폴더 제거
- `macro/` — 빈 폴더
- `c.eventStudy()` — DART/EDGAR 양쪽에서 제거

**이동:**
- `comparative/sector/` → `core/sector/` — 모든 레이어가 소비하는 인프라 (28개 import 지점)

**역할 확정 (실측 기반):**
- analysis() 14축이 스토리의 원천. NAVER QA에서 검증됨
- insight는 등급 카드(A~F). 대시보드/AI 빠른 요약 전용. 스토리 근거는 못 줌
- research는 review 파이프라인의 조립기. 단독 공개 API 아님
- 진입점은 `c.analysis("축")`이 핵심, `c.insights`는 보조

**판단 근거 (삼성전자 실측):**
- insight: "profitability A" — 한줄 판정만
- analysis("수익성"): 5개년 waterfall, 듀퐁 분해, 분기별 마진 추세 — 스토리 재료 전부
- research: insight + narrative + quant scores 조립 → review가 소비
