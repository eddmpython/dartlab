...
## 3. 폴더 구조 (2026-03-29 현행)

```
analysis/
├── financial/      # 재무제표 분석 통합 (14축 + 인사이트 + 리서치)
...
│   └── _helpers.py                  # 공통 유틸
├── forecast/       # 전망분석 (추정, 시뮬레이션, 예측신호)
├── valuation/      # 가치평가 (DCF, 멀티플, DDM)
...
| 폴더 | 상태 | 비고 |
|------|------|------|
| strategy/ | **14축 구현 완료** | 수익구조~재무정합성. analysis() 단일 진입점 |
| financial/insight/ | **동작** | 10영역 등급 카드 (A~F). AI/대시보드 요약용 |
| financial/research/ | **동작** | ResearchResult 조립. review()가 소비 |
| forecast/ | 구현됨 | OLS/시나리오/ProForma |
| valuation/ | 구현됨 | DCF/DDM/멀티플/멀티소스 |
| predictionSignals | **구현됨** | 6-2. 다중 소스 예측 신호 (모멘텀/피어/구조변화/매크로/공시) |
...
**analysis() 14축** = 스토리의 원천. 수익구조~재무정합성까지 모든 숫자를 구조화.
...
- 분석의 근거는 못 줌. "왜 D인가?"는 analysis 14축이 답함
...
### analysis() 19축 구현 현황 (2026-03-30 확정)

`analysis()` callable class가 financial/ 19축을 통합 진입점으로 노출.
...
| 2-1 | 수익성 | 5 | financial/profitability.py | 구현 완료 (듀퐁5요소 + 마진워터폴 추가) |
| 2-2 | 성장성 | 4 | financial/growthAnalysis.py | 구현 완료 (SGR+갭 추가) |
| 2-3 | 안정성 | 6 | financial/stability.py | 구현 완료 (앙상블 + 부채만기 추가) |
...
| 6-2 | 예측신호 | 7 | financial/predictionSignals.py | **신규** |

### 4계층 관계

```
scan()       시장 전체 횡단 (13축)     -- 종목 간 비교/스크리닝
analysis()   단일 종목 심층 (19축)     -- 재무제표 완전 분석
