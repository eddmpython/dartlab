...
class ClaudeCodeProvider(BaseProvider):
    ...
    def _ensure_available(self) -> None:
        if not shutil.which("claude"):
            from dartlab.ai.providers.support.cli_setup import get_claude_code_install_guide
    ...
    def complete(self, messages: list[dict[str, str]]) -> LLMResponse:
        ...
        if result.returncode != 0:
            ...
            stderr = (
                raw_err.decode("utf-8", errors="replace").strip() if isinstance(raw_err, bytes) else raw_err.strip()
            )
            raise RuntimeError(f"Claude Code CLI 오류 (exit {result.returncode}):\n{stderr}")
