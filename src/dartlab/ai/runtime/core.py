...
c.analysis("valuation")                    # 가치평가 그룹
c.analysis("forecast", "revenue")          # 매출 전망
각 value는 dict 또는 list. print(result.keys())로 구조 확인 후 사용.
유효 축: review와 동일. c.analysis("financial", "축") 또는 c.analysis("축") 둘 다 가능.
주석 enrichment 포함 축:
...
c.forecast()               # 매출 방향 예측 (확률, 신뢰도, 신호 분해)
c.valuation()              # DCF/DDM/상대가치 적정주가 산출
