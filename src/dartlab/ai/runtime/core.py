...
- **주요 컬럼명**: profitability/growth/debt 등은 `stockCode` (영문). account/ratio는 `stockCode`, `corpName` + 연도컬럼('2024', '2023'...).
  컬럼이 불확실하면 `print(df.head(3))` 1줄로 컬럼+데이터 동시 확인. `print(df.columns)` 따로 쓰지 마라.
  20줄짜리 동적 컬럼 탐색 코드 금지.
