...
### Engine-First AI 아키텍처 (2026-03-28~)

기존 2-Tier(compact map 200토큰 + tool calling 의존)를 **Engine-First** 체제로 전환한다.
...
- **도구는 보충**: 사전계산에 없는 정보만 도구로 조회 (공시 원문, 실시간 주가, 웹 검색)
- **자기 검증**: LLM 답변의 수치가 브리핑과 일치하는지 자동 확인

**핵심 원칙**: 엔진이 전부 계산한다 → AI는 읽고 해석한다.

#### 기존 2-Tier의 문제점 (폐기 배경)

- compact map(200토큰)이 너무 얕아 LLM이 도구 호출에 의존
- 엔진이 이미 계산한 것을 LLM이 도구로 재요청 (낭비)
- tool calling 불가 provider에서 분석 품질 급락
- Tier 1이 실질적으로 무력화되어 엔진의 존재 이유 상실

#### 세계적 벤치마크 근거

Bloomberg, OpenBB, FinChat 모두 "AI는 엔진을 대체하지 않고 강화한다"는 결론.
6개 도구(Dexter, OpenBB, Julius, Perplexity, Bloomberg, FinChat) 중 dartlab 수준의 자체 계산 엔진(418함수, 9학술모델)을 가진 도구는 없다.
...
- **meta-tool 패턴**: 도구 안에 도구를 넣는 구조. dartlab은 Super Tool dispatcher로 이미 해결.
