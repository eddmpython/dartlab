"""story 템플릿 -- 섹션별 블록 조합 + helper/aiGuide.

블록 메타(key, label, 순서)와 섹션 메타(title, partId)는 catalog.py가 source of truth.
이 파일은 각 섹션에서 **실제로 보여줄 블록 서브셋**과 helper/aiGuide만 관리한다.

c.story("수익구조") -> 수익구조 템플릿의 visibleKeys 블록만 조립.
c.story()           -> 전체 템플릿 순서대로 조립.
"""

from __future__ import annotations

from dartlab.story.catalog import SECTIONS, keysForSection
...
_SECTION_CONFIG: dict[str, dict] = {
    ...
    "안정성": {
        ...
        "helper": (
            ...
            "③ 비금융 Altman Z''-Score로 부실 가능성을 정량 판별한다 (1.1 미만 위험, 2.6 초과 안전)\n"
            "④ 시장 리스크(베타, 변동성)로 시장 관점 안정성을 확인한다"
        ),
        "aiGuide": (
            "부채비율이 200% 이상이면 재무 위험을 짚어라. "
            "이자보상배율이 3배 미만이면 이자 지급 부담을 경고하라. "
            "비금융 Altman Z''-Score가 1.1 미만이면 부실 위험 구간이다. "
    ...
    "이익품질": {
        ...
        "helper": (
            ...
            "③ Beneish는 canonical 입력 계약이 복구되기 전 비발행 상태인지 확인한다"
        ),
        "aiGuide": (
            "발생액비율이 0.10 이상이면 이익 현금화 부족을 짚어라. "
            "영업외손익 비중이 30% 이상이면 일회성 이익 의존을 경고하라. "
            "Beneish unavailable을 clean이나 0점으로 해석하지 마라. "
...
# ── (deprecated) 관점별 템플릿 ─────────────────────────────
# story.reportTypes.REPORT_TYPES 로 통합됨. 외부 참조 하위호환용 dict만 유지.
# 2026-Q3 제거 예정.
...
# 하위 호환 re-export.
from dartlab.analysis.financial.companyType import (  # noqa: F401
    _TEMPLATE_CHECKS,
    _checkHolding,
    detectTemplate,
    detectTemplates,
