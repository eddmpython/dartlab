...
from dartlab.data.executionSupport import (
    _activeQuery,
    _callableCall,
    _canonical,
    _compiledRequests,
...
from dartlab.data.materialization import MaterializationDirective, MaterializationError
from dartlab.data.projections import projectOutput
from dartlab.data.universe import ResolvedUniverse, resolveUniverse
...
def executeDataQuery(assetIds: Sequence[str], query: DataQuery) -> DataResult:
    ...
    if materialization.mode in {"reuse", "offline"}:
        from dartlab.data.materialization.query import replayMaterializedQuery
    ...
    if materialization.mode != "runtime":
        ...
        try:
            from dartlab.data.compositePaging import prepareCompositePaging
            from dartlab.data.materialization.query import (
    ...
    from dartlab.data.ownerPaging import executeInitialOwnerPaging, isPageableOwner
    ...
    if hasPageable and not (allResourcePaging or allOwnerPaging):
        ...
        contractHash = hashlib.sha256(_canonical({"assets": resolvedRefs, "query": query})).hexdigest()
        from dartlab.data.compositePaging import executeInitialCompositePaging
