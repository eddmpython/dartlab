...
See Also:
    ``dartlab.data.compositePaging``과 ``dartlab.data.ownerProcess``.
...
_EAGER_METADATA = {b"dartlab.data.eager-seal": b"v1"}
...
def _resultForSelector(
    ...
    *,
    ...
    resultSnapshotId = importlib.import_module("dartlab.data.contentSeal").resultSnapshotId
    result = DataResult(
    ...
    try:
        return importlib.import_module("dartlab.data.compositePaging")._encodeEagerResult(
            result,
            maxBytes=maxBytes,
        )
...
def _worker(
    ...
    try:
        ...
        from dartlab.data.ownerProcess import _writeArtifact
...
def _childMain(
    ...
    try:
        ...
        root = _ensureArtifactRoot()
        path = _artifactPath(root, artifactId)
        from dartlab.data.eagerSandbox import enforceEagerSandbox
