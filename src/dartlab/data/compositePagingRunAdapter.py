...
from dartlab.data.compositePagingModels import _EAGER_SCHEMA, _LaneAllocation, _LanePage
from dartlab.data.compositePagingPayload import _decodeEagerResult, _encodeEagerResult
...
class CompositeRunAdapterMixin:
    ...
    def _validateResource(cls, lane: Mapping[str, Any], *, deadline: float) -> Any:
        module = importlib.import_module("dartlab.data.resourcePaging")
    ...
    def _validateOwner(cls, lane: Mapping[str, Any], *, deadline: float) -> Any:
        module = importlib.import_module("dartlab.data.ownerPaging")
        queryPayload, sessionPayload, expectedPins = cls._lowerState(lane)
        module._validateQueryPayload(queryPayload)
        session = module._decodeSession(sessionPayload)
    ...
    def _validateEager(cls, lane: Mapping[str, Any], *, deadline: float) -> Any:
        ...
        if private["eagerMode"] == "sealed":
            eagerProcess = importlib.import_module("dartlab.data.eagerProcess")
        ...
        if not _isSafeEagerLane(descriptor, query):
            raise ContinuationError("CONTINUATION_CONTRACT_STALE")
        catalog = importlib.import_module("dartlab.data.execution").buildCatalog()
        requireDeadline(deadline)
        ...
        if query.universe is not None:
            universe = importlib.import_module("dartlab.data.universe").resolveUniverse(query.universe)
            requireDeadline(deadline)
    ...
    def _materializeEager(
        ...
        *,
        ...
        if private["eagerMode"] == "sealed":
            ...
            payload = importlib.import_module("dartlab.data.eagerProcess").eagerResultAt(
                private["sealed"],
                selectors=private["selectors"],
                index=cursor,
            )
        ...
        try:
            ...
            if activeQuery.universe is not None:
                resolved = importlib.import_module("dartlab.data.universe").resolveUniverse(activeQuery.universe)
            ...
            partition, projectionGaps = importlib.import_module("dartlab.data.projections").projectOutput(
                raw,
                descriptor,
                activeQuery,
                selector=selector,
    ...
    def result(
        ...
        *,
        ...
        del queryPayload
        moduleName = "dartlab.data.resourcePaging" if kind == "resource" else "dartlab.data.ownerPaging"
