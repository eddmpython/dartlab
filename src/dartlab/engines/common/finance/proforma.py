...
class HistoricalRatios:
    ...
    def __repr__(self) -> str:
        ...
        lines.append(f"  판관비율: {self.sga_ratio:.1f}%")
        lines.append(f"  유효세율: {self.effective_tax_rate:.1f}%")
        lines.append(f"  감가상각/매출: {self.depreciation_ratio:.1f}%")
...
def extract_historical_ratios(
    ...
    # 비율 계산 (중위값)
    gross_margin = _median(_safe_ratio_list(gp_vals, rev_vals)) if gp_vals else 30.0
    sga_ratio = _median(_safe_ratio_list(sga_vals, rev_vals)) if sga_vals else 15.0
    capex_ratio = _median([abs(c) / r * 100 for c, r in zip(capex_vals, rev_vals) if r > 0]) if capex_vals else 5.0
    if dep_vals:
        dep_ratio = _median(_safe_ratio_list(dep_vals, rev_vals))
    elif capex_ratio > 0:
    ...
    effective_tax = _median(tax_ratios) if tax_ratios else 22.0
    ...
    for fc_v, d_v in zip(fc_vals, debt_totals):
        if d_v and d_v > 0 and fc_v:
            ...
            if 0 < r < 30:  # 비정상 제거
                int_ratios.append(r)
    interest_rate = _median(int_ratios) if int_ratios else 4.0
    ...
    nwc_ratio = _median(nwc_ratios) if nwc_ratios else 10.0

    # 매출채권/매출, 재고/매출, 매입채무/매출
    ar_ratio = _median(_safe_ratio_list(ar_vals, rev_vals[: len(ar_vals)])) if ar_vals else 15.0
    inv_ratio = _median(_safe_ratio_list(inv_vals, rev_vals[: len(inv_vals)])) if inv_vals else 10.0
    ap_ratio = _median(_safe_ratio_list(ap_vals, rev_vals[: len(ap_vals)])) if ap_vals else 8.0
    ...
    payout = _median(payout_ratios) if payout_ratios else 0.0
...
def compute_company_wacc(
    ...
    # Ke (자기자본비용)
    # beta proxy: SectorElasticity의 revenue_to_gdp를 사용하거나 섹터 기본값
...
def build_proforma(
    ...
    for i, growth_pct in enumerate(revenue_growth_path):
        ...
        yr.cogs = yr.revenue * (1 - ratios.gross_margin / 100)
        yr.gross_profit = yr.revenue - yr.cogs
        yr.sga = yr.revenue * ratios.sga_ratio / 100
        yr.depreciation = yr.revenue * ratios.depreciation_ratio / 100
        ...
        # CAPEX & PPE
        yr.capex = -(yr.revenue * ratios.capex_to_revenue / 100)  # 음수
        ...
        if yr.cash < 0:
            warnings.append(f"+{yr.year_offset}년: 현금 음수 ({yr.cash / 1e8:.0f}억) — 추가 차입 필요")
            yr.cash = 0
