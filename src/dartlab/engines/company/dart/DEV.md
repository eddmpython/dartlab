...
상세 문서:
- `src/dartlab/engines/company/dart/docs/DEV.md`
- `src/dartlab/engines/company/dart/finance/DEV.md`
- `src/dartlab/engines/company/dart/report/DEV.md`
...
- comparable slot spine(`textComparablePathKey`) 기준 구조 이벤트도 `sections` 안에서 계산한다.
  ...
  - `cadenceScope='annual'/'quarterly'` 조회는 `mixed` row를 포함하더라도 해당 lane period만 activity에 반영한다.
...
위치: `engines/company/dart/report/types.py` — `_seriesToWide()` 공통 헬퍼.
...
- 위치: `engines/company/dart/docs/sections/diff.py`
