...
from dartlab.engines.company.dart._diff_helpers import (  # noqa: F401
    _buildTopicChangeLedger,
    _normalizeTextCell,
)

# ── 분리된 accessor 클래스 re-export (외부 import 경로 유지) ──
from dartlab.engines.company.dart._docs_accessor import _DocsAccessor  # noqa: F401
from dartlab.engines.company.dart._finance_accessor import _FinanceAccessor  # noqa: F401
from dartlab.engines.company.dart._finance_helpers import (  # noqa: F401
...
from dartlab.engines.company.dart._profile_accessor import _ProfileAccessor  # noqa: F401
from dartlab.engines.company.dart._report_accessor import REPORT_COL_KR as _REPORT_COL_KR  # noqa: F401
...
def _get_module_registry() -> list[tuple[str, str, str, Any]]:
    ...
    if _MODULE_REGISTRY is None:
        _MODULE_REGISTRY = [
            ("dartlab.engines.company.dart.docs.finance.summary", "fsSummary", "요약재무정보", None),
            ("dartlab.engines.company.dart.docs.finance.statements", "statements", "재무제표", None),
...
class Company:
    ...
    def _call_notesDetail(self, keyword: str) -> Any:
        ...
        result = _import_and_call(
            "dartlab.engines.company.dart.docs.finance.notesDetail",
            "notesDetail",
            self.stockCode,
            keyword=keyword,
        )
    ...
    def _contextSlices(self) -> pl.DataFrame | None:
        ...
        if cacheKey in self._cache:
            return self._cache[cacheKey]
        from dartlab.engines.company.dart.docs.sections import contextSlices
    ...
    def _profileTable(self) -> pl.DataFrame | None:
        ...
        if cacheKey in self._cache:
            return self._cache[cacheKey]
        from dartlab.engines.company.dart.docs.sections.artifacts import loadSectionProfileTable
    ...
    def _chapterForTopic(self, topic: str) -> str:
        ...
        if self.notes is not None:
            from dartlab.engines.company.dart.docs.notes import _REGISTRY as _NOTES_REGISTRY
    ...
    def _stripUnitHeader(sub: list[str]) -> list[str] | None:
        ...
        from dartlab.engines.company.dart._table_horizontalizer import stripUnitHeader
    ...
    def table(
        ...
        *,
        ...
        if result is None:
            return None
        from dartlab.engines.company.dart.docs.sections import parseSubtopicTable
    ...
    def sectorParams(self):
        ...
        if cacheKey in self._cache:
            return self._cache[cacheKey]
        from dartlab.engines.analysis.sector import getParams
    ...
    def rank(self):
        ...
        Example::
            ...
            from dartlab.engines.analysis.insight import buildSnapshot
            buildSnapshot()
        ...
        if cacheKey in self._cache:
            return self._cache[cacheKey]
        from dartlab.engines.analysis.rank.rank import getRank
    ...
    def _scanMarketSummary(self, df: pl.DataFrame) -> pl.DataFrame:
        """시장별 요약 통계."""
        from dartlab.engines.company.dart.scan._helpers import load_listing
