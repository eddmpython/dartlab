...
사용법::
    ...
    from dartlab.engines.company.dart.scan.snapshot import buildScanSnapshot, getScanPosition
...
def buildScanSnapshot(*, verbose: bool = True) -> dict[str, dict]:
    ...
    from dartlab.engines.company.dart.scan.governance.scanner import (
        scan_audit_opinion,
        scan_major_holder_pct,
        scan_outside_directors,
        scan_pay_ratio,
    )
    from dartlab.engines.company.dart.scan.governance.scorer import (
        grade,
        score_audit,
        score_outside_ratio,
        score_ownership,
    ...
    from dartlab.engines.company.dart.scan.workforce.scanner import scan_revenue_per_employee
    ...
    from dartlab.engines.company.dart.scan.capital.scanner import (
        scan_capital_change,
        scan_dividend,
        scan_treasury_stock,
    )
    ...
    if verbose:
        print("  [4/4] debt 스캔...")
    from dartlab.engines.company.dart.scan.debt.risk import classify_risk, scan_icr
    from dartlab.engines.company.dart.scan.debt.scanner import scan_bonds
