"""EDGAR 전용 복수 기업 비교 분석 클래스.

문자열은 US ticker로 해석한다.
"""
...
_log = logging.getLogger("dartlab.engines.edgar.compare")
...
def _resolveCompany(arg: Any) -> AnyCompany:
    """문자열이면 EDGAR Company로 변환, 아니면 그대로 반환."""
    if isinstance(arg, Company):
        return arg
    if isinstance(arg, str):
        return Company(arg)
    raise TypeError(f"str 또는 EDGAR Company여야 합니다: {type(arg)}")
...
class Compare:
    ...
    Example::
        ...
        c = Compare("AAPL", "MSFT")           # 문자열 직접
        c = Compare(Company("AAPL"), Company("MSFT"))
