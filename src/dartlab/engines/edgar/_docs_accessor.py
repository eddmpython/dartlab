...
class _DocsAccessor:
    ...
    def retrievalBlocks(self) -> pl.DataFrame | None:
        ...
        if key not in self._company._cache:
            from dartlab.engines.edgar.docs.sections.views import retrievalBlocks
    ...
    def contextSlices(self) -> pl.DataFrame | None:
        ...
        if key not in self._company._cache:
            from dartlab.engines.edgar.docs.sections.views import contextSlices

            self._company._cache[key] = contextSlices(self._company.ticker)
        return self._company._cache[key]

    def notes(self, query: str | None = None) -> pl.DataFrame | None:
        from dartlab.engines.edgar.docs.notes import notes
    ...
    def cadence(self) -> pl.DataFrame | None:
        key = "_docs_cadence"
        if key not in self._company._cache:
            from dartlab.engines.edgar.docs.sections.views import cadence
    ...
    def coverage(self) -> pl.DataFrame | None:
        key = "_docs_coverage"
        if key not in self._company._cache:
            from dartlab.engines.edgar.docs.sections.views import coverage
