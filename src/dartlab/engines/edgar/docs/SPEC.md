...
현재 package 계약은 `10-K`, `10-Q`, `20-F`를 `section_title`, `section_content`, `period_key` 중심으로 읽고, `form_type::topicId` 기준 horizontal view를 만드는 것이다.
...
- `10-K`, `10-Q`, `20-F`는 모두 structured split 대상이다.
...
- 대신 `downloadListedEdgarDocs(limit=2000)`처럼 상장 universe를 기준으로 배치 수집한다.
