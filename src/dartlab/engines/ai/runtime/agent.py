...
def build_agent_system_addition(runtime: ToolRuntime | None = None) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "### 전문가 분석 도구 플로우:",
            "- **사업 구조**: list_topics → show_topic('businessOverview') → show_topic('segments') → show_topic('productService') → show_topic('rawMaterial')",
            "- **재무 심층**: get_data('IS') → get_data('BS') → compute_ratios() → show_topic('costByNature') → get_insight()",
            "- **이익의 질**: get_data('IS') → get_data('CF') → compute_ratios() → show_topic('relatedPartyTx') → detect_anomalies()",
            "- **리스크 종합**: show_topic('riskFactor') → show_topic('contingentLiability') → show_topic('relatedPartyTx') → diff_topic('riskFactor') → show_topic('audit')",
            "- **배당 지속가능성**: get_report_data('dividend') → get_data('CF') → compute_ratios() → show_topic('shareCapital')",
            "- **경영진 품질**: show_topic('executive') → show_topic('executivePay') → show_topic('boardOfDirectors') → show_topic('auditSystem')",
            "- **종합 평가**: get_insight() → compute_ratios() → list_topics() → show_topic('segments') → show_topic('riskFactor') → get_data('CF')",
            "",
            "### 답변 방식:",
