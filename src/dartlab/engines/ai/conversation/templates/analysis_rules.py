...
- 보정 방향(상향/하향)과 크기(소폭/중폭/대폭)를 명시하세요.

보정 대상:
1. **성장률**: 엔진이 과거만 보고 놓친 구조적 변화 (신사업, 규제, 기술변화)
2. **신뢰구간**: 불확실성이 높으면 넓히고, 낮으면 좁히기
3. **리스크/기회**: 숫자에 없는 정성적 요인 (지정학, 환경, 수요변화)
4. **시나리오**: "만약 X이면 Y" 조건부 예측
...
FORECAST_OVERLAY_COMPACT = (
    ...
    "- 보정 시 구체적 근거 필수 (산업 트렌드, 규제, 경쟁)\n"
    "- 방향(상향/하향)과 크기(소폭/중폭/대폭) 명시\n"
    "- 숫자에 없는 정성적 리스크/기회 제시\n"
