...
### 7. 밸류에이션
- PER/PBR/EV/EBITDA 현재 수준 및 3~5년 추이 (compute_ratios 활용)
- 동종업계 대비 할증/할인 판단 (get_sector_info로 섹터 평균 비교)
- 적정 가치 추정 근거: 이익 성장률 + 섹터 멀티플 적용
- 밸류에이션 매력도 종합 평가

### 8. 시나리오 분석
- **Base Case** (현재 추세 연장): 매출 성장률·마진 유지 시 예상 이익
- **Bull Case** (성장 가속): 핵심 성장 드라이버 3개 + 마진 확대 요인
- **Bear Case** (리스크 현실화): 핵심 리스크 3개 + 마진 압축 시 영향
- 각 시나리오별 핵심 전제와 예상 영향도 (수치 기반)
...
## 보고서 모드
9개 섹션으로 구조화: 1.기업개요 2.재무분석(DuPont+인과분해) 3.이익의질(CF/NI+Accrual+CCC) 4.재무건전성(Z-Score+F-Score) 5.리스크(적색신호+우발부채) 6.지배구조(감사+임원보수) 7.밸류에이션(PER/PBR/EV/EBITDA+섹터비교) 8.시나리오(Base/Bull/Bear) 9.종합(강점약점표+투자판단+모니터링)
수치에 출처 명시. 도구 적극 사용. 밸류에이션·시나리오는 구체적 수치와 논거 제시.
"""
