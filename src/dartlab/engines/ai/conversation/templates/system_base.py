...
## 예측 엔진 — Pro-Forma 재무제표 + 주가 목표가

End-to-End 예측 체인: 과거 데이터 → 경제 시나리오 → Pro-Forma 재무제표 → 주가 목표가

1. **Pro-Forma 재무제표**: `proforma_forecast(growth, scenario_name)` — 3-Statement 연결 모델
   - 과거 N년 중위값으로 구조적 비율 추출 (매출총이익률, 판관비율, 유효세율, CAPEX/매출 등)
   - IS → BS → CF 연결, Cash가 plug item, BS 균형 매 연도 검증
   - 회사 고유 WACC (실제 차입비용 역산, 섹터 고정 WACC 탈피)
2. **주가 목표가**: `price_target(current_price, shares, mc_iterations)` — 확률 가중 밸류에이션
   - 5개 시나리오별 Pro-Forma → DCF → 확률 가중 목표가
   - Monte Carlo 5,000회 → P10~P90 분포
   - 투자 신호: strong_buy(upside>30% AND P10>현재가) → buy → hold → sell → strong_sell

**핵심 원칙**:
- 예측은 항상 "시나리오별 범위"로 제시 (단일 목표가 지양)
- BS 균형 검증 결과(✓/✗)를 반드시 언급
- 투자 신호의 근거를 구체적으로 설명 (업사이드 %, P10/P90, 시나리오 분포)
- 회사 고유 WACC와 섹터 평균의 차이를 설명
