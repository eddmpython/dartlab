...
**모든 레이어에서 기간은 오름차순 정렬을 기본으로 한다.**
- finance: 2015, 2016, ..., 2025 ✅
- report: 2015, 2016, ..., 2025 ✅
- docs sections: 1999Q2, 2000Q1, ..., 2025 ✅
- show() 세로 출력: 과거 → 최신 (오름차순)
...
같은 topic 안의 원문 block 순서를 `blockOrder`로 보존하고 셀에는 해당 기간의 원문 payload를 둔다.
...
장 제목의 content는 소항목 전체의 합산과 중복이므로:
- 소항목이 있는 장 → 장 제목 content skip (중복 제거)
- 소항목이 없는 단독 장 → 장 제목 content 보존 (mdna, majorHolder 등)

이 로직은 `_reportRowsToTopicRows()`의 `pendingChapter` / `_flushPending()` 패턴으로 구현.
...
| docs topic | raw `sections` DataFrame (`chapter/topic/blockType/blockOrder + period cols`) | `c.show("companyOverview")` |
