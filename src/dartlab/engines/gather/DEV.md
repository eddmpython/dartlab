...
│   ├── yahoo_direct.py # yahoo v8 chart API: 히스토리 + 컨센서스 (yfinance 미사용)
│   └── fmp.py        # FMP: 히스토리 fallback + 컨센서스 + 업종PER
├── price.py          # 주가 fallback facade (naver → yahoo_direct → yahoo)
├── history.py        # 히스토리 OHLCV fallback facade (naver → yahoo_direct → fmp → yahoo)
...
| 히스토리(OHLCV) | naver(차트API, 6000일) → yahoo_direct → fmp → yahoo | yahoo_direct → fmp → yahoo |
