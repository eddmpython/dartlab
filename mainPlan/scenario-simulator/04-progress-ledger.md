...
## 2026-07-15 P0 scenario path package binding

- **판단**: 사용자가 말한 시뮬레이터의 핵심은 관측 이력과 미래 가정을 함께 돌리는 것이다. 다만 observed history와 explicit future adjustment가 한 path처럼 섞이면 미래 가정이 검증된 경로처럼 세탁된다. 다음 P0는 path admission을 여는 것이 아니라, 관측 기반 base path와 explicit overlay를 분리해 하나의 조건부 scenario path package로 묶는 것이다.
- **구현**: `DriverPathAudit`에 `historyInputHash`, `assumptionHash`, `basePathSetHash`, `overlayHash`, `observedHistoryStatus`를 추가했다. `buildDriverPathSet`은 provider history로 만든 base empirical path hash와 explicit assumption overlay hash를 따로 남긴 뒤 최종 composed path set hash를 만든다.
- **loop 장부**: `OneCompanyScenarioCaseLedger`가 `scenarioPathPackageHash`, `pathHistoryInputHash`, `pathAssumptionHash`, `basePathSetHash`, `composedPathSetHash`, `pathOverlayHash`, observed/future status, admission transfer status, policy eligibility, recommendation ceiling을 보존한다.
- **세탁 차단**: explicit future adjustment가 있으면 `composedPathAdmissionStatus="notAdmitted"`, `pathAdmissionTransferStatus="notTransferred"`, `policyEvaluationEligibility="blocked"`로 남긴다. blocked reason에는 `explicitFutureAdjustmentPresent`, `composedPathAdmissionNotGranted`, `pathAdmissionNotTransferredFromObservedHistory`, `policyEvaluationRequiresAdmittedComposedPath`, `scoreLeaderNotRecommendation`이 추가된다.
- **hash 경계**: explicit adjustment 값을 바꾸면 `pathAssumptionHash`, `pathOverlayHash`, `composedPathSetHash`, `scenarioPathPackageHash`, `loopHash`가 바뀐다. 같은 provider history를 쓰면 `pathHistoryInputHash`와 `basePathSetHash`는 유지된다.
- **검증**: 새 package binding assertion을 `testProviderHistoryAndExplicitAdjustmentFeedScenarioLoopWithoutPathAdmission`에 추가했다. focused 1건, `test_driverObservationFrames.py` 17건, `test_driverPaths.py` 4건, `test_scenarioComposition.py` 12건, provider history projection focused 1건, `tests/simulate` 전체 409건 통과. ruff format/check, camelCase strict, docstring4 audit, Guard Index strict l0-l15, diff check, em/en dash audit 통과. `tests/run.py preflight`는 로컬 `.venv`의 pip 누락으로 설치 단계에서 실패했으며, 코드 관련 핵심 게이트는 직접 실행으로 통과했다.
- **남은 P0**: 이제 조건부 시나리오 패키지 장부는 닫혔다. 다음은 history-only admitted path를 별도로 열고, explicit overlay가 붙은 composed path에는 admission이 전이되지 않는 음성 회귀를 강화하는 것이다. 그 뒤 policy evaluation certificate를 final composed path hash에 정확히 묶어야 전략 추천으로 승격된다.


## 2026-07-15 P0 conditional scenario experiment sweep

- **판단**: 사용자의 전쟁게임식 simulator 해석이 PRD 본체다. admission, receipt, ledger는 필요한 기반이지만, 여기서 더 머물면 조건과 가정을 넣고 전략을 비교한다는 제품 목표에서 멀어진다. 이번 단위는 추천을 여는 것이 아니라, 여러 assumption set과 여러 strategy를 같은 operating world 위에서 실제로 돌려 조건부 전략 비교를 장부화하는 것이다.
- **구현**: `runConditionalScenarioExperiment`를 추가했다. 입력은 한 entity, 둘 이상의 `OperatingScenarioCase`, 둘 이상의 `StrategySpec`이며, 내부 실행은 기존 `compareOperatingScenarioCases`를 재사용한다. 새 public verb는 만들지 않고 `simulate` 내부 experiment envelope로 둔다.
- **실험 장부**: `ConditionalScenarioExperiment`는 `experimentHash`, `strategySetHash`, `simulationSpecHash`, `resultSetHash`, `assumptionSetHashes`, case ledger, cell ledger, strategy summary, fragility cell을 보존한다. cell은 case x strategy별 objective score, feasibility, breach count, regret, score leader 여부를 남긴다.
- **전략 비교 출력**: `ConditionalStrategySummary`는 strategy별 median score, worst score, best score, median regret, worst regret, leader frequency, feasible cell count, breach count를 계산한다. `ConditionalAssumptionFragility`는 assumption set별 leader, runner-up, leader margin, score spread, breach strategy, assumption refs를 남겨 어떤 조건에서 전략 선택이 취약한지 볼 수 있게 했다.
- **추천 경계**: experiment layer는 `recommendation=None`을 유지하고 `conditionalExperimentNotPolicyRecommendation`, `automaticRecommendationDisabled`, `assumptionSweepPresent`, `strategySweepPresent`를 blocked reason에 남긴다. path admission과 policy evaluation certificate가 없으면 기존 `pathAdmissionMissing`, `policyEvaluationCertificateMissing`도 유지된다.
- **hash 경계**: 같은 assumption grid와 같은 strategy set은 같은 `experimentHash`와 `resultSetHash`를 만든다. assumption 값 하나를 바꾸면 `assumptionSetHashes`, `resultSetHash`, `experimentHash`, cell ledger가 바뀐다. 기존 `comparisonHash`의 executable hash 불안정성은 experiment 정본 hash에 넣지 않고, stable한 run/result/cell/spec hash로 묶었다.
- **검증**: `test_scenarioComposition.py`가 12건에서 15건으로 늘었다. 3개 변수, 4개 assumption set, 3개 strategy를 실제로 돌리는 positive test, hash binding test, sweep shape negative test를 추가했다. focused 43건 통과. `tests/simulate` 전체 412건 통과. ruff format/check, camelCase strict, docstring4 audit, Guard Index strict l0-l15, em/en dash audit 통과.
- **남은 P0**: 다음은 실제 DART, EDGAR, price, macro, industry adapter가 만든 대량 변수 묶음을 이 experiment envelope에 공급하는 단계다. 그 뒤 admitted path와 policy evaluation certificate를 final composed path hash와 strategy set hash에 묶어 추천 승격을 별도 단계로 열어야 한다.


## 2026-07-15 P0 provider observation backed conditional experiment manifest

- **판단**: 사용자의 지적처럼 시뮬레이터는 장부를 보관하는 기능이 아니라, 관측된 현재 상태 위에 여러 조건과 미래 가정을 올리고 전략별 결과를 비교해야 한다. 기존 sweep은 수동 가정 grid를 돌렸지만, 실제 provider observation lane과 admitted current state가 같은 experiment envelope 안에서 보인다는 증거가 약했다.
- **구현**: `ConditionalScenarioExperiment`에 case ledger hash, provider observation batch refs, explicit assumption ids, path history input hashes, path assumption hashes를 추가했다. 이 값들은 top-level truth가 아니라 case ledger에서 파생한 lineage index이며, `experimentHash`와 `simulationSpecHash`에 안정적으로 결속된다.
- **hash 경계**: case ledger hash는 path, state, provider, assumption, coefficient, run/result, strategy score, blocker를 묶되, 실행 환경에 따라 흔들릴 수 있는 executable hash와 trace root는 제외했다. 같은 입력의 experiment hash는 안정적으로 유지되고, explicit future adjustment 값이 바뀌면 `pathAssumptionHashes`, `assumptionSetHashes`, `simulationSpecHash`, `resultSetHash`, `experimentHash`가 바뀐다.
- **수직 회귀**: 새 회귀는 signed exact fx/oil provider observation batch, admitted current operating state, verified multivariable coefficient binding, 4개 explicit future adjustment case, 3개 one step strategy를 `runConditionalScenarioExperiment`로 한 번에 실행한다. 결과는 4 x 3 cell, strategy summary, fragility cell, assumption별 leader margin 변화를 남긴다.
- **세탁 차단**: provider history refs와 explicit assumption ids는 case ledger union과 정확히 일치해야 한다. 같은 provider history를 유지하고 future adjustment만 바꾸면 history hash는 유지되고 assumption hash만 바뀐다. initial state admission이 있으므로 `initialStateAdmissionMissing`은 사라지지만, path admission과 policy certificate가 없으므로 recommendation은 계속 `None`이다.
- **검증**: scenarioComposition과 driverObservationFrames focused 33건 통과. `tests/simulate` 전체 413건 통과. ruff format/check, camelCase strict, docstring4 audit, Guard Index strict l0-l15, diff check, em/en dash audit 통과.
- **남은 P0**: 이제 provider observation 기반 조건부 실험표는 열렸다. 다음은 DART, EDGAR, price, macro, industry adapter의 더 많은 lane을 이 manifest로 공급하고, 1 step을 넘어 전략 리더가 실제로 뒤집힐 수 있는 admitted horizon과 policy evaluation certificate를 별도 단계로 닫는 것이다. 그 전까지 제품 표현은 "조건부 전략 실험"이어야 하며 "공식 전략 추천"은 아니다.


## 2026-07-15 P0 registry backed multi-lane conditional experiment

- **판단**: 사용자 목표는 많은 DART, EDGAR, price, macro, industry 변수가 조건과 가정으로 들어가는 war-game형 시뮬레이터다. provider observation vertical만으로는 lane breadth가 약하므로, DriverRegistry가 고른 DART filing metric과 industry time-series가 conditional experiment까지 이어지는 증거가 필요했다.
- **구현**: `OperatingScenarioCase`가 optional driver registry audit를 들고, `OneCompanyScenarioCaseLedger`와 `ConditionalScenarioExperiment`가 registry hash, lane ids, card ids, factor ids, source refs, semantic refs, common support, source observation counts, event window, warnings를 보존하게 했다.
- **registry hash 경계 보강**: `driverRegistry`의 source payload가 history panel content hash와 explicit assumption steps hash를 포함하도록 고쳤다. 같은 source card id와 lane id를 쓰더라도 explicit future assumption 값이 바뀌면 `registryHash`, `pathAssumptionHash`, `pathSetHash`가 같이 바뀐다.
- **수직 회귀**: 새 회귀는 DART filing metric lane, industry order time-series lane, manual demand adjustment lane을 registry로 묶고, admitted current operating state 위에서 4개 assumption case x 3개 strategy conditional experiment를 실행한다. DART는 fiscal period와 filing receipt date를 분리하고, industry는 snapshot이 아니라 eventTime과 availableAt이 있는 time-series만 pathHistory로 들어간다.
- **경계 보존**: DART와 industry refs는 `pathSourceRefs`와 registry ledger에 남고, explicit adjustment는 `assumptionRefs`와 `explicitAssumptionIds`에 남으며, current state는 `stateRefs`, strategy action은 `StrategySpec`에만 남는다. path step 안에는 `priceChange`, `capacityInvestment`, `borrow`, `repay` 같은 intervention action이 들어가지 않는 것을 회귀로 고정했다.
- **추천 경계**: registry lane과 admitted current state가 들어와도 explicit future adjustment가 포함된 composed path는 `notAdmitted`이고 recommendation은 계속 `None`이다. blocked reason은 `automaticRecommendationDisabled`, `conditionalExperimentNotPolicyRecommendation`, `pathAdmissionMissing`, `policyEvaluationCertificateMissing`, `scoreLeaderNotRecommendation`을 유지한다.
- **검증**: driverObservationFrames, scenarioComposition, driverRegistry focused 43건 통과. `tests/simulate` 전체 415건 통과. ruff format/check, camelCase strict, docstring4 audit, Guard Index strict l0-l15, diff check, em/en dash audit 통과.
- **남은 P0**: 다음은 EDGAR filing metric, exact price return, macro revised or exact lane을 같은 registry backed experiment에 추가하고, multi-step horizon에서 전략 리더가 실제로 뒤집힐 수 있는 policy evaluation 경계까지 확장하는 것이다. 공식 추천 승격은 admitted composed path와 policy certificate 전까지 계속 금지다.


## 2026-07-15 P0 EDGAR exact price macro registry experiment

- **판단**: 사용자가 말한 전쟁게임식 시뮬레이터가 되려면 데이터 원장만 늘리는 것이 아니라, 실제 관측 변수와 미래 가정을 같은 전략 실험에서 움직여야 한다. 다만 operating world의 현재 step contract는 quarter이므로 EDGAR, 가격, macro를 weekly로 억지 결합하지 않고 quarter grid로 정렬해야 한다.
- **구현**: `buildPriceReturnDriverObservationBatch`가 `quarter` frequency를 지원하게 했다. 가격 row는 quarter bucket별 마지막 close leg를 쓰고, exact derived return receipt는 기존처럼 이전 close leg와 현재 close leg parent receipt를 모두 묶는다. `macroDriverHistorySource`는 `frequency="quarter"`를 받아 `quarterlyMacroInnovations`로 macro level을 quarterly innovation으로 변환한다.
- **수직 회귀**: 새 회귀는 exact EDGAR filing metric provider batch, exact quarterly price return provider batch, quarterly macro oil innovation source, manual demand adjustment를 같은 `DriverRegistry`에 넣고 admitted current operating state 위에서 4개 assumption case x 3개 strategy를 실행한다. 결과는 12개 cell, strategy summary, fragility cell, case ledger, experiment lineage index에 남는다.
- **lineage 경계**: EDGAR는 accession, fiscal period, acceptedAt, row source receipt, normalization contract, provider batch ref를 남긴다. 가격은 raw close leg receipt와 derived return receipt를 분리하고, path source refs에 `priceSourceLegReceiptId`, `derivedReturnReceiptId`, return formula, adjustment policy hash를 남긴다. macro는 release vintage 부재 때문에 `macroReleaseVintageUnavailable`과 revised history 경고를 유지한다.
...
## 2026-07-15 P0 documented conditional path package receipt

- **판단**: final composed path admission을 바로 여는 것은 아직 틀렸다. `AdmissionRegistry`에서 `admitted`는 `asKnown`과 `asOfExact`에 묶여 있고, explicit future overlay가 붙은 path는 관측된 역사 경로가 아니다. 따라서 다음 단계는 admitted 승격이 아니라 content-bound documented package receipt다.
- **구현**: `composedConditionalPathPackage` receipt kind와 rule hash를 추가했다. `scenarioPathPackagePayload`, `scenarioPathPackageArtifact`, `scenarioPathPackageSubjectHash`, `scenarioPathPackageParentReceiptIds`, `validateScenarioPathPackageReceipt`가 base admission, overlay hash, assumption step hashes, composed path rows, registry hash, factor contract hash, composition contract hash를 하나의 canonical package로 묶는다.
- **receipt 계약**: conditional package receipt는 `status="documented"`, `revisionPolicy="explicitAssumption"`, `coverage="synthetic"`, `maxAdmittedStep=0`만 통과한다. base path admission receipt가 있으면 parent receipt에 반드시 포함되어야 하며, parent는 `kind="pathSet"`, `status="admitted"`, `asKnown/asOfExact`여야 한다. policy evaluation 계열 receipt가 parent로 들어오면 실패한다.
- **loop 장부**: `OperatingScenarioCase`가 optional `scenarioPathPackageReceiptId`를 받고, result와 `OneCompanyScenarioCaseLedger`가 package subject hash, receipt id, kind, status, parent ids를 남긴다. refs에는 `composedPathPackage`, `composedPathSubject`, `composedPathSet`, `basePathAdmission`, `explicitOverlay`, `explicitAssumptionStep`, `compositionContract`가 분리되어 들어간다.
- **추천 경계**: documented package receipt가 있어도 `pathAdmissionReceiptId`는 계속 빈 값이고 `composedPathAdmissionStatus="notAdmitted"`, `pathAdmissionTransferStatus="notTransferred"`가 유지된다. blocked reason에는 `conditionalReceiptNotPathAdmission`, `policyAdmittedRecommendationBlocked`, `policyEvaluationRequiresAdmittedComposedPath`가 남는다.
- **세탁 차단 회귀**: conditional package receipt를 path admission 위치에 넣으면 기존 explicit overlay admission guard가 실패시킨다. package receipt를 `kind="pathSet"`으로 발급하면 expected kind mismatch로 실패하고, 같은 kind라도 `status="admitted"`로 발급하면 package contract mismatch로 실패한다. overlay 값 하나가 바뀌면 subject hash와 receipt content binding도 바뀐다.
- **검증**: scenarioComposition focused 19건 통과. `tests/simulate` 전체 424건 통과. ruff format/check, camelCase strict, docstring4 source audit, Guard Index strict l0-l15, diff check, 긴 줄표 검색 통과.
- **남은 P0**: 이제 what-if package 재현성은 receipt로 닫혔지만, 공식 추천은 여전히 닫혀 있다. 다음은 conditional experiment evaluation receipt를 별도로 설계하거나, explicit overlay 없이 admitted final path가 있을 때만 policy evaluation certificate가 recommendation을 열도록 경계를 계속 분리해야 한다.


## 2026-07-15 P0 documented conditional experiment result receipt

- **판단**: path package receipt는 각 조건 경로를 봉인하지만, GUI가 나중에 전략 리더, fragility, blockers, provenance를 신뢰하고 그리려면 실험 결과표 자체도 content-bound receipt가 필요하다. 이 receipt는 결과 재현성 증명이며 정책 추천 면허가 아니다.
- **구현**: `conditionalScenarioExperimentResult` receipt kind와 rule hash를 추가했다. `conditionalScenarioExperimentPayload`, `conditionalScenarioExperimentArtifact`, `conditionalScenarioExperimentSubjectHash`, `conditionalScenarioExperimentParentReceiptIds`, `validateConditionalScenarioExperimentReceipt`, `bindConditionalScenarioExperimentReceipt`가 experiment spec, result set, strategy set, case ledger hashes, path package receipts, strategy summaries, cells, fragility cells, blockers를 canonical payload로 묶는다.
- **receipt 계약**: result receipt는 `status="documented"`, `revisionPolicy="explicitAssumption"`, `coverage="synthetic"`, `frequency="scenario"`, `maxAdmittedStep=0`만 통과한다. explicit assumption case는 composed path package receipt를 반드시 parent로 가져야 하며, policy evaluation이나 policy admitted parent를 상속하면 실패한다.
- **hash 안정성**: 기존 `comparisonHash`는 실행 세부값 때문에 반복 실행 간 흔들릴 수 있어 result receipt subject에서 제외했다. 대신 decision status, recommendation ceiling, strategy set, case ledger hashes, result set, blockers, warnings만 묶는 stable `comparisonReplayHash`를 payload에 넣었다.
- **추천 경계**: result receipt가 있어도 `recommendation is None`, `pathAdmissionReceiptId == ""`, `policyEvaluationCertificateId == ""`가 유지되어야 한다. verifier는 recommendation, policy certificate id, path admission id가 들어온 experiment를 거부한다. blocked reason은 `conditionalExperimentNotPolicyRecommendation`, `automaticRecommendationDisabled`, `scoreLeaderNotRecommendation` 경계를 유지한다.
- **세탁 차단 회귀**: wrong kind `policyEvaluation` receipt는 expected kind mismatch로 실패한다. required composed path package parent가 빠지면 parent mismatch로 실패한다. documented receipt라도 `asKnown/asOfExact` vintage contract를 붙이면 contract mismatch로 실패한다. strategy payload를 바꾸면 `strategySetHash`, `resultSetHash`, receipt subject hash가 바뀐다.
- **검증**: scenarioComposition focused 22건 통과. `tests/simulate` 전체 427건 통과. ruff format/check, camelCase strict, docstring4 source audit, Guard Index strict l0-l15, diff check, 긴 줄표 검색 통과.
- **남은 P0**: 이제 condition path package와 experiment result table의 재현성 receipt는 닫혔다. 다음은 actual provider lane breadth를 더 넓히거나, 별도 conditional policy evaluation receipt를 설계하되 `policyEvaluationCertificateId`와 official recommendation으로 세탁되지 않게 별도 rail로 유지해야 한다.


## 2026-07-15 P0 documented conditional strategy evaluation receipt

- **판단**: provider lane을 더 붙이기 전에, 조건부 실험 결과를 사용자가 전략 판정문으로 읽을 수 있는 rail이 필요했다. `conditionalScenarioExperimentResult`는 표 재현성 receipt이고, 그 위에서 objective, leader frequency, worst score, regret, fragility, blocker를 묶는 별도 문서가 있어야 GUI가 전략 리더와 취약 case를 신뢰하고 표시할 수 있다.
- **구현**: `conditionalStrategyEvaluation` receipt kind와 rule hash를 추가했다. `buildConditionalStrategyEvaluation`, `conditionalStrategyEvaluationPayload`, `conditionalStrategyEvaluationArtifact`, `conditionalStrategyEvaluationSubjectHash`, `conditionalStrategyEvaluationParentReceiptIds`, `validateConditionalStrategyEvaluationReceipt`, `bindConditionalStrategyEvaluationReceipt`가 sealed experiment receipt 위에서 전략 평가표를 만든다.
- **평가 내용**: strategy row는 conditional leader 여부, robustness class, leader frequency, median and worst score, median and worst regret, feasible count, breach count를 담는다. fragile case는 leader margin이 가장 작은 assumption set을 별도로 표면화한다. selection rule hash, robustness rule hash, metric rule hash, comparison rule hash, fragility rule hash, blocker rule hash가 subject에 묶인다.
- **receipt 계약**: strategy evaluation receipt는 `status="documented"`, `revisionPolicy="explicitAssumption"`, `coverage="synthetic"`, `frequency="scenario"`, `stepSpan=1`, `maxAdmittedStep=0`만 통과한다. parent는 sealed `conditionalScenarioExperimentResult` receipt 하나이며, path package와 state lineage는 그 parent가 보존한다.
- **추천 경계**: evaluation은 `recommendation=None`, `recommendationStatus="disabled"`, `decisionStatus="conditionalOnly"`, `recommendationCeiling="conditionalOnly"`만 허용한다. `pathAdmissionReceiptId`, `policyEvaluationCertificateId`를 carry하면 검증이 실패한다. blocked reason에는 `conditionalStrategyEvaluationDocumentedOnly`, `strategyEvaluationReceiptNotPolicyCertificate`, `conditionalExperimentNotPolicyRecommendation`, `automaticRecommendationDisabled`, `scoreLeaderNotRecommendation`이 필요하다.
...
## 2026-07-15 P0 documented conditional play scenario deck receipt

- **판단**: multi-step conditional experiment와 strategy delta ledger가 있어도, multi-plane control deck 자체가 receipt로 봉인되지 않으면 GUI가 나중에 condition edit order, stage execution, rebase, source seal, strategy delta, case leader delta를 신뢰하고 재현할 수 없다. 이번 단위는 추천 rail이 아니라 documented deck 실행 rail이다.
- **구현**: `conditionalPlayScenarioDeck` rule hash와 `conditionalPlayScenarioDeckArtifact`, `conditionalPlayScenarioDeckParentReceiptIds`, `validateConditionalPlayScenarioDeckReceipt`, `bindConditionalPlayScenarioDeckReceipt`를 추가했다. deck report는 base와 final lineage parent receipt snapshot, optional experiment receipt id, deck receipt annotation 필드를 가진다.
- **artifact 경계**: payload는 semantic plane order, stage execution hashes, stage patch set hashes, stage base와 patched experiment hashes, rebase rows, source seal hash, strategy delta rows, case leader delta rows, parent receipt ids, rule hash를 묶는다. receipt metadata는 `deckHash`에 넣지 않아 실행 content hash와 receipt annotation을 분리했다.
- **lineage 보존**: currentState overlay가 final experiment의 initial state admission id를 비우더라도, base experiment의 state, provider batch, provider source, price leg, derived return, coefficient receipt parent snapshot은 deck report에 보존된다. final과 stage experiment lineage도 parent list에 합쳐진다.
- **receipt 계약**: deck receipt는 `status="documented"`, `revisionPolicy="explicitAssumption"`, `coverage="synthetic"`, `frequency="scenario"`, `stepSpan=1`, `maxAdmittedStep=0`만 통과한다. wrong kind, parent 누락, admitted처럼 보이는 contract, policy parent, policy admitted parent는 실패한다.
...
## 2026-07-15 P0 realized-driver operating model tournament

- **판단 정정**: admitted driver coefficient는 `WorldModel` 내부 전이 파라미터가 아니라 factor path를 operating shock으로 바꾸는 bridge law다. 이미 변환된 operating path를 모든 후보에 공통으로 주면 coefficient 후보는 실제로 경쟁하지 않는다. 따라서 계수를 억지로 `WorldModel`에 넣지 않고, 동일한 realized driver path를 후보별 bridge부터 실행하는 합성 모델 축으로 바로잡았다.
- **구현 위치**: 새 src 파일, 새 폴더, 새 공개 verb 없이 기존 `hindcast.py`에 `OperatingTransmissionCandidate`, `OperatingModelReplayEpisode`, `runOperatingModelTournament`를 추가했다. scenario 쪽에는 기존 private 검증기를 `validateScenarioCoefficientBindings(case)`라는 내부 호출계약으로 승격해 미래 실험과 hindcast가 같은 admission 검증기를 쓴다.
- **호출계약**: candidate는 model id와 version, measured-association exposure, signed coefficient binding, runtime admission verifier, refs를 가진다. episode는 origin 시점의 `OperatingWorldInputs`, 단일 realized `DriverPathSet`, six-shock baseline, 고정 관측 정책, step별 실제 operating outcome을 가진다. debt, financing, investment bound는 실행 호출에서 명시한다.
- **PIT와 역할 경계**: initial knowledge는 origin 이하여야 하고 decision 시점은 origin과 같아야 한다. realized driver path는 origin 뒤 outcome 시점까지 알려진 `retrospectiveOnly`와 `realizedOutcome`이어야 한다. explicit assumption hash나 overlay가 섞인 path set, 가중 경로, 다중 경로, 고정 기록이 아닌 policy는 실행 전에 거부한다.
- **coefficient admission**: 모든 후보 exposure는 `measuredAssociation`만 허용한다. binding의 receipt, subject, rule, parent, frequency, step span, admitted horizon, source variable order, target shock, exposure contract hash를 기존 scenario 검증기로 다시 확인한다. verifier가 없거나 coefficient만 바꾸고 binding을 갱신하지 않은 후보는 실패한다.
- **실행과 hash**: 각 후보는 realized factor path를 `bridgeOperatingPath`로 독립 변환하고 같은 operating state와 observed policy를 `runOperatingStrategies`에 넣는다. origin run hash는 bridge hash와 candidate hash를 포함하고 executable hash는 operating executable과 transmission candidate를 함께 묶는다. 유효한 coefficient 후보가 바뀌면 loss, candidate row, tournament hash가 바뀐다.
- **정직 경계**: 평가 모드는 `conditionalOnRealizedDriverPath`다. 결과는 계속 `documented`, `notAdmitted`이며 comparison weight는 확률이 아니다. realized driver path를 이미 안 뒤 transmission law를 격리 채점하는 시험이므로 미래 driver 예측력이나 policy admission을 주장하지 않는다.
- **구조 품질과 검증**: `_attempts/operatingModelTournament`에서 import 실패 kill-test 후 후보 경쟁과 verifier 부재 차단 2건을 통과시켰고 본진 회귀로 승격했다. 새 검증 로직은 D/E 복잡도 함수가 없도록 path truth, step contract, outcome contract로 분리했다. focused 37건, `tests/simulate` 전체 441건, ruff format/check, hindcast docstring4 strict, camelCase strict, Guard Index strict l0-l15가 통과했다.
- **다음 P0**: tournament comparison weight와 candidate hash를 미래 conditional experiment의 별도 model uncertainty 축으로 연결한다. 그 축은 model x assumption case x strategy 셀을 보존하고, 가중 평균만으로 worst-model과 regime fragility를 숨기지 않아야 한다. 실제 운영에서는 signed provider observation batch에서 realized driver path와 outcome episode를 자동 조립하는 adapter도 이어서 닫아야 한다.


## 2026-07-15 P0 model uncertainty conditional experiment

- **판단**: 단일 best model로 미래 전략 실험을 실행하면 토너먼트의 법칙 불확실성을 다시 지워 버린다. 필요한 축은 평균 모델이 아니라 `model x assumption case x strategy` 전체 셀이며, 같은 가정에서 모델만 바뀌어도 전략 리더가 뒤집히는지를 보존해야 한다.
- **구현 위치**: 새 src 파일, 새 src 폴더, 새 최상위 공개 verb 없이 기존 `hindcast.py`에 `runModelUncertaintyScenarioExperiment`를 추가했다. 기존 `runConditionalScenarioExperiment`를 후보별로 호출하므로 world, bridge, strategy, objective 실행 계약은 복제하지 않았다.
- **후보 내용 계약**: `WorldModelTournamentCandidate`에 `candidateHash`를 추가했다. 일반 world 후보는 변수, 행동, step, law metadata, parameter, certificate, executable을 묶고, operating 후보는 exposure, coefficient binding, model identity, refs를 묶는다. 미래 실행 시 model id와 version이 같아도 내용 hash가 달라지면 즉시 실패한다. `comparisonHash`는 tournament hash, candidate rows, weights, slices, status를 self-verifying payload로 다시 묶어 weight row만 바꾸는 변조도 거부한다.
- **입력 역할 경계**: future case는 driver path와 baseline만 가지며 exposure와 coefficient binding을 내장할 수 없다. 실행축이 signed candidate law를 주입한다. policy admission evidence나 operating path admission receipt를 case에 섞으면 실행 전에 거부한다.
- **출력 축**: 각 셀은 model id와 version, candidate hash, retrospective comparison weight, case, strategy, score, regret, feasible, breach, leader, assumption set hash, source experiment hash, run hash, result hash를 보존한다. strategy summary는 weighted mean뿐 아니라 worst score, worst regret, worst model, worst case를 남긴다.
- **fragility**: case별로 model마다 leader strategy를 따로 남기고 `leadershipReversal`, stable leader 교집합, weighted leader share, worst model leader score, worst leader margin, model 간 strategy score spread를 산출한다. tie는 해당 model weight를 동률 leader에 나누어 합계 1을 유지한다.
- **실측 회귀**: 같은 성장 path에서 demand transmission coefficient 0.5 후보는 `hold`, 1.5 후보는 `invest`를 leader로 만든다. stress path까지 포함한 2 model x 2 case x 2 strategy의 8개 셀이 순서 비의존적으로 재현되고, worst model과 leader reversal이 결과에 남는다.
- **정직 경계**: tournament는 `eligible`, `documented`, `notAdmitted`, `conditionalOnRealizedDriverPath`여야 한다. comparison weight는 확률이 아니고 case는 동일 비중이다. 결과는 항상 `documented`, `notAdmitted`, `conditionalOnly`, `recommendation=None`이며 model tournament나 score leader를 admission 또는 공식 추천으로 승격하지 않는다.
- **구조 품질과 검증**: `_attempts/modelUncertaintyAxis` import 실패 kill-test 후 본진 계약으로 승격했다. focused 12건, `tests/simulate` 전체 443건, ruff format/check, camelCase strict, 신규 public docstring 9 section, D/E 복잡도 없음, diff hygiene, 긴 줄표 검색, Guard Index strict l0-l15 7개 rule이 모두 통과했다.
- **다음 P0**: signed provider observation batch에서 realized driver path, PIT operating state, observed action, actual outcome을 자동 조립하는 episode adapter를 붙여 실제 기업 origin 수를 늘린다. 그 다음에는 model weight를 확률로 오인하지 않는 uncertainty workbench mirror가 이 셀과 fragility를 읽게 한다.


## 2026-07-15 P0 signed provider operating replay episode assembler

- **판단**: 모델 토너먼트 커널이 있어도 origin 날짜, horizon, 관측 행동, 실제 outcome을 호출자가 따로 넣으면 실제 provider 이력과 합성 fixture를 구분할 수 없다. 과거 운영세계 채점의 입력 자체를 signed provider observation과 admitted PIT state에서 자동 도출해야 했다.
- **구현 위치**: 새 src 파일, 새 src 폴더, 새 최상위 verb 없이 기존 `hindcast.py`, `stateCompiler.py`, `driverObservationBatches.py`에만 연결했다. `_attempts/providerReplayEpisode`에서 import 실패 kill-test를 먼저 확인하고 실제 Ed25519 receipt 수직 회귀를 본진에 승격한 뒤 실험 폴더는 제거했다.
- **정확한 호출계약**: `OperatingReplayOriginBundle`은 admitted `CompiledPointInTimeState`, signed realized driver batch, signed observed action batch, signed actual outcome batch, episode identity, regime, baseline만 받는다. `assembleOperatingModelReplayEpisodes`가 origin을 state decision cutoff에서, outcome cutoff를 세 batch의 공통 cutoff에서, horizon과 event grid를 origin 이후 공통 event에서 자동 도출한다. 호출자는 날짜, horizon, action row, outcome row를 별도로 공급할 수 없다.
- **의미 계약**: driver는 기존 `DriverFactorSpec`과 observation timing SSOT를 재사용한다. action은 priceChange, capacityInvestment, borrow, repay 네 canonical signal의 unit, timing, transform을 정확히 요구한다. outcome은 operating world output의 canonical unit과 timing만 허용한다. 세 batch는 exact signal scope, 같은 entity, 같은 cutoff, 같은 event grid를 만족해야 한다.
- **서명과 PIT 검증**: `validateProviderObservationBatch`를 state compiler의 공개 재검증 seam으로 추가했다. batch content, query contract, artifact bytes, receipt kind, issuer, rule, every source parent, asKnown, asOfExact, issue timing을 기존 `_verifyBatch` 하나로 다시 검증한다. compiled state도 기존 receipt verifier로 재검증하고 candidate coefficient와 initial state는 tournament world runtime에서 다시 검증한다.
- **조립 lineage**: selected revision과 observation id, driver/action/outcome batch id와 receipt id, event grid, assembled episode 전체를 `OperatingReplayLineage.assemblyHash`로 결속했다. 이후 actual row, policy, path, state ref, receipt ref가 바뀌면 tournament 실행 전에 lineage drift로 실패한다.
- **실행 연결**: 조립 결과는 기존 `OperatingModelReplayEpisode`이며 `runOperatingModelTournament`에 그대로 들어간다. 회귀에서는 동일한 signed 이력에서 coefficient 0.5와 1.5 후보를 경쟁시켜 실제 outcome을 만든 1.5 후보가 loss 0으로 선택되는 것을 확인했다. 결과는 계속 `conditionalOnRealizedDriverPath`, `documented`, `notAdmitted`이며 미래 driver 예측이나 policy admission을 주장하지 않는다.
