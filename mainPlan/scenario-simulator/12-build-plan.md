...
## 7. R6 : 시뮬레이터 UI = 두 표면 (원장 + 랩)

**정정 (2026-07-05 운영자: "뷰어가 아니다. 시뮬레이션 과정·전체 결과·디시전이 인터랙티브하게
보여야 하고, 하나의 완벽한 시뮬레이터 제작기여야 하며, 시나리오 변경이 가능해야 한다")**:
UI 표면은 둘이다. 원 PRD 의 인터랙티브 축(00 Play 재생버튼·whatif·AssumptionLedger if 토글·
ScenarioTree 분기)이 R6 의 본체이고, 원장 표면은 그 아래 깔리는 정직의 바닥이다.

### 7a. 원장 표면 (불변·조작 불가 = 정직의 근거)

봉인 블록의 순수함수만 표시: 주간 보드(board100/top10 + 표면 의견 분해 + 기권·커버리지 +
gross/floor/net + never-claim) · 성적표(표면 x 형질 히트맵·후회 곡선·커버리지 채점·미검증 라벨) ·
회사 한 장의 사실부(프로파일 카드 + 재무 실측 구간). 데이터는 fetch + origins 경유, 별도 UI
bake 없음.

### 7b. 시뮬레이터 랩 (제작기: 과정·결과·디시전 전부 인터랙티브)

- **캔버스 = 살아있는 연쇄 DAG**: 11 §4 의 DAG 는 표시물이 아니라 **재계산 계약**이다. 노드
  클릭 = 입력·refs·적용 가정·중간값 열림. 가정 편집 = 그 노드부터 하류만 dirty 전파 재계산,
  바뀐 노드·엣지가 시각적으로 갱신 (데이터가 어떻게 모이고 어떻게 변하는지: 운영자 메모 8층
  요구의 실체). 결정론 코어라 같은 가정 = 같은 결과 (byte 재현).
- **가정 패널 = AssumptionLedger 토글**: 명명 프리셋(baseline/rate_hike/...)에서 출발, if 토글
  on/off + 탄성·가중 슬라이더(사전등록 범위 안) + 범위 밖 커스텀 허용(`provenance=custom` 라벨
  강제). 현재 가정 벌이 sweep 격자 어디에 있는지(강건 영역 vs 극단) + PBO·중앙값 맥락 상시 병기.
- **Play 재생**: 편집된 시나리오로 미래가 시간순으로 펼쳐진다 (05 fan·path·이벤트·재무 연장,
  타임라인 스크럽). 기존 터미널 replay 상태기계의 미래 대칭 확장 그대로 (새 재생 엔진 0).
- **분기·비교 = ScenarioTree**: 임의 노드에서 fork, 시나리오 A/B/봉인 baseline 나란히 diff
  (top10 진입/이탈 종목 + **책임 판독·책임 가정 역추적** = 디시전이 왜 바뀌었는지까지 인터랙티브).
- **저장·재현·공유**: 커스텀 런 = scenarioId + 가정 전체 직렬화 (누구나 byte 재현). simulated·
  custom 라벨 영구, 트랙레코드 주장 불가.
- **원장으로의 다리 (제작기 → 봉인)**: 랩에서 만든 시나리오를 **사전등록**하면 다음 주기부터
  봉인·채점 대상이 된다 ("내 시나리오가 채점받기 시작한다"). 탐색은 자유·권위는 원장이라는 06
  규율의 UI 실장이자 제작기의 완성 고리.

### 7c. 컴퓨트 배치·규약

- 공개 터미널: 결정론 코어를 **pyodide 로컬 실행** (wheel 라이브 존재, numpy 결정론이라 이식
  적합. 회사 단위·연쇄 단위 재계산 = 시나리오 변경이 공개면에서도 됨). 전 유니버스 sweep 급은
  사전 계산 격자 조회 + 로컬 앱(:8400) 풀파워. 서버 비용 0 유지 (무료 티어 규약).
- 표현: DAG v0 = 트리/산키, 3D 는 표현 교체 (재계산 계약 불변이므로 재배선 0). 재무 연장 차트 =
  MiniFinChart 확장 (simulated 구간 라벨. 신규 차트 금지, 정본 = `ui/packages/surfaces/src/terminal/charts/MiniFinChart.svelte`).
- 무중단 규약: 로컬 프리뷰 격리 → 미배선 커밋 → 완결 단위만. push 는 스크린샷 눈검수 + 운영자
  승인 후.
- R6 착수 선결: cascade DAG JSON 계약(R5) + **pyodide 에서 simulate 코어 구동 스파이크 (R6-0)**.
- **시각·모션·스택 상세 = 13-lab-ui-spec.md** (제품 지형 판정: 풀 비전 실존 제품 없음, 축별
  최고 실현체 차용표 + ComfyUI 이벤트 스트림 실행 모델 + 그래프x저널 이중 표현 + Svelte Flow/
  cosmos 스택 + 3D 기각 + 타이밍 계약 모션 스펙 + R6-0~R6-5 세부 순서). 엔진 측 선반영 1건:
  opine/cascade 는 노드 단위 yield (이벤트 스트림 프로토콜, R1 골격부터).
