...
allFilings 타입별 이벤트 표면 replay(기준선 대비 기여 측정) · panel PIT(정정 look-ahead) 실측 ·
표면 상관 행렬 → 수축 묶음 · 가정 sweep 프로토타입(격자 100벌 x 17년 실측) · 뉴스 태깅
커버리지(G8). 로컬 allFilings 일별 parquet 은 2024-09~ 한정이라 **과거 월별 백필 커버리지
확인이 이벤트 표면 replay 의 선결** (HF 월별 아카이브 실측 필요).
