...
## 2026-08-03 질문 계약 선제 주입과 결정론 교정

- 대상, 지표, 기간을 `claimCellContract`로 구조화해 최초 턴과 교정 턴에 같은 원 질문 완료 조건을 주입했다.
- 정량 근거 셀이 이미 완결된 후보는 모델을 다시 부르지 않고 exact value/date/table ref 표로 재구성한다. 근거가 실제로 부족할 때만 같은 네이티브 세션의 보완 턴으로 내려간다.
- 실제 Codex 실행에서 후보가 한 차례 거부됐지만 추가 네이티브 턴 없이 최근 5개년 매출과 영업이익 10개 셀을 결정론적으로 구성해 `verify: ok`로 끝났다. 총 80.9초, 의미 도구 3개, 종료코드 0이었다.
- 관련 runtime, gateway, capability 회귀 120개와 Ruff check, format을 통과했다. 사용자의 exact evidence 확인 동작은 대신 수행하지 않았다.

### 판정

`improve`. 사후 모델 교정에만 의존하던 경로를 실행 전 완료 계약과 결정론 복구로 강화했다. 이는 품질과 실행 효율 근거지만 verified 사용자 결과는 아니므로 북극성 분자에는 추가하지 않는다.
