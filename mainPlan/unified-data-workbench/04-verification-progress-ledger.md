...
### 2026-07-26 W22: CI Fast green. 자식 실패 보고 경로 완결

세션 시작 시점에 CI Fast 는 12 연속 red 였고 마지막까지 남은 실패는 Linux 자식 건
하나였다. 두 단계로 닫혔다.

첫째, 자식이 침묵하던 구간을 없앴다. 기존 실패 경로는 worker 가 살아 있으면 결과를
보내지 않았고, 부모는 원인 대신 자기 기한을 통째로 소모했다. 보내도록 바꾸자 같은
실패가 179.998 초에서 1.789 초로 바뀌었다. 정지가 아니라 침묵이었다는 뜻이다.

둘째, 부모가 자식의 보고를 덮어쓰던 자리를 고쳤다. 자식 worker 는 non-daemon 이라
결과를 보낸 뒤에도 잠시 더 살아 있는데, 부모는 그 지연을 보고 `CHILD_DID_NOT_EXIT`
로 코드를 갈아치웠다. 자식이 적은 typed 코드가 있으면 그것을 쓰고 없을 때만 부모의
추정을 남기도록 바꿔 `PAGEABLE_EAGER_CODE_PIN_FAILED` 가 그대로 올라온다.

교훈은 진단 순서다. 이 실패에 대해 두 번(setsid 권한, 기한 부족) 가설로 틀렸고, 둘 다
로그가 원인을 담지 않아 검증할 수 없는 가설이었다. 세 번째에 추정을 멈추고 관측을
먼저 세우자(stall frame 요약, 침묵 제거, 부모 판단 상태 telemetry) 한 번에 닫혔다.
