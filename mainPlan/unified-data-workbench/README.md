...
**DartLab DataHub는 L1, L1.5, L2가 소유한 모든 데이터 자산과 계산 capability를 하나의 카탈로그와 질의 계약으로 발견하고, PIT, 계보, 가시성, 예산을 보존한 채 외부 프로세스와 simulator에 동일하게 제공하는 독립 데이터 플랫폼 엔진이다.**

팩터 스토어는 이 제품의 정체성이 아니다. scalar-compatible observation을 `factor` 형태로 투영하는 한 가지 query projection이다.

## 최상위 결정

1. 실제 엔진 폴더와 공개 진입점의 정본은 `src/dartlab/dataHub/`와 `dartlab.dataHub(...)`다. `dartlab.data`는 호환 별칭만 유지한다.
...
`refresh`는 현재 source와 contract를 다시 해소해 exact generation을 만들거나 이미 같은 generation이 READY면 재사용한다. `reuse`는 같은 logical query의 최신 READY generation을 먼저 찾고 없으면 cold build한다. `offline`은 receipt의 exact generation만 읽는다. 세 모드는 같은 `DARTLAB_HOME`을 보는 다른 Python 프로세스에서 쓸 수 있다. 원격 프로세스는 `DataHubClient`와 `AsyncDataHubClient`, 여러 실행 노드는 lease 기반 `DataHubWorker`로 같은 계약을 소비한다.
