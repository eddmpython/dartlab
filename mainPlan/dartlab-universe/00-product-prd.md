...
**DartLab Universe의 최종 목표는 Hugging Face `eddmpython/dartlab-data`의 모든 주소 가능한 데이터와 DartLab의 모든 공개 가능 지식, 엔진, capability, Skill OS, 문서, 근거, 이미지와 시간 변화를 하나의 지식 우주로 연결하는 것이다. 사용자는 우주처럼 확대 및 축소하며 전체에서 원본 한 건까지 이동하고, 지식이 연결되고 수정되고 전개되는 과정을 재생 가능한 장면으로 본다.**

이 목표에서 법인, 공시, 재무, 산업 관계는 핵심 은하지만 우주 전체와 동의어가 아니다. 기존 금융 조사 기능은 삭제하지 않고 12개 지식 은하 중 `법인과 기관`, `공시와 문서`, `재무 관측`, `산업과 관계`, `가격과 퀀트`, `거시와 공공`에 배치한다.

전체를 담는다는 말은 수백 GB 원문을 브라우저 메모리에 복사하거나 모든 행을 동시에 점으로 그린다는 뜻이 아니다. 모든 항목을 정확히 주소화하고, 검색과 계층 탐색으로 도달 가능하게 하며, 현재 질문에 필요한 bounded scene만 투영하고, 원본 내용은 range 및 행 단위로 지연 해소한다는 뜻이다.

사용자가 직접 검수하고 명시적으로 승인하기 전에는 공개 진입 버튼이나 공개 내비게이션 링크를 만들지 않는다. `/universe` 라우트는 유지한다. 상세 구현 계획과 벤치마크는 `13-unified-knowledge-universe-plan.md`가 현재 SSOT다.
...
**DartLab Universe는 DartLab의 모든 데이터와 지식을 주소, 관계, 근거, 시간으로 연결하고, 질문을 재현 가능한 공간 장면과 지식 필름으로 컴파일하는 evidence-native 지식 우주다.**
