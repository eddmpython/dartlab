...
Right Evidence Drawer
  Relation summary | assertion timeline | source | values | limitations

Bottom Time and Coverage Rail
  dataAsOf | valid range | known range | candidate/fact coverage
```
...
| scenario | 보라 animated dash | 사용자가 적용한 가정 |
...
AI 사용 가능 환경에서는 기존 Workbench가 같은 값을 채운 `ProjectionSpec`을 반환한다. 질문별 정규식 router와 템플릿 답변을 Universe에 만들지 않는다.
...
질문 원문, API key, raw evidence, 사용자 history는 넣지 않는다. 오래된 buildId를 재현할 snapshot이 없으면 현재 데이터 재실행과 원본 재현 불가를 분리해서 표시한다.
...
다음 다섯 데모를 실제 브라우저에서 통과해야 MVP다.

1. atlas 첫 진입 후 반도체 산업 열기
2. 삼성전자 1-hop에서 fact와 candidate 분리
3. relation 하나의 DART sourceRef와 period 열기
4. knownAt을 과거로 옮겨 이후 assertion 제거
5. 삼성전자와 Apple의 같은 재무 metric 및 source 비교
