...
DartLab 웹 Python 표면을 pyproc 0.0.10의 root machine 계약으로 통합했다. 블로그 셀, 브라우저 노트북, 플레이그라운드는 한 worker와 한 machine을 공유한다. 블로그는 처음부터 편집 가능한 셀을 보여 주고, 사용자가 실행하거나 명시적으로 hover하기 전에는 Python과 데이터를 받지 않는다. 글 전체 화면은 셀 순서를 보존하는 sequential 노트북으로 투영하며 자동 실행하지 않는다.
