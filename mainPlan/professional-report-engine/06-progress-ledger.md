...
- ◐ P1d 정량 moat . ✅ **개념확립 통과**(`tests/_attempts/quantMoat/concept.py`, graduation gate 준수): C1 ROIC−WACC 지속성·C2 마진 CV·등급 논리곱(wide/narrow/none, noComposite)·정성원천(switching/network/brand) unmeasured 명시. offline 5 체크.
  ...
  - ⏸ **본진 졸업은 아직**: WACC 전사공통 8.5%·세율 22% 고정 근사라 *엔진 정합* 판정이 아니다. 회사별 WACC 로 재현 후에만 `moat.py` 배치 + axis 등록. 생존 편향도 잔존. 상세·수치·한계 = `tests/_attempts/quantMoat/README.md`.
  - ★ **엔진 정합을 막는 진짜 원인 특정(실측)**: `_investmentAnalysisRoic._estimateWacc` 는 회사마다 `gather.sources.price.fetch`(시총) + `_fetchBeta` 를 **네트워크로 개별 호출**한다(docstring 자인: 각 ~2.6s). 1,000 사면 수 시간 -> 유니버스 백테스트 불가. 그런데 **전 종목 가격 벌크가 이미 로컬에 있다**: `data/gov/prices/company/{code}.parquet`(사당 일별 OHLCV ~16 년) · `data/krx/prices/raw-{year}.parquet`(671MB) · `universe-monthly.parquet`.
    - **제안(운영자 결정)**: WACC 의 베타·시총 입력을 per-company 네트워크 fetch 대신 **로컬/HF 가격 벌크 직독**으로 전환. 그러면 (a) P1d 엔진 정합 게이트를 유니버스로 돌릴 수 있고 (b) P1a 밸류에이션 경로의 회사당 ~2.6s 외부호출도 사라진다. 밸류에이션 엔진 데이터원 변경이라 사전 승인 대상.
