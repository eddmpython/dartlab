...
## P2 · 리포트 엔진
- ⚠ **정합성 정정(2026-07-06, 실측 census)**: PRD "死코드 삭제"는 stale. `story/macro`·`sixAct`·`dashboard`는 **죽지 않았다**: `story/__init__.py:59`가 `sixActScore` export, `reportTypes.py`가 `macro`·`dashboard` reportType 으로 이 빌더들을 여전히 소비(buildStory 라이브 의존). 즉 삭제 = 기계적 파일 제거가 아니라 **story 재구축(buildStory/reportTypes 가 이 빌더 없이 서게) 선행 후 삭제**. 기계적 삭제는 macro/dashboard/sixAct reportType 회귀. 실행 전 emitter 로 이 관점들 대체 + 소비자 전수 이관 필요(깊은 마이그레이션, 운영자 착수 승인·[[plan_deep_gate]] 대상).
- ☐ 삭제 ~2,834 LOC (`story/macro`·publisher·sixAct·dashboard·sections) . 위 정합성 선행 조건 미충족이라 보류
