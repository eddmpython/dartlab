...
### 2.3 판독치 정의 (엔진별로 실제 내놓는 값)

각 판독은 "이 엔진의 데이터가 이 종목의 향후 지평 내 상대 성과에 대해 말하는 것"을 direction/score 로 요약한 것이다. 구체 정의:

| engine | 판독치의 내용 (score 의 의미) | direction 규칙 | 갱신 리듬 |
|---|---|---|---|
| quant.price | 5d 상대우위 점수: 단기 reversal·잔차 모멘텀·거래량충격·52주고가 근접의 시장 내 rank 합성 | 상위 20% = +1, 하위 20% = -1, 중간 = 0 | 매주 |
| quant.flow | 수급 압력 점수: smartMoneyZ60d + 연속 순매수일 + flowMomentum20d | z > +1 이고 streak ≥ 3 이면 +1, 반대면 -1 | 매주 |
| quant.alphas | "새 재무 정보의 방향": SUE(발표 후 경과일 감쇠)·fundmom·accruals 경보 | 새 분기/잠정실적이 도착한 주만 ±1, 그 외 중립 (레벨 우려먹기 금지) | 분기+잠정 |
| scan.financial | 산업 내 재무 percentile 의 델타 (레벨 아님) | 델타 상·하위 분위 ±1 | 분기 |
| scan.events | 최근 5거래일 이벤트 방향 합: 수주(+, 규모/시총 가중)·자사주(+)·내부자 순매수(+)·유상증자(-)·watcher diff 중요도(±) | 이벤트 점수 부호. 이벤트 없으면 중립 | 매주 (이벤트성) |
| frame.notes | 최신 보고서 노트의 전기 대비 변화: 수주잔고 증감·리스크 문단 신설/삭제·세그먼트 mix | 변화 방향 ±1, 보고서 없는 주 중립 | 분기 |
| quant.text | 최근 7일 뉴스·공시 톤 z | 톤 z 상·하위 ±1, 미커버 기권 | 매주 |
| credit | 스코어카드 등급·Altman 의 변화 방향 (악화 감지 특화) | 악화 = -1 중심. +1 은 회복 확인 시만 (상승 예측력 약함을 스스로 명시) | 분기 |
| analysis | 재무 인과 종합의 방향화. quant.alphas 와 상관 높아 **후순위** (성적표가 독립 기여를 증명하면 활성) | ±1/중립 | 분기 |
| industry | 소속 산업의 상대강도 + 수출(customs) 사이클 방향 틸트 | 약한 ± (score 절대값 ≤ 0.3 상한) | 매주/월 |
| macro | 종목 판독 아님: 시장 레짐 판정 (riskOn/Off) → family 가중에 작용 | 해당 없음 | 매주 |
| quant.forecast | 5d conformal 구간 (점예측+90% 구간). top10 비모순 게이트 전용 | 게이트 (구간 상단 ≤ 0 이면 탈락) | 매주 |

**지평 다중화 (장기 유지가능성 조항)**: 계약의 horizon 필드로 엔진은 자기 자연 지평의 판독을 낸다. v0 = 5d 단일이되, fund 계열(quant.alphas·scan.financial·analysis·credit)은 20d/60d 판독 병행 발행을 P4 에서 활성한다. 원장은 지평별로 채점하므로 "이 엔진은 어느 지평에서 강한가"가 측정된다. 주간 제품(board100/top10)은 5d 슬라이스 + 장지평 판독을 근거 맥락으로 소비한다. 재무 엔진을 5d 에 강제해 억울한 성적을 만들지 않는 것이 fund family 의 장기 생존 조건이다.
