...
## 1. 배치 (v0.6.1) : 기존 L2.5 `simulate` 엔진 확장 (신설 0)

```
L2.5 simulate            한 집: [기존] 원장·채점·시나리오 DAG + [신규] 상 차리기 → 의견화 →
                         가정 sweep → 파생 뷰. 공개 verb = dartlab.simulate 계열 확장
L3  story                시뮬레이터 산출(성적표·board·서사 재료)을 하향 소비해 산문 조합
L2/L1.5/L1               기존 그대로 (변경 0. 개별 데이터 작업대 신설 금지, dossier 폐기 df41ee60e)
```

- L3 신설안 폐기 사유: story(L3)가 동층이라 소비 불가 (운영자 지적) + L2.5 에 시뮬레이터가
  이미 실재·승인됨 (scenario-simulator 코어 9945f6f1f~ac3905fd9 + expectation-grid 원장
  P1~P6, 2026-07-03 운영자 승인·라이브 발행).
- import 방향: simulate(L2.5) → {scan·frame(L1.5), quant·analysis·credit·macro·industry(L2),
  gather·providers(L1), core(L0)} 전부 하향 OK (`issueMacro` 가 L2.5→L2 호출 선례).
- 폐기 원칙: 호출계약은 엔진 소유 verb 로만. 엔진 없는 root facade 신설 금지.
- 프론트 작업대(ui runtime data/fetch)는 유지 (본 엔진과 별개 표면).

## 2. 파일 · 함수 (본진 배치 시점, 전부 `simulate/` 내부)

| 파일 | 핵심 | 역할 |
|---|---|---|
| `simulate/reading.py` (신설) | `Reading` dataclass + 임계 상수 SSOT | 01 §2 |
| `simulate/surfaces.py` (신설) | `enumerateSurfaces()` | 카탈로그 자동 열거 + 방향화 선언 레지스트리 (01 §3) |
| `simulate/table.py` (신설) | `buildTable(asOf, market)` | 상 차리기: 전종목 x 전표면 스냅 (무bake, 벌크 우선, Company 루프 0) |
| `simulate/opine.py` (신설) | `opine(table)` | 의견화: 표면별 direction/score/기권 |
| `simulate/sweep.py` (신설) | `sweepAssumptions(readings, grid)` | 가정 격자 실행 + 강건성 집계 (01 §5) |
| `simulate/board.py` (신설) | `buildBoard(...)` | board100/top10 + red-flag 게이트 + 근거 서사 조립 |
| `simulate/runweek.py` (신설) | `runWeekly(asOf, market, seal)` | 주간 루프 오케스트레이션 |
...
| `simulate/entry.py` (확장) | 공개 verb 표면 (기존 `dartlab.simulate` 계열에 주간 판독 verb 추가) | 진입 |
| cli | `dartlab simulate ... [--asof --market --seal]` | 운영 진입 (P2) |

## 3. 테스트 (src ↔ tests 미러)

| 테스트 | 검증 |
|---|---|
| `tests/simulate/test_reading.py` | Reading 무결·기권 1급·임계 SSOT 단일성 |
| `tests/simulate/test_surfaces.py` | 자동 열거 완전성(카탈로그 대비 누락 0)·방향화 선언 존재·무방향 등재 |
| `tests/simulate/test_table.py` | 전종목 완전성(판독/중립/기권 3분법)·Company 미사용·시장 파라미터 |
| `tests/simulate/test_sweep.py` | 격자 결정성·강건성(median) 선정·최고성적 인용 금지 게이트 |
| `tests/simulate/test_board.py` | 스키마(Pandera)·refs 동행·금지 어휘 grep·시장 혼합 순위 부재·10 미만 발행 |
| `tests/simulate/test_pit.py` | pitLag 보정·rcept_dt look-ahead 부재 (합성 fixture) |
...
- [ ] tests 구조 미러 (`test_structureMirror`) + lint-imports (L2.5 기존 행 확장, 신규 레이어 0)
- [ ] publicApi manifest + `publicApiScenarios.yml` 등록 + `productSmoke --suite quick`
- [ ] Skill OS: `engines.simulate` sub-spec 4단계 (신설 또는 기존 갱신. lintSkill·capabilityRefs·검색·동기화)
