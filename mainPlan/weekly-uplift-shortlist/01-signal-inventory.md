...
### 2.6 panel·finance 심화 활용 : fund reader 의 본체 (v0.3.1 보강)

fund reader 의 연료를 "scan 파생 축 + alphas 비율" 표면에서 **panel 전계정 wide 직독**으로 격상한다.
dartlab 의 차별 자산은 남들도 있는 비율 몇 개가 아니라 전상장사 x 전계정 x 분기 시계열 + 노트/
세그먼트(frame/inventory·dossier)다. 주간 지평에 맞게 **레벨이 아니라 변화(델타)와 공시 직후
윈도우**로 소비한다.

| 신호 후보 | panel/finance 원천 | 지평 논리 |
|---|---|---|
| SUE·영업이익 가속 | 분기 매출·영업이익 YoY 의 z + 직전 분기 대비 가속 | 발표 직후 드리프트 (PEAD, rcept 후 경과일 감쇠) |
| 발생액 경보 | (NI - CFO)/자산, 재고·매출채권 증가율 vs 매출 증가율 괴리 | 이익의 질 악화는 후속 주가에 지연 반영 |
| 재무구조 델타 | 단기차입 비중·현금 변화율·이자보상 변화 (분기 대비) | 구조 개선/악화의 공시 직후 재평가 |
| 세그먼트 mix 변화 | salesByProduct 부문 비중 변화·HHI 변화 (노트) | 주력 부문 전환은 재평가 트리거 |
| 수주·CAPEX 노트 | 수주잔고·CAPEX 노트 시계열 (고가치 노트 first-class) | 선행 지표의 공시 직후 윈도우 |
| 산업 내 percentile 델타 | 전계정 산업 내 백분위의 분기 변화 | 레벨(느림)이 아니라 변화(이벤트성) |

**PIT 함정 (실측 선결, 다음 사이클)**:
- panel 은 정정·재작성 반영본일 수 있다 → 과거 replay 에서 "그 시점에 알려진 값" 재현이
  안 되면 look-ahead. 접수일(rcept_dt)은 allFilings 로 복원 가능하나 **정정 전 원본 값** 보존
  여부는 실측 필요. 재현 불가 판정 시 근사(정정 반영 값 + 접수일 시차) + 한계 flag 로 운영.
- 분기 공시 주기라 fund 신호의 유효 갱신은 분기 4회 + 잠정실적. 주간 판독에서는 "새 정보가
  도착한 주"에만 방향을 내고 나머지 주는 중립이 정직하다 (레벨 우려먹기 금지).
