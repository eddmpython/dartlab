...
`select` 는 그 표면에서 특정 계정만 꺼내는 보조 손잡이다.
커리큘럼에서 `select` 가 `panel` 보다 앞서거나 크게 서면 실패다.
새 편은 먼저 "이 데이터가 panel 안에서 어떤 행과 기간으로 존재하는가"를 보여 주고, 그다음 필요할 때만 `select` 로 좁힌다.
...
8. **좁히기**: `select` 는 원천이 아니라 panel 위의 편의 손잡이다.
9. **검산**: 직접 계산, 빈칸 해석, 연결과 별도, 단위 확인.
10. **출처 되짚기**: trace, filings, 원문 링크, 자동 분석의 근거 확인.
...
| 2 | 회사 하나를 만들 수 있음 | DART 회사의 `panel` 이 재무제표와 사업보고서 본문을 함께 담는 단일 표면임을 안다 | `panel()`, `panel("IS")`, `panel("사업")` 의 모양 차이를 말한다 |
...
| 11 | panel 검색으로 원천을 볼 수 있음 | `select` 는 panel 에서 계정만 좁히는 보조 손잡이임을 안다 | 없는 계정을 넣고 빈 결과를 panel 검색으로 되돌려 해석한다 |
| 12 | select 결과를 받을 수 있음 | 파이썬으로 직접 계산 | 매출액과 영업이익으로 마진을 계산한다 |
| 13 | 직접 계산한 숫자가 있음 | 숫자의 출처 추적 | 한 숫자가 어느 원천 항목에서 왔는지 되짚는다 |
...
| 19 | 자동 문장을 검증할 수 있음 | 자동 해석의 한계와 오답 방지 | 믿을 문장, 확인할 문장, 버릴 문장을 나눈다 |
...
- [ ] **10. 주석과 숫자, 서로 묶기** `notes-link-to-numbers` `B`
  ...
  로컬 실측 2026-07-11: `panel("주석")` 219행, `panel("재고")` 4행.
- [ ] **11. 필요한 줄만 이름으로 뽑아낸다** `select-rows` `B` (2편이 일부 흡수. 잔여는 항목명 검색)
  select 는 주연이 아니라 panel 에서 계정만 좁히는 보조 손잡이다. 빈 결과는 panel 검색으로 되돌린다.
- [ ] **12. 뽑은 숫자로 직접 계산한다** `compute-from-values` `B`
  select 결과를 파이썬으로 다뤄 영업이익률을 직접 구한다. 첫 데이터 가공.
- [ ] **13. 이 숫자는 어디서 왔나** `trace-the-number` `B`. `Company.trace("IS")` (topic 인자. 계정명을 넣으면 None)
...
- [ ] **19. 자동 해석을 그대로 믿지 않기** `dont-trust-blindly` `B`. 자동 분석의 한계 + 출처 되짚기
...
- [ ] `select` 부터 시작하지 않는다. 먼저 `panel()` 또는 `panel(topic)` 으로 원천 표면을 보여 주고,
      `select` 는 그 뒤에 필요한 행만 좁히는 보조 손잡이로 쓴다.
...
| 호출 | 로컬 | 결과 |
|---|---|---|
| `c.panel()` | 된다 | Panel (1627, 49) |
| `c.panel("사업")` | 된다 | DataFrame (156, 49) |
| `c.panel("위험")` | 된다 | DataFrame (25, 49) |
| `c.panel("경영진단")` | 된다 | DataFrame (46, 49) |
| `c.panel("주석")` | 된다 | DataFrame (219, 49) |
