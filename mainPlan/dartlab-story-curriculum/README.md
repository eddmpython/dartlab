...
- 6편 이후 세밀화 우선순위는 다음 실패 지점이다. 편성표를 확정하거나 새 글을 끼워 넣을 때는 이 순서를 먼저 본다.
  1. **매출액이 안 보일 때**: 항목명 차이, 회사별 계정명, select 실패를 다룬다.
  2. **빈칸은 0이 아니다**: 공시 없음, 해당 계정 없음, 상장 전 기간, 기준 차이를 구분한다.
  3. **연결과 별도, 숫자가 갈린다**: 같은 회사 같은 기간인데 숫자가 달라지는 이유를 설명한다.
  4. **영업이익률, 직접 계산**: 받아온 숫자를 그대로 믿지 않고 직접 계산하고 검산한다.
  5. **이 숫자, 어디서 왔나**: 표의 한 숫자를 원천 항목과 호출 조건까지 되짚는다.
