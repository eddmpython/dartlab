...
> **승계됨 (2026-07-10)**: 폴더 SSOT 머리는 `04-screener-os-prd.md` 다. 본 문서는 Phase 1~3 완료 기록으로 보존한다. 미착수 Phase 4 (프론트가 spec 소비) 는 04 의 P5 로 흡수됐다.
...
- 2026-07-06: **Phase 2b 완료 (복합축 승격).** `_COMPOSITE_AXIS_FIELDS` 11개(axis.debt.icr/riskGrade/debtRatio/shortRatio, quality.cfToNi/accrualRatio/grade, audit.opinion/riskLevel, dividendTrend.pattern, liquidity.grade)를 `_catalog` source="axis"로 등재 + `_loadCompositeAxis` resolver(raw 스캐너 네이티브 컬럼 직독, dispatch 한글 리네임 우회, `_axisCache` 축 1회). 축 스캐너 실측 정독: 스캐너 함수는 영문 컬럼(scanDebt만 한글), scanIcr 는 이자를 {이자비용·금융비용·금융원가} 넓게 잡아 raw interest_expenses(78종목) 대비 커버리지 우수 → axis.debt.icr 이 sparse ICR 문제 해결.
- 2026-07-06 게이트: 신규 유닛 6(레지스트리 정합·해소·캐시·컬럼부재·실패흡수) pass + **requires_data 드리프트 가드**(실 스캐너 컬럼 실재 검증, 메모리 안전 스캐너별 로드/해제) pass · ruff · publicApiCoverage OK · dartlabGuard strict l0-l15 PASS. flagship v2 = axis.debt.icr>2 추가. 실측: 삼성전자(ICR 11.8)·현대모비스(9.2)·삼성물산(12.3)·LG(30.7)·SK스퀘어(632)·LG생활건강(41) 등 robust ICR 안전 종목 한 spec.
- 2026-07-06: **왓처 구독 배선 완료.** `scan/screen` 에 `evaluateScreenMembers(id)`(스크린→현 멤버셋) + `watchedScreens()`(notify=true opt-in). notify-watch `.github/scripts/notify/watch.py` 에 `screenAlert` stateful 토픽 추가(newOrders 동형 `/active` set-diff 커서 = 멤버십 진입 발화, 이탈 후 재진입 재발화). flagship JSON `notify:true`. 첫-실행 flood 회피로 default 토픽 제외(운영자 롤아웃 게이트). operation.notifyPipeline SSOT 등재.
- 2026-07-06 게이트: 신규 유닛 notify 3(screenAlert 매치·stateful·default제외) + scan 3(watchedScreens·evaluate·empty) pass · ruff · publicApiCoverage OK · dartlabGuard strict l0-l15 PASS. **실 데이터 end-to-end**: watchedScreens → evaluateScreenMembers(40 멤버 삼성·현대모비스...) → eval_screen_alert(40 매치, "[스크린] 삼성전자 · 하락장 재무안전" /terminal?sym=005930) → stateful /active. **원래 동기 "주가 빠질 때 안전 종목 감시" 완성.**
- 2026-07-06: **Phase 3 완료 (시계열·상대 노드).** `define` AST 를 단항 시계열(`mean/min/max/yoy/cagr/slope`, 연간 격자 `_loadFieldSeries` = scanAccount/scanRatio freq="Y" 최근 N년 위치정규화)과 횡단 상대(`percentile/zscore`, `by` 생략=전체·`by:"industry"`=업종)로 확장. `_evalDefineNode` dispatch 분기(_TEMPORAL_OPS·_RELATIVE_OPS). 시계열 field=원천 시계열 필드만(valuation/krx/note ValueError), 상대 field=원천 또는 @참조. 업종 join 원천 = `dartlab.listing()` 업종 컬럼(_industryMap, 2663 커버·159 업종). slope=OLS(null 무시 long-form), percentile=(rank-1)/(n-1)*100, zscore=(v-평균)/표준편차·std0→null. 단위: yoy/cagr=배, percentile=백분위, zscore=표준편차, 축약=원 단위. 회귀 0(op 없는 기존 define 동일 경로).
- 2026-07-06 게이트: 신규 유닛 15(temporal 7·relative 7·통합 1, test_screen_define 17→32) + 실 데이터 requires_data 5(업종매핑 커버·연간격자·백분위 단조 corr>0.99·z평균~0·데모스크린 흑자게이트) pass · ruff clean · dartlabGuard strict l0-l15 PASS(7 rules·active debt 0·folderMirror 포함 외부 6 게이트) · publicApiCoverage OK(scanAxes 25 불변, define 문법 additive) · SKILL.md define 노드 문법표 추가. **데모 스크린 `resilientCompounders.json`**(3년 연속 흑자 min>0 + 매출 CAGR>3% + 3년평균 저부채<120 + 업종 ROE 백분위>60) `dartlab.scan("screen","resilientCompounders")` 로 40종목(비츠로셀·티앤엘·아이디스홀딩스 등, 전원 업종 ROE 100). **산술+시계열+상대를 한 언어중립 JSON spec 으로 결합 = 컴포저블 쿼리 비전 백엔드 완성.**
- 잔여(백엔드 완성, UI만): **Phase 4**(퍼블릭 scan 뷰가 screens/*.json 렌더, TS 3~4중 정의 붕괴, 눈검수 격리·자동push 금지). 왓처 배포는 운영자 게이트(operation.notifyPipeline 런북 = PUSHHUB·VAPID secret, 백엔드는 라이브 배포됨).
