"""Python MCP SDK client로 실제 attach해 MCP 0.11 강화 사이클을 검증한다.

검증 대상:
- ToolAnnotations (readOnly/destructive/idempotent/openWorld) advertise
- CallToolResult.structuredContent: RunPython의 ref dict 직접 노출
...
async def _run_probe() -> dict:
    ...
    # progress notification 테스트를 빠르게 하도록 임계 1 s, 간격 0.4 s로 override한다.
    ...
    # cwd를 tests/가 아닌 곳으로 명시한다. sys.path[0]가 'tests/'면 'tests/mcp/__init__.py'
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # 8. progress notification: 2.5 s sleep + progress_callback으로 progress emit 카운트.
...
def test_mcp_strong_annotations_and_structured_and_prompts():
    """0.11 ToolAnnotations, structuredContent, prompts, setLevel의 e2e 동작을 검증한다."""
