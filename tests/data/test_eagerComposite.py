...
def _eagerDescriptor(
    ...
    return DataAssetDescriptor(
        ...
        owner="tests.data.test_eagerComposite",
        layer="L2",
        kind="computed",
        label="Mixed eager fixture",
        description="Fixture-only mixed eager executor",
        sourceRef=f"python:tests.data.test_eagerComposite:{attribute}",
        queryable=True,
        executorKind="callable",
        executorModule="tests.data.test_eagerComposite",
...
def testMixedInitialSealsGeneralEagerAndResumeTouchesOnlyLocator(
    ...
    monkeypatch.setattr(
        "dartlab.data.execution.buildCatalog",
        lambda: catalog,
    )
    ...
    monkeypatch.setattr(
        "tests.data.test_eagerComposite._fixtureMixedEager",
        failIfEagerRunsAgain,
    )
    monkeypatch.setattr(
        "dartlab.data.eagerSupervisor.runEagerSeal",
        failIfEagerRunsAgain,
    )
