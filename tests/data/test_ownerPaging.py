...
import dartlab.data.catalog as dataCatalog
import dartlab.data.ownerPaging as ownerPaging
import dartlab.data.ownerPagingEntity as ownerPagingEntity
...
from dartlab.data.featureQuery import buildFeatureObservationSet
from dartlab.data.featureRegistry import StateVariableSpec, buildStateVariableRegistry
from dartlab.data.ownerProcess import OwnerProcessPage
from dartlab.data.universe import ResolvedMarket, ResolvedUniverse
...
def _universe(size: int = 10) -> pl.DataFrame:
    return pl.DataFrame(
        {
...
def testOneQueryPagesWholeListedUniverseWithoutExternalSubjectLoop(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert pages[-1].universeCoverage[0].returnedEntities == 10
...
def testEntityFailureAdvancesCursorAndKeepsCumulativeCoverage(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert finalCoverage.returnedEntities == 9
...
def testOwnerCodePinIsStableAcrossFreshProcess() -> None:
    ...
    script = (
        ...
        "from dartlab.data.ownerPaging import _ownerCodePin;"
...
def testRequireCompleteFailsBeforeOwnerButMixedUsesOuterChain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [partition.requestId for partition in mixed.partitions] == ["features"] * 8 + ["locator"]
...
def testMixedOuterResumeRejectsOwnerSourceDriftWithoutRerunningAnyLane(
    ...
    _sourceCalls, ownerCalls = _install(monkeypatch)
    import dartlab.data.execution as executionModule
...
def testRowBudgetCutsPageBeforeEntityCapWithoutLossOrDuplication(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [len(page.partitions) for page in pages] == [3, 3, 3, 1]
