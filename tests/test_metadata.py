...
class TestModuleMeta:
    def test_core_modules_exist(self):
        ...
        core = ["BS", "IS", "CF", "audit", "dividend", "majorHolder", "employee", "executive", "segments", "fsSummary"]
        for name in core:
    ...
    def test_related_modules_valid(self):
        ...
        for name, meta in MODULE_META.items():
            for related in meta.relatedModules:
                assert related in MODULE_META, f"{name}.relatedModules에 '{related}'가 있지만 MODULE_META에 없음"
