...
def testReceiptPageAndHandleUseTwoLedgerConnections(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    store = MaterializationStore(tmp_path / "materialization", policy=policy())
    exactPins = pins(query="reader-connection-budget")
    ...
    originalConnect = store._connect
    calls = 0

    def countedConnect():
        """Ledger 연결 횟수를 세고 실제 연결을 반환한다."""

        nonlocal calls
        calls += 1
        return originalConnect()

    monkeypatch.setattr(store, "_connect", countedConnect)
    ...
    calls = 0
    assert store.readReadyHandle(exactPins) == handle
    assert calls == 2


def testReceiptPagesReadTerminalManifestOncePerProcess(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    store = MaterializationStore(tmp_path / "materialization", policy=policy())
    exactPins = pins(query="manifest-cache")
    _, terminalRoot = build(
        store,
        exactPins,
        tuple(draft(index) for index in range(8)),
    )
    receipt = MaterializationReceipt(
        generationKey=generationKey(exactPins),
        terminalRootDigest=terminalRoot,
    ...
    originalRead = store.cas.readBytes
    manifestReads = 0

    def countedRead(digest: str, *args: Any, **kwargs: Any) -> bytes:
        """Terminal manifest CAS read만 별도로 센다."""

        nonlocal manifestReads
        if digest == terminalRoot:
            manifestReads += 1
        return originalRead(digest, *args, **kwargs)

    monkeypatch.setattr(store.cas, "readBytes", countedRead)
    for ordinal in range(8):
        handle, page = store.readReceiptPage(receipt, ordinal)
        assert handle.receipt == receipt
        assert page.ordinal == ordinal
    assert manifestReads == 1


def testReaderCompletionRejectsLeaseExpiredDuringManifestRead(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    clock = FakeClock()
    store = MaterializationStore(
        tmp_path / "materialization",
        policy=policy(readerLeaseSeconds=1.0),
        clock=clock,
    )
    exactPins = pins(query="reader-completion-expiry")
    build(store, exactPins, (draft(1),))
    originalRead = store.cas.readBytes

    def expiringRead(*args: Any, **kwargs: Any) -> bytes:
        """Manifest를 읽은 직후 reader lease를 만료시킨다."""

        payload = originalRead(*args, **kwargs)
        clock.advance(2.0)
        return payload

    monkeypatch.setattr(store.cas, "readBytes", expiringRead)
    with pytest.raises(MaterializationError) as expired:
        store.readReadyHandle(exactPins)
    assert expired.value.code == "MATERIALIZATION_NOT_READY"
...
def testProductionStoreReusesRootAndTimeoutIdentity(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from dartlab.dataHub.materialization.runtime import (
        _cachedMaterializationStore,
        materializationStore,
    )

    monkeypatch.setenv("DARTLAB_HOME", str(tmp_path))
    _cachedMaterializationStore.cache_clear()
    try:
        first = materializationStore()
        assert materializationStore() is first
        assert materializationStore(pageTimeoutMs=1_000) is not first
    finally:
        _cachedMaterializationStore.cache_clear()
