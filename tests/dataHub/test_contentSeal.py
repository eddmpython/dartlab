...
def testFactorResultIdentityBindsReturnedValues(monkeypatch) -> None:
    ...
    first = dartlab.dataHub("query", "scan.ratio", query=_factorQuery())
    repeated = dartlab.dataHub("query", "scan.ratio", query=_factorQuery())
    current["value"] = 13.0
    changed = dartlab.dataHub("query", "scan.ratio", query=_factorQuery())
