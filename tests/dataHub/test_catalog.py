...
def testCatalogCoversEveryCurrentLowerLayerRegistryAndResource():
    ...
    result = dartlab.dataHub("catalog")
    ...
    assert featureMetadata["pageMaxEntities"] == 64
...
def testCatalogFiltersWithoutExecutingAsset(monkeypatch):
    ...
    result = dartlab.dataHub("catalog", query=CatalogQuery(owners=("scan",), search="ratio"))
...
def testCatalogSnapshotIsDeterministic():
    ...
    first = dartlab.dataHub("catalog")
    second = dartlab.dataHub("catalog")
...
def testEveryQueryableAssetHasResolvableExecutor():
    ...
    assets = [asset for asset in dartlab.dataHub("catalog").assets if asset.queryable]
...
def testPrivateResourceIsCataloguedButNotQueryable():
    ...
    result = dartlab.dataHub("catalog", query=CatalogQuery(search="forwardTests"))
...
def testDartAndEdgarResourcesDeclareTheirUniverseMarkets():
    ...
    assets = {asset.assetId: asset for asset in dartlab.dataHub("catalog").assets}
...
def testDeprecatedRegistryAxisIsCataloguedButNotQueryable():
    ...
    asset = next(item for item in dartlab.dataHub("catalog").assets if item.assetId == "gather.calendar")
    assert asset.hidden
    assert not asset.queryable
