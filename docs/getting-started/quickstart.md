...
property 하나로 바로 DataFrame을 받는다. `c.BS`, `c.IS`, `c.CF` 세 가지다.
...
모두 Polars DataFrame이며, 행은 계정항목, 열은 연도다. Bridge Matching으로 계정명 변경을 자동 추적해서 시계열 연속성을 보장한다.

### 분기별 / 반기별

`fsSummary()` 메서드는 기간 파라미터를 지원한다.
...
재무제표 주석 데이터는 `c.notes`로 통합 접근한다. 12개 항목을 지원한다.
