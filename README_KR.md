...
13축 시장 횡단분석. `dartlab.scan()` 하나로 전부 접근한다.
...
`gather()` 하나로 모든 외부 시장 데이터를 **Polars DataFrame**으로 수집한다. 모든 요청은 자동 fallback 체인, circuit breaker 격리, TTL 캐시를 거친다. 동기 시그니처 — 내부적으로 async 병렬 실행.

```python
import dartlab

dartlab.gather()                                # 가이드 -- 4축 + 설명

# OHLCV 시계열 — 수정주가, 1회 요청으로 6000거래일+
dartlab.gather("price", "005930")               # KR: 기본 1년
dartlab.gather("price", "005930", start="2015-01-01")  # 기간 지정
dartlab.gather("price", "AAPL", market="US")    # US: Yahoo Finance chart API

# 수급 시계열 (KR 전용)
dartlab.gather("flow", "005930")                # DataFrame (date, foreignNet, institutionNet, ...)

# 거시 지표 — 전체 wide DataFrame
dartlab.gather("macro")                         # KR 12개 지표 (CPI, 금리, 환율, 생산, ...)
dartlab.gather("macro", market="US")            # US 25개 지표 (GDP, CPI, 연방기금, S&P500, ...)
dartlab.gather("macro", "CPI")                  # 단일 지표 (자동 KR 감지)
dartlab.gather("macro", "FEDFUNDS")             # 단일 지표 (자동 US 감지)

# 뉴스
dartlab.gather("news", "삼성전자")               # Google News RSS → DataFrame

# Company 바인딩 -- 종목코드 불필요
c = dartlab.Company("005930")
c.gather("price")                               # gather("price", "005930")과 동일
```
...
| 네이버 모바일 API | 수급, 업종 PER | `m.stock.naver.com/api` — JSON 엔드포인트 |
...
# 뉴스 수집
c.gather("news")                      # 최근 30일
dartlab.gather("news", "AAPL", market="US")  # 미국 기업 뉴스
