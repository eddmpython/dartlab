...
공시에서 **숫자(재무제표)**와 **텍스트(사업보고서)**를 함께 추출하고, 회사 facade, CLI 워크플로우, AI 웹 인터페이스로 제공한다. 현재 공개 사용 흐름은 `sections → show → trace`다.
...
둘 다 같은 `Company` 인터페이스를 반환하고, 같은 `sections → show → trace` 흐름으로 동작한다.
...
# EDGAR도 같은 흐름
us = dartlab.Company("AAPL")
us.sections          # 같은 company spine 인터페이스
us.topics            # 사용 가능한 topic 목록
us.show("BS")        # SEC XBRL 재무제표
us.show("10-K::item1Business")  # 10-K 서술형 섹션
```
...
- [x] Company property 기반 접근 + Notes 통합
