...
DartLab은 [DART 전자공시](https://dart.fss.or.kr/)를 파싱하고 분석하는 Python 패키지다. 현재 안정화의 중심은 DART 기업 분석이며, 그 위에 `dartlab.Company(...)` facade, CLI, AI 웹 인터페이스를 얹는 구조로 가고 있다.

지금의 제품 메시지는 명확하다. **DART core가 안정 기본값이고, EDGAR와 mixed-market 경로는 확장 단계**다. DartLab은 공시에서 **숫자(재무제표)**와 **텍스트(사업보고서)**를 함께 추출하고, 이를 비교 가능한 표와 회사 facade, CLI 워크플로우로 제공한다.
...
# EDGAR 경로도 같은 facade로 받을 수 있지만, 안정 기본값은 DART다
...
`profile`은 일부러 뒤로 미뤘다. 앞으로는 터미널과 노트북에서 문서처럼 읽히고, 변화 지점을 먼저 보여주는 회사 보고서 뷰로 만들 예정이다.
...
- [ ] EDGAR Company UX를 DART Company 수준으로 정렬
