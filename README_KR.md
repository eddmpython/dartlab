...
내부 동작: 일자별 공시 목록 → 원문 파싱 (99.4% 성공) → ngram 인덱스 + 동의어 확장. 모델 로딩 불필요, GPU 불필요. 검색 지연: **~1ms**, 정밀도 **95%**.
...
질문하면 AI가 dartlab의 전체 API를 사용해 Python 코드를 작성하고 실행한다. 사용자는 AI가 실행하는 모든 코드와 결과를 볼 수 있다. 단순히 답을 얻는 게 아니라, 기업을 분석하는 방법 자체를 배울 수 있다. AI는 `analysis()`, `scan()`, `gather()`, `review()` 등 어떤 함수든 조합해서 질문이 요구하는 흐름을 스스로 만든다.
