...
원본 재무제표는 숫자다. 매출이 302조원이라는 사실만으로는 이 기업이 건강한지, 성장 중인지, 위험한지 알 수 없다. Analysis가 이 간극을 메운다.
...
Analysis는 구조화된 데이터를 만든다. Review는 이걸 사람이 읽을 수 있는 보고서로 조립한다. 각 analysis 축이 테이블, 플래그, 서사와 함께 보고서의 한 섹션이 된다.
