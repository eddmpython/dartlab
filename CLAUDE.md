...
이 파일은 모든 에이전트가 공통으로 읽는 작업 진입점이다. 전체 설계와 운영 계약을 복제하지 않고,
정본의 위치와 작업 전에 반드시 확인할 안전 경계만 적는다.
...
- Windows Python 명령은 UTF-8로 실행한다. 기본 형식은 `uv run python -X utf8 ...`다.
...
- `pytest tests/`를 직접 실행하지 않는다. 전체 검증 진입점은
...
- 실패하면 대상 코드와 검사 환경을 각각 확인한다. 기존 실패와 이번 변경의 회귀를 구분한다.
