...
| dataSyncDaily | `daily-data-sync` | true |

## 수집 엔진

- `ZipDocsCollector`: ZIP(document.xml) 기반 (빠름, 권장)
- `batchCollect()` / `batchCollectAll()`: 비동기 멀티키 병렬
- `_collectDocsDirect()`: dataSyncDaily docs 전용 경로 — listing API 재조회 없이 이미 발견한 rcept_no의 ZIP을 직접 다운로드+파싱 (syncRecent.py)

## 3-Layer Freshness

`loadData()`나 `Company("005930")` 호출 시 로컬 데이터 최신성을 자동 확인.
