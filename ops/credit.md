# Credit — dartlab 독립 신용평가 체계

dartlab은 공시 데이터만으로 재현 가능한 독립 신용평가를 수행한다.
...
| 소비 | Company 전체, analysis 14축, scan, notes, gather, sections |
...
[Layer 2] 7축 정량 스코어링
    ...
    │
    ▼
[Layer 3] 업종 조정 + 시계열 안정화
    │
    ├── 업종별 기준표 적용 (IndustryGroup 세분화)
    ├── 3개년 가중이동평균 (등급 급변동 방지)
    └── 캡티브 금융 / 지주사 구조 감지 + 표기
    │
    ▼
[Layer 4] 등급 결정 + 보고서 생성
    │
    ├── 종합 점수 → dCR-등급 매핑
    ├── 등급 보고서 생성 (모든 축 상세 포함)
    ├── 신평사 등급 대조 (동의/비동의 + 근거)
...
- 이유: 신용평가의 지표 정의와 계산 방식은 신용평가 맥락에 최적화되어야 한다
...
- company.finance.ratios의 부실 모델 점수(Z-Score, O-Score 등)는 예외적으로 참조 허용
  - 이유: 이미 L0에서 검증된 계산이고, 신용평가만의 특수 해석이 필요 없는 순수 수치

## 규칙 2: 신용평가 특화 기능

기존 엔진의 좋은 것을 참고하되, 신용평가 맥락에 맞게 **재구현**한다.
...
| 섹션 | 내용 | 데이터 소스 |
|------|------|-----------|
| 1. 등급 요약 | 등급, 점수, PD, eCR, 전망, 업종 | engine.py |
| 2. 기업 개요 | 업종, 주요사업, 부문구성, 시장지위 | calcCompanyProfile + segments + rank |
| 3. 재무 하이라이트 | 매출/이익/EBITDA 전년비 + 추세 + 차입금 구성 | metricsHistory + narrative |
| 4. 등급 근거 | 강점/약점/양호 — narrative.py 서사 | buildNarratives |
| 5. 7축 상세 | 축별 서사 + 지표 테이블 | 7축 narrative + scoreMetric |
| 6. 재무 요약 5개년 | 핵심 지표 시계열 (무차입 표시) | metricsHistory |
| 7. 등급 전망 | 상향/하향 트리거 자동 생성 | 조건부 로직 |
| 8. 신평사 대조 | 동의/비동의 + notch 차이 | audit.py + external_grades.json |
| 9. 면책 | 방법론 버전 + 면책 사항 | 정적 |
