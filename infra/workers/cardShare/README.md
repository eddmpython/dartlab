...
## 왜 워커인가 (정공법)
캐러셀 런타임 뷰는 hfMedia(`manifests/carousels.json`, 라이브 발행)다. landing 정적 빌드엔 캐러셀 데이터가 없다.
...
## 한 번 배포 → 영구
워커 1회 deploy + landing env 1회 설정 후, 새 캐러셀은 데이터(manifests/carousels.json)만 올리면
