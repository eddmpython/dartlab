<script lang="ts">
	...
	let notReady = $state(false); // 리포트 null = 아직 미베이크(다음 동기화) 또는 실패. "준비중" 정직 표시.
	let seq = 0; // in-flight 토큰 · stale 응답 폐기

	rt.ipo.recent().then((f) => {
		filings = f;
		listBusy = false;
		if (f[0]) pickIssuer(f[0]); // 첫 발행사 자동 선택(퍼블릭·로컬 모두 리포트 로드)
	});

	// 퍼블릭·로컬 공통: 리포트는 왓치가 구운 HF(dart/ipo/reports.parquet)를 직독(로컬은 미베이크만 /api).
	// null(미베이크·실패)이면 "준비중" + DART 원문 링크로 정직 표시(옛 "로컬 설치" 안내 폐기).
	function pickIssuer(f: IpoFiling): void {
		...
		rt.ipo
			...
			.then((r) => {
				if (my !== seq) return;
				report = r && r.sections.length ? r : null;
				notReady = !report;
			})
			.catch(() => {
				if (my !== seq) return;
		...
		<div class="ipoBody">
			...
			<div class="ipoDetail">
				...
				{:else}
					...
					{#if reportBusy}
						<div class="ipoState"><span class="ipoSpin">◴</span>{lang === 'en' ? 'Loading report…' : '리포트 불러오는 중…'}</div>
					{:else if notReady || !report}
						<!-- 미베이크(다음 동기화 전) 또는 실패 · "준비중" + DART 원문 링크(정직 floor). -->
						<div class="ipoPublicNote">
							<p>
								{lang === 'en'
									? 'The 6-category offering report is being prepared and will appear after the next sync. Meanwhile, read the filing directly on DART.'
									: '6카테고리 공모분석 리포트는 준비 중이며 다음 동기화 후 표시됩니다. 그동안 아래 DART 원문에서 신고서를 직접 확인하세요.'}
							</p>
