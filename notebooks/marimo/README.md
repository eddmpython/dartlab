# Marimo Notebooks

Interactive [Marimo](https://marimo.io/) notebooks. Run locally or open in [Molab](https://molab.marimo.io/) (free cloud).

## Core Notebooks

| Notebook | What it covers |
|----------|---------------|
| **01_company.py** | Company -- sections, show, trace, diff, BS/IS/CF, ratios, EDGAR |
| **02_scan.py** | Scan -- scan() 13-axis market scan, 2,700+ companies |
| **03_review.py** | Review -- structured report + AI interpretation |
| **04_gather.py** | Gather -- price, flow, macro, news |
| **05_analysis.py** | Analysis -- financial, valuation, forecast |
| **06_ask.py** | Ask -- natural language analysis with LLM |

## Run locally

```bash
uv add dartlab marimo
marimo edit notebooks/marimo/01_company.py
marimo edit notebooks/marimo/02_scan.py
marimo edit notebooks/marimo/03_review.py
marimo edit notebooks/marimo/04_gather.py
marimo edit notebooks/marimo/05_analysis.py
marimo edit notebooks/marimo/06_ask.py
```

## Open in Molab (browser)

- [01_company.py](https://molab.marimo.io/github/eddmpython/dartlab/blob/master/notebooks/marimo/01_company.py)
- [02_scan.py](https://molab.marimo.io/github/eddmpython/dartlab/blob/master/notebooks/marimo/02_scan.py)
- [03_review.py](https://molab.marimo.io/github/eddmpython/dartlab/blob/master/notebooks/marimo/03_review.py)
- [04_gather.py](https://molab.marimo.io/github/eddmpython/dartlab/blob/master/notebooks/marimo/04_gather.py)
- [05_analysis.py](https://molab.marimo.io/github/eddmpython/dartlab/blob/master/notebooks/marimo/05_analysis.py)
- [06_ask.py](https://molab.marimo.io/github/eddmpython/dartlab/blob/master/notebooks/marimo/06_ask.py)

## Samples

`samples/` contains internal check notebooks (dartCompany, edgarCompany, marketScan, aiAnalysis, dartReview, sampleReview).
