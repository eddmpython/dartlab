...
- **공통 파이프라인**: 기획이 글과 이미지를 동시에 결정한다. `brief.json` 또는 동등한 기획 산출물에는 관통선, 핵심 인싸이트, evidenceMap, DART/EDGAR·dartlab·price·macro·internal-blog 근거, 보여줄 표·차트·이미지, 이미지 프롬프트, 참고글 연결 계획, 오독 방지 조건이 같이 있어야 한다.
...
- 산출: **독자질문 1(관통선)** + **핵심 인싸이트 1(그 질문의 답)** + 막 구조표 + 막별 테이블 + 제목/description 후보 + **막별 비주얼 기획**(고정 템플릿 아님. 이야기가 요구하는 차트·표·카드를 막마다 정한다: 부문 믹스=도넛, OPM 궤적=라인, peer=바, 수주 runway=런웨이 차트. 카드뉴스 `imagePlan`처럼 블로그 차트도 스토리가 정한다). 각 비주얼은 `placement`, `insertAfter`, `narrativeUse` 로 본문 중간 삽입 위치와 독자 이해 역할까지 결정한다.
