...
- **공통 파이프라인**: 기획이 글과 이미지를 동시에 결정한다. `brief.json` 또는 동등한 기획 산출물에는 관통선, 핵심 인싸이트, evidenceMap, DART/EDGAR 근거, 보여줄 표·차트·이미지, 이미지 프롬프트, 오독 방지 조건이 같이 있어야 한다.
