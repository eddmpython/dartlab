...
- **천 단위 콤마**: 9개 카테고리 모두 독자에게 수량으로 보이는 정수부 4자리 이상 숫자를 `1,000`, `12,345.67`처럼 쓴다. 제목, description, 본문, 표, 캡션, 인용문, 검증표가 같은 규칙을 따른다. 연도·날짜·종목코드·버전·모델명·URL·코드·해상도 같은 식별자는 예외다. 기획 워크플로가 먼저 요구하고 `publishGate.py`가 최종 하드 실패시킨다.
- **장르 델타의 한계**: 아래 장르별 규칙과 파생 `PIPELINE.md`는 주어, 근거, 분량, 시각물만 바꿀 수 있다. 내러티브 집중, 쉬운 설명, 천 단위 콤마는 어떤 장르도 덮어쓸 수 없는 공통 계약이다.
...
- **강제 게이트**: `blogMediaGate.py --staged`가 커밋 대상에서 포스트 내부의 추적 이미지와 SVG, 로컬 렌더링 참조, 카탈로그 밖 HF 객체를 모두 차단한다. push 훅과 원격 `Blog Media SSOT` CI는 `--ref HEAD`로 전체 블로그를 재검사한다. 작업자가 발행 순서를 잊어도 커밋과 배포가 실패해야 정상이다.
