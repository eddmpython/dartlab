...
- **Git 계약**: `brief.json.imagePlan[]`이 의미를, `assets/CREDITS.md`가 출처를, 중앙 `media/catalog.json` 하나가 staging 별칭·포스트별 본문/OG/card 역할·HF 객체 SHA-256 대응을 가진다. 본문 `ogImage`와 `cardPreview`는 카탈로그에서 파생한 HF URL만 쓴다. SVG는 사람이 diff로 검토 가능한 텍스트 차트라 Git 추적 예외다.
