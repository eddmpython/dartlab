...
건설업 매출은 진행률 추정에 의존한다. 진행률을 낙관적으로 잡으면 매출이 앞당겨지고, 나중에 추가원가가 잡히면 한꺼번에 매출이 줄거나 손실이 터진다. 그래서 매출 성장률보다 `매출 → 청구 → 현금 회수`의 연결 고리를 먼저 확인해야 한다. [매출 인식 시점 변경은 어디가 신호인가](/blog/revenue-recognition-timing-signals)에서 이 문제를 더 깊게 다뤘다.
