...
이 글은 재고자산과 평가손실을 통해 **수요의 질과 이익의 질**을 읽는 방법을 정리한다. 재고 증가가 좋은 경우와 위험한 경우, 재고 구성표에서 봐야 할 것, 회전율과 매출총이익률의 관계, 평가손실이 진짜 경고가 되는 순간을 실전 기준으로 설명한다. 운전자본 전체 감각이 필요하면 [`매출채권과 대손충당금 읽는 법`](/blog/receivables-and-allowance), 생산과 투자 쪽 맥락이 필요하면 [`사업보고서에서 건설중인자산 읽는 법`](/blog/construction-in-progress)을 같이 보면 좋다.
...
이 지점은 [`매출은 느는데 왜 위험할 수 있나`](/blog/why-rising-sales-can-still-be-risky)와도 연결된다. 재고는 결국 수요, 가격, 현금흐름 문제를 함께 드러내기 때문이다.
...
특히 재고와 매출채권이 동시에 쌓이면 성장의 질이 더 약해질 가능성이 크다. 그래서 재고를 읽은 뒤 [`매출채권과 대손충당금 읽는 법`](/blog/receivables-and-allowance)으로 넘어가면, 단순한 재고 문제가 아니라 전체 운전자본 문제인지 더 선명해진다.
