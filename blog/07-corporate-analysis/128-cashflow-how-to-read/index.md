...
![현금흐름 분석의 세 축: CF 3구간, FCF, 이익의 현금 뒷받침을 한 화면에 압축한 도식](./assets/hero-cashflow.svg)
...
| 구간 | 의미 | 양수일 때 | 음수일 때 |
|------|------|-----------|-----------|
| **영업활동(OCF)** | 본업에서 번 현금 | 사업이 현금을 만든다 | 본업에서 현금이 나가고 있다 |
| **투자활동(ICF)** | 설비/자산에 쓴 현금 | 자산 매각 또는 투자 회수 | 설비 투자, 자산 취득 |
| **재무활동(FCF)** | 차입/상환/배당 | 외부에서 자금 유입 (차입, 유증) | 부채 상환, 배당 지급 |
...
위 명령 한 줄이면 CF 3구간 시계열, FCF 추이, 영업CF 마진, 이익의 현금 뒷받침 비율, CF 패턴 판정까지 한 번에 나온다.
...
자유현금흐름(Free Cash Flow)은 영업CF에서 CAPEX를 뺀 것이다.
...
![FCF의 의미와 해석 — 영업CF에서 CAPEX를 빼고 남는 진짜 여유 현금](./assets/fcf-deficit.svg)

- **FCF 양수**: 본업으로 투자를 감당하고도 돈이 남는다. 이 돈으로 배당, 자사주 매입, 부채 상환이 가능하다
- **FCF 음수**: 본업 현금으로 투자를 감당하지 못한다. 차입이나 유상증자로 부족분을 메워야 한다
- **FCF 음수가 오래 지속**: 외부 자금 의존도가 누적된다. 차입 한도에 가까워지면 위험해진다

FCF 음수가 반드시 나쁜 것은 아니다. 아마존도 수년간 FCF 적자였고, 그 투자가 현재의 사업을 만들었다. 문제는 **기간**과 **추세**다. 3년 이상 FCF 적자가 지속되면서 영업CF 자체도 줄어들고 있다면, CAPEX를 줄이지 않는 한 FCF가 개선될 경로가 보이지 않는다.

예를 들어 삼성SDI(006400)는 2021년부터 2025년까지 5년 연속 FCF 적자다. 배터리 공장에 매년 수천억~수조 원을 투자했는데, 영업CF가 이를 감당하지 못했다. 2025년에 CAPEX를 전년 대비 70% 줄였지만 영업CF도 함께 줄어서 FCF는 여전히 적자다.
...
| **음수 (OCF+, NI-)** | 순이익 적자지만 영업CF 양수. 감가상각비 같은 비현금 비용이 크다 |
| **음수 (OCF-, NI+)** | 흑자인데 현금이 나간다. 운전자본이 급증하거나 일시적 요인 |
...
영업CF 마진이 10% 미만이면 현금 창출력이 약해진 것이다. 특히 CAPEX가 큰 산업에서 영업CF 마진이 낮으면, FCF가 구조적으로 적자가 될 수밖에 없다.
...
2. **FCF 적자 장기화**: 3년 이상 FCF 적자가 지속되면 외부 자금 의존도가 누적된다. [자금 구조](/blog/funding-structure-how-to-read)에서 차입 비중이 함께 올라가고 있는지 교차 확인
...
이런 신호들은 단독으로 보면 안 된다. 현금흐름은 [수익 구조](/blog/revenue-structure-how-to-read), [자금 구조](/blog/funding-structure-how-to-read), [자산 구조](/blog/asset-structure-how-to-read)와 함께 봐야 완성된다. 영업CF가 줄어드는 원인이 매출 자체의 문제인지(수익 구조), 운전자본 효율의 문제인지(자산 구조), 아니면 일시적 요인인지를 구분해야 한다.
...
1. **CF 3구간 + FCF**: 영업CF, 투자CF, 재무CF, CAPEX, FCF를 5년 시계열로. 각 연도의 CF 패턴(성숙형/확장형/구조조정형 등) 판정 포함
2. **이익의 현금 뒷받침**: 영업CF, 순이익, 매출, 영업CF/순이익 비율, 영업CF 마진을 5년 시계열로
3. **현금흐름 경고 신호**: 영업CF 적자, FCF 적자, 위기형 패턴, 3년 연속 감소 등 자동 감지
...
**Q. FCF 적자가 몇 년이면 심각한 건가?**

일률적인 기준은 없다. 성장기 기업에서 2~3년 FCF 적자는 흔하다. 핵심은 영업CF가 개선되고 있는지, CAPEX 투자가 매출 성장으로 이어지고 있는지다. 5년 이상 FCF 적자가 지속되면서 영업CF도 감소 추세라면, 외부 자금 의존이 구조화된 것이므로 주의해야 한다. 차입 한도, 이자보상배율, 유동비율을 함께 확인한다.
...
확장형(+/-/+)은 외부 자금을 동원해 적극 투자하는 패턴이다. 성장기에 자연스럽지만, 오래 지속되면 부채가 누적된다. 확장형이 3년 이상 계속되면 차입 비중, 이자보상배율, FCF 추이를 반드시 확인한다. 투자한 자산이 매출로 전환되기 시작하면 성숙형으로 이동해야 정상이다.
