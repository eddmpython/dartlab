...
방금 나온 표에서 왼쪽은 계정이고 오른쪽은 기간이다. 자산, 부채, 자본 같은 항목이 기간별 숫자로 펼쳐진다. 사람이 손으로 옮겨 적던 재무상태표가 여기서는 바로 다룰 수 있는 표가 된다.
...
이 한 줄이 이천 개가 넘는 회사의 공시를 한 번에 훑고, 앞쪽 몇 회사를 실제 표로 보여 준다. 브라우저에서도 돈다. 다만 브라우저에서는 자주 쓰는 계정만 담은 경량본을 내려받기 때문에, 서버에서 돌릴 때보다 다룰 수 있는 항목이 적다.
...
말로만 하면 와닿지 않는다. 손익계산서 격자를 직접 열어 본다.
...
`panel`은 격자를 그대로 연다. 앞의 `"IS"`가 손익계산서이고, `freq="Y"`가 연간 기준이다. 연간이 아니라 분기로 보고 싶으면 `freq="Q"`로 바꾼다. 나중에 특정 항목 한 줄만 좁혀 보고 싶을 때 `select`를 쓰지만, 첫 감각은 항상 전체 panel에서 잡는다.

여기까지 왔다면 여러분은 이미 dartlab의 핵심을 한 번씩 만졌다. 회사를 잡았고, 격자 표를 실제로 봤고, 시장 전체 표를 훑었고, 기간 기준을 바꿔 볼 준비를 했다. 이 네 가지가 나머지 모든 기능의 바닥이다.
