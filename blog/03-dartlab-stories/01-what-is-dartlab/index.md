...
date: "2026-07-10"
description: "DART 공시와 EDGAR 재무 데이터를 브라우저에서 바로 열고, 회사 격자와 시장 스캔을 실행해 설치 없는 공시분석 흐름과 데이터 구조를 코드로 확인한다."
...
이번 편에서는 DART 공시분석을 설치 없이 시작하고, 데이터가 어떤 모양이며, 그것이 어떻게 갱신되는지를 훑었다. 다음 편 [재무제표, 파이썬 한 줄](/blog/call-financial-statements)은 손을 더 깊이 넣는다. **재무제표를 실제로 불러오고, 계정과 기간 값을 확인하고, 그것을 표로 읽는 법**이다.
