...
9. **이미지 = 기획이 정한 만큼.** 고정 하한을 두지 않는다. Phase 1 기획 루프가 `imagePlan` 에 그 편에
...
10. **길이 = 밀도 우선.** 하한 3,000자, 목표 5,000자. 코드·표·SVG 를 뺀 읽는 글자수 기준이다.
   설명이 코드보다 길 필요는 없지만, 코드만 던지고 왜를 안 적으면 그건 문서지 이야기가 아니다.
11. **주어가 회사가 아니다.** `topicSlug` 를 쓰고 `stockCode` 는 달지 않는다. 예제 회사는 예제일 뿐이다.
12. **썸네일.** `gen_blog_thumbnails.py`(SSOT). kicker 라벨 = `PREFIX["dartlab-stories"] = "dartlab 이야기"`.
...
13. **누적이 곧 커리큘럼.** 편 번호가 학습 순서다. 앞 편이 세운 개념을 뒤 편이 딛는다. 새 편을 끼워
