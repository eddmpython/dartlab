...
이번 편에서 배울 것은 조금 다르다. 코드셀은 따로따로 도는 계산기가 아니다. 위 셀에서 만든 `c`와 `is_`를 아래 셀이 그대로 쓴다. 그래서 순서가 중요하다. 회사를 먼저 만들고, 그 회사로 손익계산서를 열고, 그 표를 다시 보는 순서다.

![브라우저 안에서 이어지는 코드셀 작업대](./assets/dartlab-story-03-codecell.webp)

이걸 모르면 셀을 누르는 순서가 꼬였을 때 당황한다. 어떤 칸은 잘 되는데 어떤 칸은 이름을 모른다고 한다. 대부분은 데이터 문제가 아니라 **위 셀을 아직 실행하지 않은 문제**다.
...
이 행동 검사가 끝나면 한 가지는 확실히 알게 된다. 위에서 아래로 실행하고, 바꾼 곳부터 다시 실행한다. 이 두 문장만 기억해도 초보 단계의 절반은 지나온 것이다.
...
이제 여러분은 코드셀을 누르는 법이 아니라, 코드셀이 어떻게 이어지는지 알게 됐다. 다음 편은 회사를 고르는 일이다. 회사명으로 생각하고, 목록에서 종목코드를 확인하고, 그 코드로 `Company`를 여는 순서를 익힌다.

다음 편 [DART 회사 찾고 열기](/blog/pick-company-by-code)은 그 질문에 답한다. 회사명, 종목코드, `Company` 사이를 분명히 나누고, 코드 하나를 바꿔 다른 회사의 재무제표를 여는 습관을 만든다.
