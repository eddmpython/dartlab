...
## 본문 셀에서 고치고 전체 화면으로 이어 간다

본문의 코드셀은 읽는 사람 모두에게 같은 예제로 시작하지만, 셀 안에서 바로 회사 코드와 계산식을 바꿀 수 있다. 코드를 고친 뒤 왼쪽 재생 아이콘이나 `Ctrl+Enter`를 누르면 바뀐 코드가 실행된다.

첫 셀 오른쪽 위의 "전체 화면"을 누르면 지금 글의 코드셀과 설명이 여러분의 노트북 한 권으로 열린다. 거기서는 셀을 더하고, 삭제하고, 순서를 바꾸고, 결과를 저장할 수 있다. 다만 위에서 만든 이름을 아래에서 쓴다는 규칙은 그대로다.
