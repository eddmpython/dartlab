...
annual = c.select("IS", ["매출액", "영업이익"], freq="Y")
annual
```

`IS`는 손익계산서, `["매출액", "영업이익"]`은 보고 싶은 계정, `freq="Y"`는 연간이다. 현재 브라우저 데이터 기준으로 이 결과는 두 계정과 13개 연간 열을 가진 표로 열린다. 실제 화면에서는 공시 갱신에 따라 열 수가 나중에 달라질 수 있다.

dartlab이 돌려주는 표는 파이썬에서 다루기 좋은 DataFrame 형태다. 표 조작 자체가 궁금하면 [Polars 공식 문서](https://docs.pola.rs/)를 참고할 수 있다. 이 글에서는 Polars 전체 문법보다 `shape`와 `head`처럼 기간 차이를 확인하는 데 필요한 손잡이만 쓴다.

같은 계정을 분기로 바꿔 보자.

```python
quarterly = c.select("IS", ["매출액", "영업이익"], freq="Q")
quarterly
```

이 표는 훨씬 넓다. 같은 회사, 같은 계정인데 열이 더 많다. 연간 13칸 정도가 아니라 분기 43칸 정도가 나온다. 1년에 하나씩 보던 것을 1년에 네 번씩 보기 때문이다.

```python
annual.shape, quarterly.shape
```

이 셀이 오늘의 핵심이다. 같은 계정을 골라도 `Y`와 `Q`는 표의 폭을 바꾼다. 폭이 바뀌면 읽는 방식도 바뀐다.
...
```python
bs_y = c.select("BS", ["자산총계", "자본총계"], freq="Y")
bs_q = c.select("BS", ["자산총계", "자본총계"], freq="Q")

bs_y.shape, bs_q.shape
```
...
```python
c.select("IS", ["매출액"], freq="Q")
```

두 셀을 이어서 실행하면 같은 계정의 두 얼굴을 볼 수 있다. 하나는 긴 호흡, 하나는 짧은 호흡이다.
...
```python
sales_y = c.select("IS", ["매출액"], freq="Y")
sales_q = c.select("IS", ["매출액"], freq="Q")

sales_y.shape, sales_q.shape
```

`sales_y`는 연간 매출, `sales_q`는 분기 매출이다. 이름만 봐도 기준이 보인다. 코드를 조금 더 길게 쓰는 대신, 표를 잘못 읽을 가능성이 줄어든다.
...
이번 편의 모든 코드는 브라우저에서 실행되는 공개 계약 안에 있다. `Company`, `panel`, `select`, 표의 `shape`와 `head`만 쓴다. 이 범위는 설치 없이 실습할 수 있다.
