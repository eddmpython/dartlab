...
결과는 `KR`과 `US`로 갈라진다. 같은 `Company` 입구로 들어왔지만 dartlab은 안쪽에서 한국 회사와 미국 회사를 구분한다. 국내 회사는 DART 공시 데이터로 가고, 미국 회사는 EDGAR 데이터로 간다.

여기서 중요한 것은 같음과 다름을 동시에 잡는 일이다. 같은 것은 **질문을 던지는 방식**이다. 다른 것은 **원천과 기준**이다. 이 둘을 섞으면 나중에 숫자를 바로 비교하는 실수를 한다.

![DART와 EDGAR를 같은 Company로 여는 흐름](./assets/dartlab-story-06-company-bridge.webp)

## 같은 질문을 던져 본다

[재무제표, 파이썬 한 줄](/blog/call-financial-statements)에서 `panel("IS")`를 배웠다. `IS`는 손익계산서다. 이번에는 같은 질문을 삼성전자와 Apple에 동시에 던져 보자.

```python
kr_is = kr.panel("IS")
us_is = us.panel("IS")

kr_is.shape, us_is.shape
```

브라우저 실행 검증 기준으로 삼성전자 손익계산서는 `(36, 43)`, Apple 손익계산서는 `(20, 67)` 형태로 열렸다. 공시 데이터가 갱신되면 숫자는 달라질 수 있다. 핵심은 정확한 행과 열 수를 외우는 것이 아니다. 같은 `panel("IS")` 질문이 두 시장에서 모두 표를 돌려준다는 점이다.
...
삼성전자 표는 `snakeId`, `항목`, 그리고 `2026Q1`, `2025Q4` 같은 기간 열로 열린다. `snakeId`는 여러 회사의 이름 흔들림을 줄이기 위한 표준 이름이고, `항목`은 사람이 읽는 계정명이다.
...
Apple 표도 왼쪽에 `snakeId`와 `항목`이 있고, 오른쪽으로 기간 열이 펼쳐진다. 그래서 우리는 같은 질문을 던질 수 있다. "매출액이 어디 있나?", "영업이익은 어떤 행인가?", "최근 기간 열은 어디인가?" 같은 질문이다.
...
도구가 같은 모양을 만들어 주면 사람은 안심한다. 왼쪽에는 계정, 오른쪽에는 기간. 겉으로는 비슷하다. 하지만 겉모양이 같다는 말과 회계 기준이 같다는 말은 다르다.
...
그래서 6편의 행동은 비교가 아니라 확인이다. 먼저 둘 다 열린다는 것을 확인하고, 그다음 원천을 되짚는다.
...
두 결과 모두 `finance`를 1차 원천으로 잡는다. 이것은 dartlab이 손익계산서를 공시 원문 산문에서 억지로 찾는 것이 아니라, 재무제표 쪽 원천을 우선해서 선택했다는 뜻이다.

여기서도 오해하면 안 된다. `trace("IS")`가 `finance`라고 해서 DART와 EDGAR의 모든 본문 섹션이 같은 방식으로 열린다는 뜻은 아니다. 이번 편은 재무제표 panel의 같은 표면만 확인한다. 미국 공시의 사업 설명, 위험 요인, 경영진단 문장을 DART 사업보고서 본문과 똑같이 읽는 문제는 다음 단계에서 확인해야 한다.
...
```python
kr.panel("BS").shape, us.panel("BS").shape, kr.panel("CF").shape, us.panel("CF").shape
```

브라우저 실행 검증 기준으로 삼성전자는 `BS (63, 43)`, `CF (65, 43)` 형태였고, Apple은 `BS (49, 71)`, `CF (46, 66)` 형태였다. 행 수와 열 수가 다르다. 당연하다. 회사가 다르고, 시장이 다르고, 공시 축적 기간도 다르다.

그런데 호출 모양은 같다. 이 사실이 중요하다. 독자는 새 도구를 처음부터 다시 배우지 않고, 같은 습관으로 질문을 확장할 수 있다.

1. 회사를 연다.
2. `panel("IS")`, `panel("BS")`, `panel("CF")`로 표를 연다.
3. `shape`로 크기를 본다.
4. `head`로 앞부분을 확인한다.
5. 필요하면 `trace`로 원천을 되짚는다.

이 다섯 단계는 국내 회사와 미국 회사에서 모두 출발점이 된다. 분석 결론은 아직 아니다. 출발점이다.
...
이 편에서 배우는 것은 순위가 아니다. 같은 질문을 던지는 법이다. "손익계산서를 열어라"라는 질문은 같지만, "두 매출을 같은 단위로 비교하라"는 질문은 아직 하지 않았다. 그 질문에는 통화 환산, 기간 정렬, 회계연도 확인이 필요하다.

그래서 좋은 노트북은 질문을 나눈다. 먼저 원천을 연다. 다음에 기간을 맞춘다. 그다음 단위를 맞춘다. 마지막에 비교한다. 이 순서를 건너뛰면 코드가 맞아도 문장이 틀린다.
...
이번 편의 코드는 브라우저에서 그대로 실행되는 공개 계약 안에 있다. `dartlab.Company`, `market`, `panel`, `trace`, 그리고 표의 `shape`와 `head`만 쓴다. 첫 실행은 파이썬과 데이터를 내려받느라 느릴 수 있다. 그 뒤에는 같은 커널에서 이어서 돈다.
...
두 번째 검사는 같은 질문 던지기다. `kr.panel("IS").shape`와 `us.panel("IS").shape`를 각각 확인한다. 행 수와 열 수가 다르다는 사실을 보고 놀라지 말아야 한다. 회사가 다르면 표의 크기도 다르다.
