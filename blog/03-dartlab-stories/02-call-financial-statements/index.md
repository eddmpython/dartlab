...
큰 표를 먼저 봤다면, 그다음에는 필요한 줄만 좁혀 볼 수 있다.
...
`panel`이 표 전체를 여는 명령이라면, `select`는 그중 몇 줄만 꺼내 보는 명령이다. 처음부터 `select`로 시작하지 말고, `panel`에서 어떤 줄이 있는지 본 다음 쓰는 편이 안전하다.
...
연간과 분기는 보는 칸이 다르다. 연간은 1년에 한 칸이고, 분기는 1년에 여러 칸이다. 매출이 몇 년 동안 늘었는지 볼 때와 최근 분기가 꺾였는지 볼 때는 서로 다른 표를 봐야 한다.
