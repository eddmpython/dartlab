---
title: "재무제표 세 장을 코드로 꺼내는 법"
date: "2026-07-10"
description: "dartlab으로 손익계산서, 재무상태표, 현금흐름표를 불러오고 원하는 계정만 골라내는 법을 브라우저에서 직접 실행하며 익힌다. 연간과 분기의 차이, 연결과 별도의 차이, 계정 이름이 회사마다 다른 문제, 빈칸의 의미까지 다룬다."
...
[지난 편](/blog/what-is-dartlab)에서 이 표를 한 번 열어 봤다. 이번 편은 그 표를 자유롭게 다루는 법이다. 아래 코드 칸은 전부 이 브라우저 안에서 그대로 돈다. 첫 코드 칸 아래의 "노트북 생성하기"를 누르면 이 글 전체가 여러분의 노트북이 된다.
...
앞 숫자가 그 회사가 손익계산서에 적은 계정의 개수, 뒤 숫자가 열의 개수다. 열에는 기간뿐 아니라 계정 이름 칸도 포함되어 있다.

여기서 돌려받는 표는 [polars](https://pola.rs/) DataFrame 이다. `shape`, `head`, `null_count` 같은 것이 전부 거기서 온다.

`shape`를 먼저 보는 습관이 중요한 이유가 있다. 회사마다 계정 수가 다르고, 상장한 지 얼마 안 된 회사는 기간이 짧다. 코드를 짜기 전에 상대의 크기를 확인하지 않으면, 없는 기간을 찾다가 빈손으로 끝난다.
...
```python
c.panel("BS", scope="separate").shape     # 별도
```

계정 수부터 다르다. 연결이 63개, 별도가 54개다. 자회사를 묶으면서 생기는 계정이 연결에만 있기 때문이다.

숫자는 더 크게 벌어진다. 자본총계를 두 방식으로 뽑아 보라.
...
```python
c.select("BS", ["자본총계"], freq="Y", scope="separate")
```
...
sk.select("IS", ["매출액", "영업이익"], freq="Y")
```

`005930`이 `000660`이 되었을 뿐인데 다른 회사의 5년치 손익이 나온다. 종목코드 하나로 회사를 지목하는 이유는 [회사 하나는 종목코드 하나](/blog/company-one-stock-code)에 정리해 두었다. 이것이 격자로 눕혀 둔다는 말의 실제 의미다. 회사가 바뀌어도 다루는 방법이 안 바뀐다.
