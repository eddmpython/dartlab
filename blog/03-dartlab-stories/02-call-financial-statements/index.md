...
dartlab은 이 문제를 다르게 푼다. 회사 하나의 공시를 미리 읽어서, 왼쪽에는 계정 이름, 오른쪽에는 기간별 숫자가 붙은 큰 표로 만들어 둔다. 손익계산서를 부르면 그 표의 손익 부분이 나온다. 두 해치만 보는 것이 아니라 저장된 기간을 한 번에 본다.

![계정과 기간이 만나는 격자](./assets/dartlab-story-02-grid-v2.webp)

[지난 편, DART 공시분석, 설치 없이](/blog/what-is-dartlab)에서 이 표를 한 번 열어 봤다. 이번 편은 손익계산서, 재무상태표, 현금흐름표를 직접 열어 보고, 화면에서 어디를 봐야 하는지 익히는 편이다. 아래 코드 칸은 전부 이 브라우저 안에서 그대로 돈다.
...
두 표를 나란히 보면 같은 회사인데도 줄과 숫자가 달라질 수 있다. 연결은 자회사까지 묶어서 보고, 별도는 그 법인 하나만 보기 때문이다.
