...
`005930`에서 0 하나만 빠지면 `5930`이 된다. 삼성전자가 아니라 전혀 다른 입력이다. 처음 회사를 열 때 가장 먼저 확인할 것은 종목코드다. 회사 이름은 흔들린다. 약칭이 있고, 계열사가 있고, 띄어쓰기가 제각각이다. 종목코드는 안 흔들린다. 그래서 분석 대상은 이름이 아니라 코드로 잡는다. 이 손잡이를 잘못 잡으면 뒤에서 아무리 멋진 표를 열어도 다른 회사를 보고 있게 된다.
...
아래는 로컬 터미널이나 로컬 Python에서 확인하는 흐름이다. 이 브라우저 글의 인라인 셀은 가벼운 공시 실습용이라 전체 거래소 목록 조회가 비어 있을 수 있다.
