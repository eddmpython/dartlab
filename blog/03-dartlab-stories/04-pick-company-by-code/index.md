...
title: "DART 회사 찾고 열기"
date: "2026-07-11"
description: "dartlab에서 회사 목록을 보고 종목코드를 찾은 뒤, 코드 또는 회사명으로 Company를 열어 같은 재무제표 표를 조회하는 흐름을 익힌다."
...
## 회사명, 코드, Company를 나눠 본다

투자자는 보통 회사 이름으로 생각한다. 삼성전자, SK하이닉스, NAVER처럼 말이다. 그런데 데이터 도구는 이름만 믿으면 헷갈릴 수 있다. 이름은 바뀔 수 있고, 띄어쓰기와 약칭이 흔들리며, 비슷한 이름의 계열사도 많다. 그래서 회사 이름과 종목코드를 따로 본다.

국내 상장사는 여섯 자리 종목코드를 쓴다. 삼성전자는 `005930`, SK하이닉스는 `000660`, NAVER는 `035420`이다. 이 번호를 넣으면 dartlab은 국내 회사로 보고 [DART](https://dart.fss.or.kr/) 공시 데이터로 간다.
...
둘째, 회사명 호출이 된다고 해서 확인을 건너뛰면 안 된다. 이름이 비슷한 회사, 지주회사와 사업회사, 보통주와 우선주가 섞이면 사람이 먼저 잘못 고를 수 있다.

셋째, 코드가 맞다고 분석이 끝나는 것은 아니다. 코드는 첫 단추일 뿐이다. 표를 열고, 기간을 고르고, 계정의 뜻을 확인해야 한다.
...
이 행동 검사를 통과하면 다음 편의 기간 실습이 쉬워진다. 회사가 정확히 고정되어 있어야 연간과 분기 차이를 볼 수 있다. 회사가 틀린 상태에서 기간만 바꾸면, 다른 회사의 표를 더 열심히 잘못 읽는 일이 된다.
...
또한 이번 편은 회사명 검색만 믿게 만들지 않는다. 로컬 설치본에서는 `listing`, `fuzzySearch`, `nameToCode`, `Company("삼성전자")`를 쓸 수 있다. 하지만 브라우저 실습에서는 확인된 코드를 넣어 같은 결과가 나오게 한다. 편한 입력과 정확한 확인은 같이 가야 한다.

마지막으로 코드를 투자 아이디어처럼 다루지 않는다. `005930`을 넣었다고 삼성전자 분석이 끝난 것이 아니다. 이제 겨우 올바른 회사를 연 것이다. 그다음에 기간을 정하고, 계정을 보고, 공시와 비교해야 한다.

## 다음 편에서 할 것

이제 회사를 정확히 여는 법을 배웠다. 다음은 표의 방향이다. 같은 회사라도 연간으로 보느냐, 분기로 보느냐에 따라 질문이 달라진다. 매출이 5년 동안 커졌는지 보는 질문과, 최근 두 분기가 꺾였는지 보는 질문은 다르다.

다음 편 [재무제표 기간, Y와 Q 조회](/blog/the-grid)에서는 `freq="Y"`와 `freq="Q"`를 비교한다. 같은 회사 표를 연간으로 볼 때와 분기로 볼 때 무엇이 달라지는지 직접 확인한다.
