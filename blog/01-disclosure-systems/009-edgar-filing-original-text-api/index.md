...
실전에서 exhibits를 다룰 때 가장 자주 겪는 문제는 "어떤 exhibit를 저장하고 어떤 걸 건너뛸지" 판단이다. filing index 페이지에 들어가면 exhibit 목록이 나오는데, 여기서 형식별 우선순위를 미리 정해 두면 불필요한 저장을 줄일 수 있다.

| exhibit 유형 | 실전 활용도 | 저장 우선순위 |
| --- | --- | --- |
| EX-99.1 (보도자료) | 실적 발표, 사건 요약 | 높음 |
| EX-10 (계약서) | 대규모 거래 조건 | 높음 (8-K와 함께) |
| EX-31/32 (인증서) | SOX 인증, 내부통제 확인 | 10-K/10-Q에서만 |
| EX-23 (감사 동의서) | 감사법인 확인 | 낮음 |
| EX-21 (종속회사 목록) | 연결 범위 파악 | 연 1회 10-K에서만 |

모든 exhibit를 전부 받으면 저장 비용이 빠르게 늘어난다. 목적에 따라 form별로 exhibit 수집 범위를 다르게 설정하는 것이 현실적이다. 예를 들어 8-K에서는 EX-99.1과 EX-10만, 10-K에서는 EX-21과 EX-31/32만 받는 식이다.
...
### submissions JSON의 filings.recent와 filings.files는 정확히 어떻게 다른가

`filings.recent`는 가장 최근 filing들을 배열로 바로 담고 있다. SEC 기준으로 최소 최근 1년치 또는 1,000건 이상이 들어간다. `filings.files`는 그보다 오래된 filing이 있을 때 추가 JSON 파일 경로를 배열로 알려준다. 실전에서는 recent만 읽고 끝내는 수집기가 많은데, 5년 이상 히스토리를 모으려면 files 배열의 각 경로를 순서대로 따라가서 같은 구조의 JSON을 추가로 받아야 한다.

### 같은 accession에 filing이 수정(amended) 되면 어떻게 되나

SEC에서는 수정본을 별도 accession으로 제출한다. 예를 들어 10-K/A는 원본 10-K와 다른 accession을 가진다. 따라서 submissions JSON에서 form 필드가 `10-K/A`, `10-Q/A`, `8-K/A`인 항목이 수정본이다. 수집기를 만들 때 원본과 수정본을 같은 테이블에 쌓되, form 필드로 구분하고 filing_date 기준으로 최신을 우선하는 로직을 넣어 두는 것이 안전하다.

### EDGAR에서 XBRL 태깅된 원문과 plain HTML 원문은 같은 파일인가

보통 다르다. 최근 filing은 iXBRL(인라인 XBRL)로 제출되어 HTML 안에 XBRL 태그가 내장된다. 하지만 오래된 filing은 XBRL과 HTML이 별도 파일이다. primaryDocument가 `.htm` 확장자면 대부분 iXBRL이 적용된 원문이고, 같은 디렉터리에 `.xml` 확장자 XBRL 인스턴스 문서가 따로 있을 수 있다. 숫자만 추출할 때는 companyfacts API가 더 효율적이고, 원문 맥락이 필요할 때만 HTML을 직접 파싱하는 것이 낫다.

## 같이 읽으면 좋은 글

- [EDGAR의 모든 것](/blog/everything-about-edgar)
