...
한국에서 공시 데이터를 프로그래밍으로 다루려면 반드시 거쳐야 하는 곳이 OpenDART다. DART 시스템 자체가 궁금하다면 [DART의 모든 것](/blog/everything-about-dart)을 먼저 읽는 것을 추천한다. 여기서는 실제로 OpenDART를 써보면서 느낀 장단점을 솔직하게 리뷰하겠다.

![DART Timeline](./assets/002-dart-timeline.svg)
...
![OpenDART API Categories](./assets/002-api-categories.svg)
...
![API Limit Simulation](./assets/002-api-limit-simulation.svg)
...
![OpenDART vs SEC EDGAR](./assets/002-opendart-vs-edgar.svg)
...
솔직히 API 인프라 측면에서는 EDGAR가 압도적이다 (EDGAR 시스템의 전체 구조는 [EDGAR의 모든 것](/blog/everything-about-edgar)에서 다룬다). 하지만 OpenDART가 제공하는 배당, 최대주주, 임원 보수 같은 구조화된 정기보고서 데이터는 EDGAR에서 얻으려면 직접 파싱해야 한다.
...
- [파이썬으로 재무제표 분석하기](/blog/python-financial-analysis) — OpenDART 데이터를 파이썬으로 실제 분석하는 방법
- [전자공시 대량 수집은 왜 느리고 어디서 막히나](/blog/bulk-download-and-rate-limit) — 호출 제한과 대량 수집 전략
