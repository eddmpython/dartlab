...
같은 기간 PUBG는 매 분기 수천억을 벌어다 줬다. 5년 누적 잉여현금흐름(잉여현금흐름) **29,352억**. 주주에게 돌아간 배당은 **0원**. 상환한 부채도 **0원**.
...
### 모바일 52% — 텐센트가 비용을 대고 크래프톤은 로열티만 받는다

5년간 매출 누적 **12.6조원**. PUBG가 그 중 **80%+**를 벌어다 줬다. 매출 구조를 뜯어보면:

| 플랫폼 | 2025 매출 비중 | 구조 |
|--------|-------------|------|
| 모바일 | **52%** (~17,300억) | 텐센트 운영 → 크래프톤 로열티 수취 |
| PC | 36% (~12,000억) | 스팀 직접 판매 + inZOI |
| 콘솔+기타 | 12% | |

핵심은 모바일 52%의 구조다. **텐센트**가 PUBG Mobile을 중국(피스키퍼 엘리트)과 글로벌에서 운영하고, 크래프톤은 **IP 로열티**를 받는다. 개발비·마케팅비·서버 비용을 텐센트가 지불하고 크래프톤은 돈만 받는 구조 — 이것이 영업이익률 30~44%의 비밀이다. PC(36%)도 스팀이 배포·결제·서버를 다 해주니 자체 인프라가 거의 필요 없다.

### 인도 MAU 1억 명 — 결제 앱까지 산 이유

이 구조에서 가장 재미있는 시장이 **인도**다. BGMI(Battlegrounds Mobile India) — 2020년 인도 정부가 PUBG Mobile을 개인정보 사유로 금지한 뒤, 크래프톤이 인도 서버+인도 데이터 저장이라는 조건으로 다시 출시한 버전. 인도 MAU **1억 명+**, 인도 모바일 게임 매출 1위. 캐시프리(결제 앱)를 776억에 인수한 것도 이 시장을 위해서다 — 인도 게이머의 인앱 결제 경험을 직접 장악하려는 베팅.

영업이익률이 29~44%를 오간다. 게임 산업에서 이 정도 마진은 비정상적으로 높다. PUBG라는 단일 IP가 한 번 만들어진 뒤 추가 개발 비용 없이 계속 돈을 벌기 때문이다. 넥슨(37%), EA(22%), Ubisoft(7%) 대비 크래프톤(31.7%)이 높은 이유가 이것이다 — PUBG가 "완성된 엔진"이다.
...
### 서브노티카 5,858억, 인도 결제 앱 776억 — 게임 밖의 쇼핑
...
### ADK 7,103억 — 일본 3대 광고회사를 게임회사가 산 이유
...
### 순이익-영업이익 갭 -3,207억 — 영업 밖에서 증발한 돈
...
| 2022 | 32,495억 | 11,024억 | 7,516억 | 23.1% |
...
### 인건비 2,840억+RSU 300억 — 13개 신작이 필요로 하는 개발자 비용

그런데 ADK 이전부터 비용이 꾸준히 올라가고 있었다. **인건비**가 핵심이다.

```python
c.analysis("financial", "비용구조")
```

| 연도 | 급여+복리후생 | 직원 수 | 1인당 인건비 |
|------|------------|--------|-----------|
| 2022 | 2,447억 | ~1,800명 | 1.36억 |
| 2023 | 2,575억 | ~2,100명 | 1.23억 |
| 2024 | 2,840억 | ~2,400명 | 1.18억 |

인건비 **2,840억** — 영업비용(15,273억)의 **18.6%**. 여기에 주식보상비용(RSU) **300억+**이 더해진다. 게임 개발자를 확보하려면 연봉+RSU 패키지가 필요하고, 13개 신작을 동시 개발하려면 사람이 필요하다. PUBG 하나로 돈을 벌 때는 인건비가 낮았지만, 포트폴리오를 넓히는 순간 비용 구조가 바뀐다.

**장병규 의장**은 스스로를 "겜알못(게임을 알지 못하는 사람)"이라 부른다. 크래프톤을 만든 것은 게임 감각이 아니라 **자본 배분 판단**이었다. "브렌던 그린을 데려와라"도 자본 배분, ADK 인수도 자본 배분, 3년 1조 환원도 자본 배분. 이 회사의 스토리는 게임이 아니라 **"PUBG가 번 돈을 어떻게 쓰는가"**다.
...
### 금융자산 2.6조 + 무형자산 1.8조 — 잠든 베팅의 현재 위치
...
2025년 3월 28일. **inZOI** 스팀 얼리 액세스 출시. 40분 만에 글로벌 판매 1위. **1주 만에 100만장** — 한국 패키지 게임 역대 최단. 판매의 95%가 해외. 27개국 동시 인기 1위. The Sims에 처음으로 진짜 경쟁자가 생겼다는 평가.

2025년 10월 출시한 **미메시스(Mimesis)**도 100만장 이상 판매. 현재 개발 중인 신작:

| 프로젝트 | 장르 | 상태 |
|---------|------|------|
| inZOI | 라이프 시뮬레이션 | 얼리 액세스 (100만장) |
| 미메시스 | 공포 어드벤처 | 출시 (100만장+) |
| 서브노티카 2 | 오픈월드 생존 | 개발 중 (언노운 월즈) |
| 다크 앤 다커 (모바일) | 던전 PvPvE | 개발 중 |
| 프로젝트 밸런스 | TPS | 내부 개발 중 |
| + 나머지 8개 | 다양 | 비공개 |

### 13개 프로젝트, 15개 스튜디오 — "5년 내 매출 7조" 비전

총 **13개 프로젝트**, 산하 스튜디오 **15개**. "5년 내 매출 7조" 비전. 이 목표는 PUBG 매출 유지(~2.5조) + 신작 포트폴리오(~4.5조)를 합산한 것이다. 그러나 현재 PUBG가 여전히 매출의 80%+. inZOI와 미메시스는 아직 PUBG의 보조 수준이다.

13개 프로젝트 중 매출이 실제로 잡힌 건 inZOI와 미메시스 2개뿐이다. 나머지 11개는 아직 베팅이다. PUBG가 매출의 80%인 상태에서, 이 베팅들이 매출로 전환되는 속도가 크래프톤의 다음 5년을 결정한다.

재무제표에서 inZOI가 아직 보이는가? PC 매출에 포함돼 있지만, 분기별로 분리 공시되지 않는다. PUBG가 여전히 PC 매출의 대부분. inZOI가 "PUBG 다음"이 되려면 반복 매출(라이브 서비스)로 전환돼야 한다 — 한 번 사면 끝인 패키지와 매 분기 돈이 들어오는 라이브는 재무제표에 찍히는 방식이 다르다.
