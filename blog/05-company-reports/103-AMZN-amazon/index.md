...
youtubeId: ""
ai:
  verdict: "간판은 매출 $716.9B의 세계 최대 이커머스 소매다. 그런데 dartlab 연결 재무가 증명하는 건 단 하나 — 이익이 매출에서 분리됐다. 연결 영업이익률이 정상 기저 ~5%대(2019 5.2%·2021 5.3%)에서 2025년 11.2%로 약 2.1배가 됐고, 2022→2023년 매출은 11.8%밖에 안 늘었는데 영업이익은 12.2B→36.9B로 정확히 3배 뛰었다. 더 판 게 아니라 이익 구조가 바뀐 것이다. 그 분리를 만든 엔진의 정체는 연결 숫자로는 못 짚는다 — 2025 10-K 세그먼트가 AWS 매출 $128.7B(전사 18%)·AWS 영업이익 $45.6B(전사 57%)를 보여준다. 광고는 매출 $68.6B(+22%)로 제2 엔진이지만 영업이익률은 별도 공시되지 않아 추정으로 봉합하지 않는다. 단, 2022 OPM 2.4%는 붕괴저점이라 출발선으로 쓰면 과장(6.6배)이 되므로 왜곡 없는 기저(5%대) 대비 약 2.1배로 읽어야 한다."
  direction: 개선
  confidence: 높음
  archetype: 성장
  ...
  weaknesses: ["진짜 엔진(AWS)과 광고 매출은 연결 손익 한 줄에 안 보여 소매 간판에 가려짐 — 세그먼트·서비스별 매출(외부) 의존", "광고는 매출만 공시되고 별도 영업이익률은 미공시 — 고마진 추정은 가능해도 숫자 단정 금지", "AI 인프라 투자로 2025 FCF $11.2B까지 감소 — 이익 엔진이 현금으로 곧장 주주에게 흘러간다고 말하면 과장", "2022 저점 기준 '6.6배'는 cherry-pick 과장 — 왜곡 없는 기저 대비는 약 2.1배", "BS 매핑 불안정 — 대차대조표 인용 주의"]
...
## 2막 — 제대로 된 기준선: 2022는 붕괴저점이지 출발선이 아니다
...
이건 더 판 게 아니라, *이익률이 구조적으로 높은 다른 매출이 그 사이 커졌다*는 신호다. 연결 숫자가 분명히 말할 수 있는 건 정확히 여기까지다 — **이익이 매출에서 분리됐다.** '무엇이 분리를 만들었나'는 연결 손익계산서에 한 줄도 안 적혀 있다. 그 안 적힌 엔진을 어디서 확인하나?
...
두 번째 실전 체크는 "어느 숫자가 미공시인가"를 표시하는 것이다. AWS 매출과 AWS 영업이익은 공시된다. 북미·해외 매출과 영업이익도 공시된다. 광고 서비스 매출은 공시된다. 하지만 광고 서비스 영업이익은 공시되지 않는다. 이 차이를 글 위에 표시해 두면 과장이 줄어든다. 숫자가 보이는 곳에서는 숫자로 말하고, 숫자가 보이지 않는 곳에서는 경계 문장을 붙인다. 이 원칙 하나만 지켜도 AMZN 글의 절반은 더 견고해진다.
...
다섯 번째 실전 체크는 "2022를 출발선으로 쓰지 않는 것"이다. 2022는 과잉투자와 비용 압박이 겹친 붕괴저점이다. 2022 OPM 2.4%에서 2025 OPM 11.2%를 비교하면 이야기가 너무 극적으로 보인다. 하지만 왜곡 없는 기준선은 2019~2021년 5%대다. 여기서 11%대로 올라온 것이 진짜 구조 변화다. 이 차이를 지키면 글이 덜 자극적이지만 더 강해진다. 과장된 숫자는 클릭을 만들 수 있지만, 검증표 앞에서 버티지 못한다.
...
| '6.6배'(2022 저점 기준)는 cherry-pick — 왜곡 없는 기저 대비 약 2.1배 | 기준연도 구분 | 정정/주의 |
