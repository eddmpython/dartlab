...
> 같은 시리즈: [삼성바이오로직스](/blog/207940-samsung-biologics) · [알테오젠](/blog/196170-alteogen) · [셀트리온](/blog/068270-celltrion) · [에스퓨얼셀](/blog/288620-sfuelcell) · [기업이야기 시리즈 전체](/blog/series/company-reports)
...
[삼성바이오로직스](/blog/207940-samsung-biologics)가 CMO(위탁생산) 공장으로 영업이익률 45%를 찍는 것과 비교하면, 같은 바이오 위탁생산인데 왜 결과가 정반대인가. 코로나 특수가 만든 "가짜 마진"의 구조를 분해한다.
...
왜 영업이익률 51%였던 회사가 4년 만에 -19%가 됐는가. 코로나 백신이라는 "한시적 특수"의 구조를 이해해야 한다.
...
### 왜 매출총이익률이 붕괴했는가 — "코로나 마진"의 정체

2021년 매출총이익률 61%는 정상이 아니었다. 코로나 팬데믹이라는 **비상 상황에서만 가능한 마진**이었다:
...
이 3가지가 모두 사라졌다. 코로나 백신 수요는 2022년부터 급감했고, AZ는 코로나 백신을 철수했고, 노바백스 계약도 종료됐다. 매출총이익률 61%를 만든 조건이 전부 소멸한 것이다.

### 분기별 영업이익률 — 7분기 연속 적자

| 분기 | 2023Q3 | Q4 | 2024Q1 | Q2 | Q3 | Q4 | 2025Q1 | Q2 | Q3 | Q4 |
|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 영업이익률(%) | **+26.3** | -9.3 | -126.2 | -74.5 | -64.3 | -32.4 | -9.8 | -23.1 | -12.8 | -28.0 |

2023Q3의 +26.3%가 유일한 흑자 분기였다. 독감 백신 + 대상포진 시즌 효과다. 그 뒤 7분기 연속 적자. 2025Q1이 -9.8%로 가장 양호했지만 흑자에는 못 미쳤다.

*영업이익률 51%는 코로나가 만든 것이었고, 코로나가 끝나자 함께 사라졌다. 그런데 매출은 돌아왔다. 왜 마진은 안 돌아오는가.*

---

## 2막: 원가율 88% — 매출이 돌아와도 마진이 안 돌아오는 이유

왜 매출 6,514억인데 영업이익률이 -19%인가. 두 가지가 동시에 작동하고 있다.
...
이것을 [삼성바이오로직스](/blog/207940-samsung-biologics)와 비교하면 구조가 보인다:

| 항목 (2025) | SK바이오사이언스 | [삼성바이오로직스](/blog/207940-samsung-biologics) |
|:---|:---|:---|
| 매출 | 6,514억 | 4.56조 |
| 영업이익률 | **-19.0%** | **45%** |
| 매출원가율 | 88% | 55% |
...
삼성바이오로직스는 4개 공장이 이미 가동 중이고, 글로벌 제약사(로슈·모더나·사노피 등)의 위탁생산 계약이 수년치 쌓여있다. **규모의 경제가 작동**해서 매출원가율이 55%, 영업이익률이 45%다. SK바이오사이언스는 아직 공장을 짓는 단계다. 매출은 자체 백신 판매에 의존하고, CDMO 매출은 시작 단계다. **규모가 안 되니 원가율이 높다.**
...
이 패턴은 [에스퓨얼셀](/blog/288620-sfuelcell)(원가율 127%, 매출 66% 폭락)과 비슷하지만, 결정적 차이가 있다 — **SK바이오사이언스는 매출이 돌아오고 있다.** 에스퓨얼셀은 매출 자체가 소멸하는 구조였지만, SK바이오사이언스는 매출은 반등했고 CDMO 투자가 성과를 내면 마진도 돌아올 가능성이 있다.

*문제는 "언제"냐다. 설비투자 2,604억이 매출로 전환되기 전까지 적자는 계속된다.*
...
> **용어 정리**: CMO = 남의 약을 대신 만들어주는 것(위탁생산). CDO = 남의 약을 대신 개발까지 해주는 것(위탁개발). CDMO = 둘 다 하는 것.
...
[삼성바이오로직스](/blog/207940-samsung-biologics)의 성공은 바이오 CMO/CDO 모델의 가능성을 증명했다. 글로벌 제약사는 자체 공장을 짓는 대신, 한국의 CMO에 생산을 맡긴다. 규모의 경제가 작동하면 영업이익률 40%+ 가 가능하다. SK바이오사이언스는 이 모델을 따라가려 한다.
...
독감 백신은 매년 시즌마다 반복 매출이 들어오는 "기저 매출"이다. 2023Q3에 유일하게 영업이익률 +26.3%를 찍은 것도 독감·대상포진 시즌 효과였다. 이 시즌 매출이 연간 기준으로 확대되면 원가율 개선의 첫 신호가 된다.

*코로나 이후 SK바이오사이언스의 전략은 "자체 백신 + CDMO"의 투 트랙이다. 문제는 두 트랙 모두 아직 규모가 안 된다.*

---

## 4막: 설비투자 2,604억 — 역대 최대 투자의 의미
...
| 설비투자 | **-2,604** | -1,722 | -499 | -722 | -428 |
| 잉여현금흐름 | **-1,579** | -3,002 | -205 | -1,972 | 4,938 |

**표시: 2021년 영업활동현금흐름 5,366억(코로나 캐시). 2025년 영업활동현금흐름 1,025억(흑자 전환). 하지만 설비투자 2,604억이 잉여현금흐름을 -1,579억으로 만든다.**

2021년 코로나 특수로 벌어들인 현금은 약 5,000억원이었다. 그 돈이 4년간의 적자와 설비투자 투자로 소진되고 있다. 2025년 영업활동현금흐름이 흑자로 돌아온 것은 긍정적이지만, 설비투자가 영업활동현금흐름의 2.5배라서 외부 자금(차입)으로 메우고 있다.
...
부채비율이 2023년 8.7%에서 2025년 45.1%로 급등했다. CDMO 공장 증설을 위한 차입이 원인이다. 하지만 **절대치 45%는 양호**하다. [한화오션](/blog/042660-hanwha-ocean)의 226%, [에스퓨얼셀](/blog/288620-sfuelcell)의 99%와 비교하면 매우 안전한 수준이다. 2021년 IPO로 확보한 자본 1.6조원이 방어벽 역할을 하고 있다.

### 현금 1,056억 — 2024년 대비 66% 감소

현금이 3,127억(2024) → 1,056억(2025)으로 급감했다. 설비투자 2,604억을 쏟으면서 현금이 빠진 것이다. 분기 영업적자가 약 -300억이고 설비투자가 분기 650억 수준이면, 현재 현금으로 약 1년은 버틸 수 있다. 다만 추가 차입이나 유상증자 없이는 2027년부터 유동성이 빡빡해진다.
...
dartlab 신용등급 dCR-BBB+. 투자등급이다. 건강점수 73.55는 양호하지만, **이자보상배율 -2.8배**가 하방 압력의 핵심이다. 영업이익이 마이너스라 이자를 갚을 영업이익이 없다. 투자 사이클이 끝나고 영업이익이 흑자로 돌아서야 이 등급이 유지된다.
...
왜 영업이익률 51%는 "가짜"였는가. 그리고 SK바이오사이언스에 "진짜 역량"은 있는가.

### 코로나 마진은 재현 불가능하다

2021년 영업이익률 51%를 만든 조건: ① 전 세계적 수요 폭발, ② 생산 능력의 희소성, ③ 정부 선구매 보장, ④ 기술이전 일회성 수익. 이 4가지가 동시에 작동하는 상황은 팬데믹 외에는 없다. **영업이익률 51%는 "다시 올 수 없는 마진"**이다. 이 숫자를 기준으로 SK바이오사이언스를 평가하면 영원히 "실망"할 수밖에 없다.

투자자에게 중요한 질문은 "영업이익률 51%로 돌아가는가"가 아니라 **"코로나 없이 정상적으로 벌 수 있는 영업이익률은 몇 %인가"**다. 삼성바이오로직스의 45%? 그건 15년 투자의 결과다. 일반 백신 제조사의 영업이익률은 보통 10~15% 범위다. SK바이오사이언스가 CDMO + 자체 백신으로 안착할 수 있는 정상 영업이익률은 아마 **10~20%** 사이일 것이다. 현재 -19%에서 거기까지 가려면 원가율이 88%에서 70% 이하로 내려와야 한다.

이것은 [HMM](/blog/011200-hmm)의 코로나 해운 특수(영업이익률 44%)와 같은 패턴이다. 해운도 코로나 시기에 운임이 폭등하면서 역대급 마진을 찍었고, 이후 정상화됐다. 차이는 HMM은 매출이 정상으로 돌아온 뒤 영업이익률이 6~8%로 안착했다는 것이고, SK바이오사이언스는 아직 적자라는 것이다.
...
| 항목 | [삼바](/blog/207940-samsung-biologics) | [알테오젠](/blog/196170-alteogen) | [셀트리온](/blog/068270-celltrion) | SK바이오사이언스 |
|:---|:---|:---|:---|:---|
| 영업이익률 | **45%** | **49.5%** | **15%** | **-19%** |
...
SK바이오사이언스는 바이오 4사 중 유일하게 **"아직 투자기"**다. 삼바는 15년간 투자해서 영업이익률 45%에 도달했다. 알테오젠은 17년간 R&D 적자를 거쳐 라이선스 수확기에 진입했다. SK바이오사이언스의 CDMO 투자가 성과를 내려면 최소 3~5년이 필요하다.

### IPO 자본의 역할 — 코로나 때 쌓은 돈이 J커브의 연료

2021년 IPO(기업공개)로 SK바이오사이언스는 자본 1.6조원을 확보했다. 코로나 백신 CMO 수익까지 합치면 약 2조원의 자본이 축적됐다. 이 돈이 없었으면 CDMO 공장 증설(설비투자 2,604억)은 불가능했다.

비유하자면 이렇다. 코로나는 "복권 당첨"이었다. 복권 당첨금으로 가게를 차린 것이 CDMO 투자다. 가게가 돈을 벌기 시작하면 복권 없이도 먹고살 수 있다. 가게가 안 되면 당첨금만 날린다. 지금은 가게를 인테리어하는 중이다 — 아직 손님이 많지 않지만, 인테리어 비용(설비투자)은 나가고 있다. [네이버](/blog/035420-naver)의 검색 광고처럼 "한 번 지으면 한계비용 ≈ 0"인 사업이 아니라, CDMO는 공장이 가동돼야 비로소 수익이 나는 자본집약적 사업이다.

*코로나 영업이익률 51%는 다시 오지 않는다. 하지만 그때 쌓은 자본과 인프라로 "진짜 사업"을 만드는 중이다.*
...
1. **분기 영업이익률 흑자 전환** — 2023Q3(+26.3%) 이후 7분기 연속 적자. 한 분기라도 흑자가 나오면 원가 구조 개선의 첫 신호. 독감·대상포진 시즌(3분기)이 가장 가능성 높다.

2. **원가율 80% 이하** — 현재 88%. 코로나 시절 39%는 불가능하지만, 80% 밑이면 매출총이익률 20%로 판관비를 감당할 수 있다. CDMO 가동률이 올라야 가능.

3. **CDMO 대형 계약 공시** — [삼성바이오로직스](/blog/207940-samsung-biologics)의 다년 CMO 계약처럼, 3~5년 장기 CDMO 계약이 수주잔고에 쌓이면 매출 가시성이 생긴다.

4. **대상포진 백신 허가** — GSK 싱그릭스의 독점을 깨면 자체 백신 매출이 급등할 수 있는 카드. 허가 일정과 임상 진행 상황이 핵심.

5. **현금 2,000억 이상 유지** — 현재 1,056억. 설비투자 부담이 지속되면 추가 차입 또는 유상증자 필요. 현금이 500억 밑으로 떨어지면 유동성 위험.

### 산업 패턴 — 바이오 CDMO의 "J커브"

바이오 CDMO는 전형적인 J커브 사업이다. 공장을 짓는 데 3~5년, 가동률이 올라가는 데 2~3년, 규모의 경제에 도달하는 데 5년+. 삼성바이오로직스가 15년 걸렸다. SK바이오사이언스는 이 J커브의 **바닥(투자기)**에 있다.

J커브의 바닥에서 투자자가 판단해야 할 것은 하나다: **"이 회사가 J커브를 완주할 체력이 있는가."** SK바이오사이언스의 체력: 자본 2.04조, 부채비율 45%, dCR-BBB+. [에스퓨얼셀](/blog/288620-sfuelcell)(현금 6억, Altman Z 0.20)과 비교하면 훨씬 여유가 있다. 코로나 특수 때 쌓은 자본이 J커브를 완주할 연료가 되는 구조다.

같은 J커브를 겪은 회사들의 결과를 보면 힌트가 된다. [삼성바이오로직스](/blog/207940-samsung-biologics)는 2011년 설립 후 7년간 적자였다가 2018년부터 흑자 전환, 2025년 영업이익률 45%에 도달했다. [알테오젠](/blog/196170-alteogen)은 2008년 설립 후 17년간 영업적자를 거쳐 2025년 영업이익률 49.5%를 찍었다. 바이오에서 J커브는 길다 — 하지만 완주하면 보상도 크다. SK바이오사이언스가 이 대열에 합류할 수 있는가, 아니면 [에스퓨얼셀](/blog/288620-sfuelcell)처럼 J커브의 바닥에서 체력이 다할 것인가. 그것이 다음 2~3년에 결정된다.

---

## 매출이 돌아와도 마진이 안 돌아오는 이유

SK바이오사이언스의 2025년은 "매출 반등, 마진 미회복"이다. 6,514억으로 2.4배 뛴 매출은 좋은 소식이다. 하지만 원가율 88%와 판관비율 30.6%가 매출 반등의 효과를 전부 먹어치우고 있다.

코로나 영업이익률 51%는 "가짜 마진"이었다. 팬데믹이라는 비상 상황이 만든 일시적 초과이익이었다. 2025년의 -19%가 "진짜 현실"이다. 여기서 J커브를 타고 올라가려면 CDMO 공장이 가동되고, 대형 계약이 쌓이고, 가동률이 올라가서 원가율이 80% 밑으로 내려와야 한다.

여기서 반복된 패턴이 있다. [에스퓨얼셀](/blog/288620-sfuelcell)은 "인증 없는 적자"로 소멸 경로에 있고, [뉴스케일파워](/blog/SMR-nuscale-power)는 "인증 있는 적자"로 기다림의 경로에 있다. SK바이오사이언스는 그 중간이다 — 코로나라는 "인증"은 만료됐고, CDMO라는 새 "인증"은 아직 완성되지 않았다. 자본 2.04조가 이 전환기를 버텨줄 수 있는 시간은 약 3~4년이다.

2026년에 봐야 할 한 줄: **CDMO 대형 수주 공시와 분기 영업이익률 흑자 전환.** 둘 중 하나라도 나오면 J커브의 변곡점이다. 둘 다 없으면 투자기는 더 길어진다. 코로나가 준 자본이라는 연료가 얼마나 오래 버틸 수 있는가 — 그것이 SK바이오사이언스의 시간이다.
...
| 2021 영업이익률 51.1% | `c.analysis("financial","수익성")` marginWaterfall | ✅ 실측 | |
| 2025 영업이익률 -19.0% | marginWaterfall | ✅ 실측 | |
| 2025 매출원가율 88.3% | marginWaterfall | ✅ 실측 | |
| 2021 매출총이익률 61.4% | marginWaterfall | ✅ 실측 | |
| 2025 매출총이익률 11.7% | marginWaterfall | ✅ 실측 | |
| 2025 영업이익 -1,235억 | IS 분기 합산 | ✅ 실측 | |
| 영업활동현금흐름 2025 +1,025억 | `c.select("CF",...)` 분기 합산 | ✅ 실측 | |
| 영업활동현금흐름 2021 +5,366억 | CF 분기 합산 | ✅ 실측 | |
| 설비투자 2025 2,604억 | CF 분기 합산 | ✅ 실측 | |
| 현금 1,056억 | `c.select("BS",["현금및현금성자산"])` 2025Q4 | ✅ 실측 | |
| 자본 20,375억 | BS 2025Q4 | ✅ 실측 | |
| 부채비율 45% | `c.analysis("financial","안정성")` leverageTrend | ✅ 실측 | |
| 이자보상배율 -2.8배 | `c.analysis("financial","종합평가")` summaryFlags | ✅ 실측 | |
| dCR-BBB+ | `c.credit("등급")` grade | ✅ 실측 | |
| 제품 65% / 상품 35% | `c.analysis("financial","수익구조")` segmentComposition | ✅ 실측 | |
| 판관비율 30.6% | summaryFlags | ✅ 실측 | |
| 삼바 영업이익률 45% | 삼성바이오로직스 블로그 #41 | ✅ 교차 | |

📅 dartlab 실측 2026-04-18

---

<CompanyFinancials code="302440" />
