...
여기까지 읽고 "코드를 직접 짜야 하나?" 싶을 수 있다. 안 짜도 된다. dartlab에는 AI가 있다. scan 결과를 직접 다루는 대신, **질문만 하면 AI가 scan을 포함한 모든 엔진을 알아서 조합한다.**
...
```bash
uv run dartlab ask "매출 성장률 상위 종목 중에서 현금흐름이 건전한 곳만 골라줘"
```

AI가 scan을 호출하고, 필터링하고, 결과를 해석해서 돌려준다. 위에서 코드로 직접 한 것과 같은 일을 **한국어 질문 한 줄**로 할 수 있다. 코드가 편하면 코드를 쓰고, 질문이 편하면 질문을 하면 된다.
