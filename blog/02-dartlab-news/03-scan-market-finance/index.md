...
"**자기자본수익률가 높은 종목을 찾고 싶다.**" 네이버 증권에서 종목을 하나 검색하고, 재무 탭을 열고, 자기자본수익률를 확인하고, 뒤로 가서 다른 종목을 검색하고... 이걸 2,700번 반복할 수는 없다.
...
2,700개 종목 중 자기자본수익률가 가장 높은 50개를 1초 만에 뽑는다. 같은 방식으로 `operatingMargin`, `debtRatio`, 아무 비율이나 바꿔 넣으면 된다.
...
1. `dartlab.scan("ratio", "roe")` 호출 — 전 종목 자기자본수익률를 뽑고
