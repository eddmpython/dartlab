...
12. 천 단위 콤마: 독자에게 수량으로 보이는 정수부 4자리 이상 숫자는 1,000, 12,345.67처럼 세 자리마다 콤마를 쓴다. 제목, description, 본문, 표, 캡션, 인용문, 검증표에 모두 적용한다. 연도·날짜·종목코드·버전·모델명·URL·코드·해상도 같은 식별자는 예외다.
...
7. 독자에게 수량으로 보이는 정수부 4자리 이상 숫자는 1,000처럼 세 자리마다 콤마를 쓴다. 연도·날짜·종목코드·버전·모델명·URL·코드·해상도는 예외다.
8. 마지막은 요약이 아니다. 독자가 다음 공시, 다음 차트, 다음 경제지표, 다음 회사 비교에서 확인할 한 줄을 남긴다.`
...
const DARTLAB_BEGINNER_NOTE = `dartlab 이야기 초보자 서사 게이트:
1. 독자는 전문가가 아니다. 글은 기: 어디서 막혔나, 승: 직접 실행해 첫 결과를 본다, 전: 그 결과를 잘못 읽기 쉬운 함정, 결: 이제 바꿔 볼 한 줄과 다음 편으로 닫는다.
2. "파사드", "스키마", "런타임", "정본", "계약", "엔진", "프로바이더", "컨텍스트", "아키텍처" 같은 전문 용어를 독자용 설명의 중심에 두면 실패다.
3. 꼭 써야 하는 용어는 바로 옆에 "쉽게 말해" 문장과 실제 코드·표·값을 붙인다.
4. 각 섹션은 독자가 처음 온 사람이어도 따라 할 수 있는 한 문장으로 시작한다. 뭘 누르고, 어떤 칸을 보고, 왜 다음으로 넘어가는지 말한다.
5. 마지막은 요약이 아니라 독자가 지금 바꿔 볼 입력값, 계정명, 기간, 회사 코드 중 하나로 닫는다.`
...
3-0. 초보자 서사: 기승전결을 교육형으로 쓴다. 기는 독자가 어디서 막혔는지, 승은 직접 실행해 첫 결과를 보는 장면, 전은 그 결과를 잘못 읽기 쉬운 함정, 결은 지금 바꿔 볼 한 줄과 다음 편 연결이다.
...
12. 초보자 언어: 독자는 내부 설계자가 아니다. "파사드", "스키마", "런타임", "정본", "계약", "엔진", "프로바이더", "컨텍스트", "아키텍처" 같은 말은 본문 중심어가 아니다. 꼭 필요하면 "쉽게 말해"로 풀고 실제 코드나 표의 어느 칸을 보면 되는지 붙인다.
...
- expert-jargon: 전문가 말투가 중심인가. OPM, EBITDA, CAPEX, FCF, ROE, PER, PBR, 밸류에이션, 컨센서스, 레버리지, 파사드, 스키마, 런타임, 정본, 계약 같은 말을 쉬운 설명과 실제 근거 없이 쓰나.
