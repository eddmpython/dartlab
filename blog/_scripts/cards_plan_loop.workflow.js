...
4. 쉬움·담백: 소리 내어 읽어 자연스러운 한국어. 설명 안 한 약어 금지(capex는 설비투자로 푼다). 단 AI는 일반어라 그대로 쓴다(인공지능으로 풀지 마라). 허황된 비유·추상 표현 금지. "돈곡선", "기대로 빌린 상승", "관심과 벌어지며 꺾이는 분기" 처럼 뜻을 비유로 감싸 두 번 읽어야 하는 문장은 실패다. 구체 주어(회사·숫자)에 사실을 붙이고 평이한 결론으로 닫되, 앞 문장과 뒤 문장이 원인·결과로 딱 붙게 쓴다. 매출은 "매출"이라 쓰고 곡선·흐름 같은 은유로 바꾸지 않는다.
...
7. 구조 독창성: 매 카드가 같은 뼈대를 반복하면 실패다. 특히 "화제·주가(관심)는 갔는데 매출·이익(돈)은 안 따라온다, 그 지표가 흑자로 도는 분기를 봐라" 식의 관심 대 돈 시계열 대비는 이미 여러 번 썼으니 기본값으로 쓰지 않는다. 주제마다 고유한 뼈대를 찾는다. 보기: 가치사슬에서 진짜와 이름표만 가르기, 첫 매출까지의 시간표, 공정별 집중도, 원인의 사슬, 승자와 패자의 분해, 널리 퍼진 오해 하나를 데이터로 정정. 주력 시각도 매번 finCard 붕괴선으로 가지 말고 뼈대에 맞는 형태(표·비교·구성)를 고른다. 직전 카드들과 뼈대가 겹치면 감점, 심하면 kill.
