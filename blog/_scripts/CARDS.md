...
- **비주얼(차트·표·그림)은 그 글이 이미 가진 것을 차용한다.** 중앙 `media/catalog.json`의 글 `diagrams`(마진 지도·사이클 차트 등), 본문 표, 실사 객체를 카드에 쓴다. 편집이 필요하면 `seedBlogMedia.py --post ...`로 로컬 staging을 임시 복원하고 다음 블로그 미디어 발행 성공 뒤 다시 삭제한다. 없는 데이터표를 게이트 통과용으로 억지로 만들지 않는다. 논지를 증명하는 그림 하나(예 8단계 마진 지도)가 숫자펀치 5장보다 낫다.
