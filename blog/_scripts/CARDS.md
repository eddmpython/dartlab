...
1. **작가 패널 토론(다중 에이전트)** — 서로 다른 렌즈(훅 강도·서사 스파인·앞장-다음장 연결·디자인/이미지 적합성·정직성)로 독립 검토 후 약점 합의. 나열형 체크리스트는 통과시키지 않는다.
...
> `cards.plan.json` 이 있는 글은 `planning.narrativeContract`, `reviewGate.status: "passed"` 와 각 required round `status: "passed"` 가
