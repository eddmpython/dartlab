---
title: "벡터 검색은 어떻게 기억을 찾나: 임베딩·RAG·클라우드가 나누는 AI 인프라 지도"
date: "2026-07-04"
description: "AI가 '의미로 검색'하는 원리(임베딩, 벡터 DB, RAG)를 풀고, 그 기술 스택을 반도체·클라우드·벡터 DB·기업 소프트웨어 회사로 나눠 DART와 EDGAR 근거에 연결했다. 기술 자체보다 데이터, 배포망, 연산 인프라가 재무제표에 먼저 남는 이유를 본다."
category: tech-story
series: tech-story
seriesOrder: 2
...
carousel:
  ...
  caption: |
    ...
    그 기술 스택을 반도체, 클라우드, 벡터 DB, 기업 소프트웨어로 나눠 DART와 EDGAR 근거에 연결했습니다. 자체 모델을 만드는 회사보다, 데이터와 배포망과 연산 인프라를 가진 회사가 재무제표에 먼저 흔적을 남겼습니다.
  ...
  slides:
    - layout: editorial
      line: "벡터 검색은 어떻게 기억을 찾나"
      sub: "임베딩·RAG·클라우드가 나누는 AI 인프라 지도"
    - layout: editorialBeat
      kicker: "간판마다 AI"
      line: "챗봇도 의미 검색도 추천도 같은 엔진을 씁니다. 그 엔진은 모델, 벡터 DB, 클라우드, 반도체로 나뉩니다."
      sub: "각 층위의 대표 회사를 공시 근거로 연결했습니다"
    - layout: editorialBeat
      kicker: "기술은 이미 상품"
    ...
    - layout: editorialBeat
      ...
      sub: "기술만이 아니라 데이터, 배포망, 연산 병목을 함께 봐야 합니다"
---
...
그런데 이상한 게 있다. 'AI'를 간판에 크게 내건 상장사들의 재무제표를 열어보면, 기술의 화려함과 숫자의 방향이 자주 어긋난다. 어떤 회사는 모델을 만들지만 손익계산서에는 비용이 먼저 보이고, 어떤 회사는 조용히 클라우드와 반도체 주문으로 수요를 받는다.

이 글은 두 가지를 나란히 놓는다. 먼저 벡터 검색이 실제로 어떻게 작동하는지 원리를 풀고, 그다음 그 기술 스택을 반도체, 클라우드, 벡터 DB, 기업 소프트웨어 회사로 나눠 DART와 EDGAR 근거에 붙인다. 걷다 보면, 기술 자체보다 데이터와 배포망, 연산 인프라가 왜 먼저 재무제표에 남는지 드러난다.

## 1. AI 스택의 손익계산서를 먼저 열어 보기
...
## 5. 공정 지도: 벡터 검색 스택은 어디에 걸리나

벡터 검색은 하나의 제품처럼 보이지만 실제로는 여러 층위가 이어진 공정이다. 한국사는 DART 손익계산서와 사업보고서, 미국사는 EDGAR 10-K의 사업 설명·세그먼트·CAPEX 문장을 함께 본다. 숫자 비교는 뒤에서 dartlab 실측으로 붙이고, 여기서는 먼저 어느 회사가 어느 층위에 서는지 세운다.

| 공정/층위 | 기술 역할 | 대표 회사 | 공시 근거 | 재무로 보이는 흔적 |
|---|---|---|---|---|
| 연산 반도체 | 임베딩과 추론을 실제로 돌리는 GPU·HBM | SK하이닉스(DART), NVIDIA(EDGAR) | DART 손익, EDGAR 10-K 데이터센터 설명 | AI 사용량이 메모리와 GPU 수요로 먼저 번역된다 |
| 클라우드 플랫폼 | GPU 클러스터, 스토리지, 벡터 검색 서비스를 묶어 판다 | NAVER(DART), Microsoft·Amazon·Google(EDGAR) | DART 사업보고서, EDGAR CAPEX·클라우드 세그먼트 | 데이터센터 투자와 클라우드 매출로 수요가 보인다 |
| 벡터 DB·검색 엔진 | ANN 인덱스와 검색 API를 제공한다 | Elastic·MongoDB·Snowflake(EDGAR), 오픈소스 생태계 | EDGAR 10-K 제품 설명, 오픈소스 프로젝트 | 기술은 표준화되고 구독·사용량 과금으로 남는다 |
| 기업 RAG 응용 | 고객 문서와 업무 흐름에 검색을 붙인다 | 더존비즈온·NAVER(DART), Palantir(EDGAR) | DART 사업 설명, EDGAR 플랫폼 설명 | 이미 가진 고객 데이터와 배포망이 마진 방어막이 된다 |
| 국내 순수 AI | 자체 LLM·챗봇·검색 솔루션을 판다 | 솔트룩스·마음AI(DART) | DART 손익계산서와 사업보고서 | 모델·인력·GPU 비용이 먼저 비용으로 드러난다 |

이 지도가 중요한 이유는 간단하다. "AI 회사"라는 한 단어로 묶으면 기술을 만드는 회사, 그 기술을 돌리는 회사, 그 기술을 기존 고객에게 배포하는 회사가 섞인다. DART와 EDGAR를 같이 봐야 이 층위가 갈라지고, 그래야 숫자가 왜 다르게 나오는지 설명할 수 있다.

## 6. 순수 AI 계층: 기술은 빠르지만 비용도 같이 커진다
...
## 9. 판단: 기술 스택에서 대체 불가능한 자리를 보라

원리에서 재무까지 걸어왔다. 이제 첫 질문에 답할 수 있다. 벡터·LLM 기술 스택에서 어느 층위가 재무제표에 먼저 남는가.
...
이것은 [모래가 반도체가 되기까지](/blog/sand-to-semiconductor)에서 본 법칙의 반복이다. 거기서는 부가가치가 공정 난도가 아니라 대체 불가능성에 앉았다. 여기서는 이익이 기술 자체가 아니라 데이터·유통·컴퓨트라는 대체 불가능한 자산에 앉는다. 하드웨어든 소프트웨어든, 상품화된 층위는 가격 결정력이 약하고 대체 불가능한 층위가 재무제표에 먼저 남는다는 같은 이야기다.
...
본문의 모든 회사별 수치는 아래 호출로 재현된다. 한국 상장사는 DART 기반 dartlab 실측, 미국 상장사는 EDGAR 10-K의 사업·세그먼트 설명을 공정 지도 근거로 붙였다. 스크린(`scan`)은 방향을 잡는 용도이고, 표의 매출·영업이익률은 손익계산서를 연간으로 합산한 실측이다. 데이터 기준: 2026-07-04, 2025년 연간까지 반영.
