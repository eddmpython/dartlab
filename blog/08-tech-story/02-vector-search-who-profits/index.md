...
같은 AI인데, 돈을 버는 회사들은 결이 다르다. 이들은 AI를 새로 만들어 판 게 아니라, 원래 잘하던 사업에 AI를 얹었다. 기업용 소프트웨어(ERP)에 AI를 붙인 더존비즈온, 검색과 커머스에 자체 LLM(하이퍼클로바)을 얹은 네이버를 실측했다.

```python
for code in ["012510", "035420"]:      # 더존비즈온, 네이버
...
격차가 극명하다. 더존비즈온은 영업이익률 28.6%, 네이버는 매출 12조에 영업이익률 18.3%다. 순수 AI 회사가 매출 100~400억에 적자를 내는 동안, AI를 얹은 강자는 조 단위로 벌고 20% 안팎을 남긴다.
...
| 더존비즈온 2025 매출 4,463억, 영업이익률 28.6% | `Company("012510").select("IS", [...])` | 실측 |
| 네이버 2025 매출 12조 350억, 영업이익률 18.3% | `Company("035420").select("IS", [...])` | 실측 |
