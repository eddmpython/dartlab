...
5. **소비자 기기·플랫폼은 제품 시스템으로 본다(필수).** 안경·로봇·스마트폰·웨어러블처럼 여러 부품과 사용 맥락이 묶인 기술을 한 부품 병목으로 축소하지 않는다. `핵심 기능 -> 전력·열·무게·공간·지연시간 -> 입력·프라이버시·유통/피팅 -> 기기·휴대폰·클라우드 역할 분담 -> 제품 성숙도·반복 사용`을 점검한다. 모든 축을 억지로 길게 쓰는 게 아니라, 독자의 질문과 결과를 실제로 바꾸는 제약을 `brief.json.sections[]`·`visuals[]`·본문에 연결한다. "최고 사양"과 "하루 쓸 수 있는 지속 성능", "플랫폼 지원"과 "실제 출하", "데모"와 "반복 사용"을 구분한다.
6. **서사 구조 = 6막 시간선 대신** `기술 질문 -> 원리 해부 -> (기기라면 제품 시스템 제약) -> 공정·회사 지도 -> 왜 이 회사가 이 칸에 있는가 -> 세대·경쟁 지형 -> 재무 흔적(대표 기업) -> 오해 바로잡기 -> 관전 시나리오`.
7. **오해 방지 단계(필수).** 기술 과대광고·세대 명칭 혼동·"차세대"라는 말의 실체 없음을 짚는다. 기술 성숙도(양산 단계인지 실험 단계인지)를 정직하게 표기한다.
8. **템플릿 제목 금지.** 제목과 H2는 "누가 돈을 버나", "왜 못 버나", "아직 적자" 같은 금융 결론형 문구를 반복하지 않는다. 기술의 구체 메커니즘, 공정 병목, 세대 변화가 제목의 중심이어야 한다.
9. **회사 종속 아님.** 기술이야기는 주어가 기술·테마라 subject join 키가 **topicSlug**다(`kind: tech`, `OPERATIONS.md` 5절). frontmatter 에 회사 `stockCode` 를 달면 블로그에 그 회사 단일 **터미널 버튼**과 **기업이야기 팟캐스트**가 잘못 조인된다. 여러 회사를 기술 관점으로 횡단하는 게 기본이므로 `topicSlug` 를 쓰고 `stockCode` 는 달지 않는다. 체크: (a) 원리 해부 (b) 공정·회사·근거 지도 (c) 회사가 그 칸의 핵심인 이유 (d) 재무 착지 (e) DART·EDGAR 근거 (f) 오해 방지 (g) 기술 성숙도·출처 (h) 기기라면 제품 시스템 제약 (i) `brief.json` 92점 루프 (j) 마지막 시나리오형 관전 포인트 (k) `publishGate.py` 통과.
10. **마지막 관전 포인트는 시나리오다.** `brief.json.watchScenarios[]`에 2~4개를 기획한다. 본문 마지막 H2에서 "만약 공급 병목이 풀리면 어떤 경로로 원가와 회사 위치가 달라질까", "만약 양산 전환이 늦어지면 어느 공시 숫자가 먼저 흔들릴까"처럼 조건별로 푼다. 각 시나리오는 변화 경로, 예상 결과, 확인 지표, 반증 조건을 가진다. 예측 숫자를 지어내지 않고 evidenceMap의 근거 범위에서만 쓴다.
11. **길이 = 밀도 우선.** 회사편 20,000자가 아니다. 원리도해·공정 지도·재무 실측표·재현 코드로 근거를 채운 **6,000~12,000자**. 패딩 금지.
12. **썸네일 = 통합 생성기.** `gen_blog_thumbnails.py`(레이아웃 SSOT, frontmatter 구동)를 써서 로컬 staging을 만든다. kicker 라벨은 `PREFIX["tech-story"] = "기술이야기"`. 별도 생성기를 만들지 않는다. `publishBlogAssets.py`가 최종 OG를 HF에 올리고 `ogImage`를 해시 URL로 바꾼다.
