...
![한 주식 조각에서 발행주식수 묶음을 지나 회사 전체 모형으로 확장되는 투자 작업대](./assets/03-thumbnail-bg.webp)
...
![시가총액은 주가와 발행주식수의 곱이라는 가장 중요한 정의를 한 장으로 고정한다](./assets/01-market-cap-formula.svg)
...
![1주 가격이 높아도 발행주식수가 적으면 작은 회사일 수 있다는 착시를 깬다](./assets/02-company-size-map.svg)
...
![주식분할은 한 조각 가격과 조각 수를 동시에 바꾸지만 전체 가치가 자동으로 늘지는 않는다는 점을 보여준다](./assets/04-stock-split.svg)
...
![종목 화면에서 주가 다음에 확인할 시가총액, 발행주식수, 거래대금, 희석, 밸류에이션 체크리스트](./assets/06-checklist.svg)
