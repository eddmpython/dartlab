...
v2.1 → v2.2 변경:
- 2024 XBRL 횡전개(기업명=열) 포맷 지원 추가
- extractTransposedProfiles(), extractTransposedMovements()
- 네이버/SK하이닉스/POSCO홀딩스 2024 횡전개 파싱
"""
...
def _isNameCell(text: str) -> bool:
    ...
    if any(sClean.startswith(kw) or sClean == kw for kw in _MOVEMENT_ITEM_KEYWORDS):
        return False
    # 긴 설명문은 기업명이 아님 (20자 이상 + "에 대한"|"설명"|"기술"|"내역" 등 포함)
...
def _findTransposedBlocks(rows: list[list[str]]) -> list[dict]:
    ...
    블록 구조:
    - 기업명행: 첫 셀 비어있고, 나머지 셀에 기업명 다수
    - 항목행: 동일 너비, 첫 셀이 항목키워드
    - 당기/전기 구분행: 셀수 <=2, "당기"|"전기" 포함

    Returns:
        [{period: "당기"|"전기", names: [...], items: {항목: [값...]}, blockType: "profile"|"movement"}]
    """
    blocks = []
    currentPeriod = None
    ...
    while i < len(rows):
        ...
        # 기업명 행 감지: 셀 수 >= 8, 첫 셀 비어있거나 카테고리, 나머지에 기업명 다수
        if len(cells) >= 8:
            first = cells[0].strip()
            isFirstEmpty = not first or first in ("전체 관계기업 투자금액", "전체 관계기업 및 공동기업",
                                                   "관계기업", "공동기업", "전체 공동기업")
            if isFirstEmpty:
                ...
                if nameCount >= 3 and nameCount >= len(cells) * 0.3:
                    ...
                    # 카테고리/그룹 행이 기업명 행 앞에 있을 수 있으므로
                    # 바로 앞 행 체크 (관계기업|공동기업 카테고리)
                    # 기업명 다음 행들을 항목행으로 수집
                    items = {}
                    blockType = None
                    j = i + 1
                    while j < len(rows):
                        ...
                        for k in range(1, len(jcells)):
                            vals.append(jcells[k].strip() if k < len(jcells) else "")

                        items[itemName] = vals

                        j += 1

                    # 블록 타입 판별: 항목 전체를 보고 결정
                    # movement: "기초"와 "기말" 둘 다 있어야 함
                    # profile: 지분율/장부금액/취득원가 중 하나 이상
                    movCount = 0
                    hasOpening = False
                    hasClosing = False
                    profCount = 0
        ...
        for idx, name in enumerate(names):
            ...
            for itemName, field in fieldMap.items():
                ...
                if field == "ownership":
                    ...
                    profile.bookValue = parseAmount(valStr)
                elif field == "acquisitionCost":
                    profile.acquisitionCost = parseAmount(valStr)
                elif field == "location":
