...
다만 EDGAR는 DART처럼 제도화된 장 체계가 아니라 `10-K`, `10-Q`, `20-F`, `40-F`의 form 구조를 그대로 살린다.

핵심 목표는 다음 세 가지다.

- `10-K`, `10-Q`, `20-F`, `40-F`의 section title / item title 전수조사
...
- `10-K`, `10-Q`, `20-F`, `40-F`를 같은 topicId로 억지 통합하면 정보 손실 가능성 큼
...
1. universe 정의
2. 다운로드 커버리지 확인
3. `10-K`, `10-Q`, `20-F`, `40-F`별 title frequency 전수조사
4. item 경계 안정성 평가
...
- `002_downloadFirst2000.py`
  - issuer-deduped collectible universe 2,000개를 2009년부터 배치 수집
  - progress에 `forms_found`, `rows_saved`, `filings_saved`, `full_document_rows`, `table_rows`, `failure_kind`를 함께 누적
...
- `10-K`, `10-Q`, `20-F`는 item 기반 section map으로 갈 수 있다
- `40-F`는 우선 annual fallback으로 수용하고, split은 별도 실험으로 분리한다
...
- false negative를 줄이려면 issuer-deduped collectible universe와 `40-F` 포함이 필요하다
...
결론:
- 057의 1차 상한 모집단은 exchange listed 7,525 ticker row다.
- 실제 장기 수집 대상은 이 중 issuer-deduped collectible universe로 좁혀야 한다.
...
- 40-F는 annual `Full Document` fallback으로 먼저 흡수하는 것이 맞다.
- 현 단계 패키지 배치는 `10-K/10-Q/20-F structured + 40-F fallback` 조합으로 올린다.
...
### 014_collectibleUniverseAudit.py 추가

- exchange listed ticker universe는 unique CIK보다 훨씬 커서 ticker 기준 배치는 중복 issuer를 다수 포함한다.
- `AG`, `AGI`, `ACB` 같은 foreign issuer는 `20-F`가 아니라 `40-F` + `6-K` 케이스가 존재한다.
- `6-K`는 정기 period 축이 아니므로 기본 docs batch 대상에서 제외하고, `40-F`를 annual docs form으로 포함해야 한다.

결론:
- docs collectible universe는 ticker가 아니라 issuer-deduped 기준으로 다시 정의해야 한다.
- false negative 제거를 위해 regular docs form에 `40-F`를 포함한다.

### 015_downloaderReadiness.py 추가

- progress 기록과 로컬 docs parquet를 함께 읽어 downloader readiness를 요약
- form별 `rows_saved`, `filings_saved`, `full_document_ratio`, `table_ratio`를 집계
- 실패는 `no_supported_regular_filing`, `fetch_error`, `parse_error`, `storage_error`, `legacy_env_error`로 운영 해석 가능하게 정리

결론:
- EDGAR docs downloader는 이제 "받았는가"만이 아니라 "어떤 품질로 저장됐는가"까지 지속 감시할 수 있다.
...
- batch progress만 읽어도 form별 저장 품질과 failure taxonomy를 해석 가능
