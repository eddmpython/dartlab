...
- **Ngram+Synonym 역인덱스가 최종 채택** (008)
- Polars 직접 검색(인덱스 없음)은 대규모에서 실용 불가 (009에서 확인)
- docs(사업보고서) 확장 시에도 ngram 인덱스 빌드 필요
- 인덱스 크기: 9K행에 5.9MB → 450만행 추정 ~300MB (JSON) → parquet 전환 검토
