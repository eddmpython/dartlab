"""
실험 ID: 012-003c
실험명: 5% 이상 주주 파서 수정 + 전체 검증

목적:
- 003b에서 발견한 문제 수정
- 핵심: "| 5% 이상 주주 | 삼성생명 | 513,535,800 | 8.60 |" 구조 처리
- 소액주주 키워드에 걸리지 않도록 섹션 종료 조건 강화

가설:
1. 첫 행에 "5% 이상 주주"가 구분 셀로 포함 → 바로 데이터 추출
2. "소액주주", "※", 빈 행 2개 → 섹션 종료
...
def parseFivePercentHolders(content: str) -> list[dict] | None:
    ...
    for line in lines:
        ...
        if not s.startswith("|"):
            if s.startswith("※") or s.startswith("주)") or s.startswith("주1") or s.startswith("주2"):
                continue
            if holders and s:
                break
            continue
        ...
        if "기준일" in cellJoined or "단위" in cellJoined:
            continue
        if "구분" in cells[0] and "주주명" in cellJoined:
            continue
    ...
    for line in lines:
        ...
        if not s.startswith("|"):
            if inTable and result:
                if "주식사무" in s or s.startswith("마.") or s.startswith("바."):
                    break
            continue
...
if __name__ == "__main__":
    for code in STOCKS:
        ...
        print("[5%이상]", "O" if fiveR else "X")
        if fiveR:
            for h in fiveR:
                shares = f"{int(h['shares']):,}" if h.get("shares") else "-"
                ratio = f"{h['ratio']}%" if h.get("ratio") else "-"
                print(f"  {h['name']}: {shares} ({ratio})")

        print("[소액주주]", "O" if minR else "X")
        if minR:
            print(f"  {minR['minorityHolderCount']:,}명 ({minR['holderRatio']:.1f}%) / 주식 {minR['shareRatio']:.1f}%")

        print("[의결권]", "O" if votR else "X")
        if votR:
            ti = votR.get("totalIssuedCommon")
            vt = votR.get("votableCommon")
            nv = votR.get("noVotingCommon")
            print(f"  발행={int(ti):,}" if ti else "  발행=-", end="")
            print(f" / 의결권없음={int(nv):,}" if nv else " / 의결권없음=-", end="")
            print(f" / 행사가능={int(vt):,}" if vt else " / 행사가능=-")
