...
// 리뷰 소음으로 무너진다. 파일 단위여야 레슨 하나의 diff 가 깨끗하다.
//
// 번들 비용(실측, 레슨 10 편 기준). 레슨 원본 14.1KB, 이것이 실리는 공유 청크는 raw 140KB /
// gzip 45KB 다. 즉 **대부분이 레슨이 아니라 `yaml` 파서**다. 레슨을 늘려도 파서 비용은 고정이고
// 레슨 텍스트만 선형으로 는다. 지금은 eager 로 한 진실(파일 하나)을 유지하는 편이 낫고,
// 레슨 원본 총량이 임계를 넘으면 `tests/audit/lessonSchema.py` 가 색인/본문 분리를 강제한다.
